...
export const KNOWLEDGE_CONTRACT_CASES = [
  ...
  { title: 'knowledge.contract.import.sanitized-import', minAssertions: 1 },
...
export const KNOWLEDGE_RULE_REGISTRY = [
  ...
  {
    id: 'knowledge.missing.import.draft-only',
    kind: 'missing',
    file: 'src/main/knowledge/KnowledgeCandidateService.ts',
    pattern: 'never creates a Candidate[\\s\\S]*persistDraftResult[\\s\\S]*putDraft',
    probe: 'source-pattern',
    note: 'knowledge ingest must persist session Draft only',
  },
  {
  ...
  },
  {
    ...
    pattern: 'allowed-tools:\\s*\\[subagent_report,\\s*turn_complete,\\s*artifact_read,\\s*knowledge_browse,\\s*knowledge_search,\\s*knowledge_read,\\s*knowledge_compile,\\s*read_image\\]',
    probe: 'source-pattern',
    note: '$knowledge-scout may report meaningful child progress and declare four read Knowledge tools, scoped artifact reads, read_image, and structured completion',
  },
  {
  ...
  },
  {
    ...
    note: 'executable Knowledge contract suite: durable store, deferred tools, skill intersection, human-only write, knowledge import, six lanes, fixed≠verified',
  ...
  {
    id: 'knowledge.forbidden.import.auto-candidate',
    kind: 'forbidden',
    file: 'src/main/knowledge/KnowledgeCandidateService.ts',
    pattern: 'persistDraftResult[\\s\\S]*putCandidate',
    probe: 'source-pattern',
    note: 'knowledge persistDraftResult must not create a Candidate',
  },
  {
