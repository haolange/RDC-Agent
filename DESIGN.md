...
The effort control sits immediately left of the context usage ring. It is a pill that opens an upward popup containing one capability-driven reasoning rail plus two switches: `Max context` (context window expansion) and `Fast mode` (fast variant switch). `Auto` is not a product state. The reasoning rail is generated from `reasoningControl`: `none` renders a single locked `Off`; `toggle` renders `Off | On`; `always-on` renders a single locked `On`; `levels` renders only the official named levels for that provider/model, and prepends `Off` only when the model can really disable reasoning. Product naming is canonical: `On`, `Minimal`, `Low`, `Medium`, `High`, `Extra`, `Max`, `Ultra`. Provider wire spellings such as `xhigh` stay in the mapping layer and must not leak into UI. Dragging is continuous: the thumb follows pointer position, preview color follows the level that would be committed, and pointer release selects `round(ratio * (visibleStops - 1))`, clamps it to the visible subset, and animates the thumb back to that stop. Release must keep a snap hold so `turnControls` commit cannot briefly drop below `Max`/`Ultra`. Ordinary levels use one solid `--token-effort-*` tone across the full track with stop dots always visible. Only product top-tier `Max` (and `Ultra` when a model exposes it; same visual pipeline) uses a translucent dark-gray rail plus a soft lavender canvas pixel-field evolution matching the reference grow sequence: while dragging on Max, stop dots stay visible and a tight irregular pixel cluster previews near the thumb; on release (or keyboard/click commit), one shared ~800ms symmetric progress clock grows a stable coordinate-noise field leftward as a dithered dissolve cloud while stop dots fade out. The field is layered above the stops so mid-growth dots remain visible only through pixel gaps. Settled Max keeps a textured near-full field with fixed occupancy, right-brighter luminance, light per-cell shimmer, and no stop dots. Closing and reopening the popup while still on Max replays the evolution with stop dots hidden from the first frame; leaving Max uses the same stable topology and shared clock in reverse, spatially retreating energy from left to right while restoring stop dots from their current opacity over ~800ms. It must never use a whole-field opacity fade, a dark-to-solid track crossfade, or per-frame noise reshuffling. Purple emphasis may appear on the closed pill label. `prefers-reduced-motion` jumps to a static dense Max field (no stop dots) or ordinary solid fill with no continuous animation. Unsupported reasoning states are not rendered as dead stops. `Max context` and `Fast mode` rows remain visible and render only label plus switch; disabled state is expressed by the gray disabled row and switch, without visible reason copy, numeric context ranges, fast model ids, or other detail copy. Reasoning level, `Max context`, and `Fast mode` are remembered per `agentId + providerId + modelId` within the current session UI, snapshotted into each send request as `turnControls.reasoningLevel`, `turnControls.maxContextMode`, and `turnControls.fastModel`, and persisted on the session as the active route's last-sent controls.
...
Provider catalog ownership is explicit. `login-authorization`, `official-direct`, `cloud-platform`, `official-compatible`, and `coding-token-plan` providers are `app-managed`: RDC-Agent owns the model list and capability table, and provider connection/refresh only validates account or API availability. By default, unavailable app-managed catalog models remain visible as disabled rows. `volcengine-coding-plan` is a deliberately narrow product exception: only canonical catalog models matched by the authenticated `/models` response enter the visible and persisted list; partial matches may be saved, while zero matches report authenticated-but-unsupported and fail closed. Existing routes that reference a filtered model keep their selected id but become invalid until the user chooses another model—runtime must never switch models silently. `third-party-compatible`, `local`, and `image` providers are `user-managed`: model lists come from the endpoint or user configuration, and RDC-Agent uses conservative capability defaults instead of applying the mainline provider table.
...
The context breakdown popover anchors above the composer shell at full input width. The summary is always visible in a Cursor-style hero: large `{n}% Full`, `~occupied / window Tokens`, and a full-width multi-color stacked bar using `--token-context-*` colors, plus quiet This run stats. The grouped detail legend collapses by default (token counts only, no per-row percent). Expand state is UI memory persisted as `appearance.contextBreakdownExpanded` in `~/.rdx/config.json`, driven only by the popover toggle—it is not a Settings-page option.
