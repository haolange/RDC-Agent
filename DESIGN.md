...
- **子代理可见记录**：一次委派以父 `subagent` tool call 为唯一 Work Process 卡片锚点。子执行步骤按父会话中的独立委派记录增量持久化，由 child session / Task execution / generation 关联；父消息只存关联与摘要。同步和后台都在同一卡片内依次展示委派任务、实际可见的工作步骤、结果摘要；无步骤时省去空过程。第二层「调用与回执」默认收起，先展示存在的标识与记录索引，调用 JSON 和长回执按条展开。后台记录由 Task 执行生命周期更新，不受父回复结束约束；隐藏推理不进入记录。
