...
A live context value may grant a selectable Max mode only when `liveProjection.context.authority` is `account-effective` and the observed account limit is at least one million tokens. A `model-catalog` capacity observation without separate entitlement remains `Unverified`; it cannot manufacture account permission. Missing context, protocol, or reasoning effort metadata stays unknown and never receives a guessed 256K window, OpenAI-compatible route, or `high` effort.
...
Kimi Coding Plan uses stable product identities independently of backend rollout versions. `kimi-for-coding` remains the canonical selectable alias even when its implementation changes; backend names may appear only in provenance or desensitized diagnostics and must never create a selector row, alias, persisted route, or migration. The account-authoritative catalog may additionally admit exact `k3` and the internal `kimi-for-coding-highspeed`: `k3` is selectable only while the credential-scoped catalog returns that exact id, while HighSpeed is never selectable and exists only as the Fast execution target. Direct Moonshot `kimi-k3`, OpenCode Go `kimi-k3`, and OpenRouter `moonshotai/kimi-k3` remain independent surface-local identities. Live catalogs may narrow entitlement and publish exact reasoning/context facts, but a display name or backend version can never cross a surface boundary.
