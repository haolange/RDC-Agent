...
Dragging the reasoning rail is continuous: the thumb follows pointer position, preview color follows the level that would be committed, and pointer release selects `round(ratio * (visibleStops - 1))`, clamps it to the visible subset, and animates back to that stop. Release must keep a snap hold so `turnControls` commit cannot briefly drop below `Max`. Ordinary levels use one solid `--token-effort-*` tone across the full track with stop dots always visible. Only product top-tier `Max` uses the translucent dark-gray rail plus soft lavender canvas pixel-field evolution: a stable coordinate-noise field grows leftward over the shared ~800ms clock while stop dots fade, and reverses through the same topology when leaving the tier. It must never use a whole-field opacity fade, a dark-to-solid track crossfade, or per-frame noise reshuffling. Pointer input is coalesced to one update per frame; the leaf canvas/ref owns animation state and stops RAF when the popover closes, the page is hidden, the visual reaches a stable state, or reduced motion is active. The `1M context` and `Fast mode` rows stay visible, terse, and capability-driven. Fixed 1M is rendered on and disabled; unsupported is off and disabled; a route change keeps the row pending and disabled until a capability response with the current route revision arrives. Reasoning, `1M context`, Fast, and model-level route choices are remembered per `agentId + providerId + modelId`, snapshotted into the next turn, and never modify a running turn.
