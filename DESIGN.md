...
Settings scoped editors (Skills / Tools MCP / Hooks / Policy) are peer navigation sections. Each uses a full-width User | Project scope row, then an Agents-style master-detail body: Import + New toolbar, resource list, and a labeled detail editor. Agents keeps the same scope row without a duplicate New; create/import stay on the Agents toolbar only. They do not show a decorative “RDX Runtime” kicker. Workspace path cards and Control Panel runtime context remain separate surfaces and may still name the RDX Runtime roots. Settings must not restore a Diagnostics navigation section.
...
Skills use the standard directory form `skills/<skill-id>/SKILL.md` with optional `scripts`, `references`, and `assets`. The runtime uses one Progressive Skill surface: a bounded metadata catalog is always injected when non-empty (budget via `SkillCatalogBudget`, ~2% of the context estimate or 8000 characters fallback; overflow emits diagnostics without truncating full skill bodies). There is no lean/standard harness mode and no `.agent.md` `harness` field.

Full skill bodies preload from three sources before the first model call: `.agent.md` `skills`, composer `$skill-id` mentions in the user message, and session-scoped `/skills <id>` (or `/skills run <id>`) arming. Missing ids fail closed. An empty profile `skills` list means no author preload, not no discovery. Other effective skills remain discoverable through the short index and the core `skills` / `skill_read` tools.

Skill `allowed-tools` may narrow but never expand the effective profile tool set. Reference files are lazy and scripts execute only through normal tool and permission policy.
...
`PromptPlan` is the complete instruction plan. Each segment has an id, kind, scope, source path, source hash, precedence, content, token estimate, and a stability marker (`stable` or `volatile`); stable segments must form one contiguous prefix whose fingerprint (`stablePrefix`) anchors prompt-cache reuse, with volatile runtime facts appended after it. It combines source-controlled core contracts, the effective agent, the active `RDX.md` chain, preloaded skills, the Progressive Skill discovery catalog (omitted only when empty; budget from `SkillCatalogBudget`), the effective tool list, permissions, and runtime facts. Capability prose is generated from actual effective tools; generic hardcoded capability claims are forbidden.
...
Work Process product copy separates the top-level transcript header from loop thinking folds. The header uses “工作中” (English: Working) with Active Signal while the turn is running, and “工作过程” (English: Work process) after settle, plus duration/action meta. Loop thinking folds use a quiet spark icon before the label, “正在思考” (English: Thinking) while streaming/running, and “已思考 · {duration}” (English: Thought for {duration}) after settle—or “已思考” / “Thought” when duration is unavailable. Do not use “深度思考” or settled “思考了”. Readable `raw`, `summary`, and `unknown` thinking share these labels; the UI must not scare users with “语义未验证” or split copy such as “原始思考”. Future dedicated Debug View surfaces may still expose the contract `semantic` from desensitized snapshots. Loop commentary is narrative prose rendered with markdown (never promoted into the thinking slot). Final answers render only in the assistant message body as full-bleed prose—not a raised bubble.

Providers that officially expose multiple wire protocols must offer an explicit model-level protocol selector (`routeOptions`) that distinguishes `AnthropicMessages`, `OpenAICompatibleChatCompletions`, and `OpenAIResponses`. Protocol selection is never a provider-row toggle: Settings lists may summarize “multi-protocol” with the surface default, while the selectable enum lives on each model. A fixed model route wins; otherwise the user’s `preferredRouteOptionId` wins; otherwise the compiled surface default. Unsupported combinations stay disabled and fail closed. Switching route does not rewrite persisted Session/Turn controls.
...
Desensitized `RequestEnvelopeSnapshot` records remain on disk under the app `llm-calls` store and are readable through `rdx-runtime:listSnapshots` / `getSnapshot` IPC for a future dedicated Debug View (Copilot Chat Debug View class: command palette / chat overflow, never Settings nav). There is no Settings Diagnostics product entry and no Request Inspector UI in the current product surface. Snapshots must not mount in the default Session or Trace right panels, must not render inside the Work Process transcript body, and must not compete with thinking/tool evidence for attention. Snapshots are provider-neutral redacted envelopes (not Work Process loop meta, not Terminal runtime logs) and never expose credentials or protected reasoning payloads.
...
Settings → **Appearance** is a first-class nav section (peer to General). Settings page chrome uses a single-line header (section title + close), with no page-level marketing subtitle under the title. Content cards provide the visual weight below the hairline.

**Outer mode**: `appearance.theme` is `dark` | `light` | `system`, resolved to `data-resolved-theme`. Language stays in General. Mode tiles show a mini window chrome mock (title bar + sidebar + content + accent pill); System is a split Light/Dark mini mock.

**Live preview**: a dual-pane visual mini UI (sidebar nav + content skeleton + accent CTA), not a JSON/code dump. Each pane injects local `--ap-surface` / `--ap-ink` / `--ap-accent` from the corresponding Light/Dark `ThemeChromeConfig` so edits refresh the preview without mutating global `:root`. Light theme / Dark theme editor cards follow the current App chrome surface (they do not force local light/dark card backgrounds).

**Global chrome** (`appearance.chromeThemes.light|dark`): independent Light/Dark `ThemeChromeConfig` with `presetId`, `accent`, `surface`, `ink`, `contrast` (0–100), and `fonts.{ui,code}`. Built-in presets are RDC (product default) plus Absolutely / Ayu / Catppuccin / Dracula / Everforest / GitHub / Gruvbox / Linear. `ThemeChromeCompiler` patches `:root` CSS variables; Import/Copy uses portable `rdx-theme-v1:` strings (never parse `codex-theme-v1:`). Translucent / semi-transparent sidebar is **not** a product setting. Color fields use a round swatch + hex input; presets use `DropdownSelect` with an Aa accent swatch (no native `<select>` greying).

**Preferences** in Appearance (defaults unchanged where noted):

- `fontScale`: `small` | `medium` | `large`
- `composerMarkdown` (default **off**): Markdown highlight plus Write/Preview; sent user bubbles also render through `MessageMarkdown`
- `usePointerCursors` (default **off**): `html[data-pointer-cursors='true']`
- `reduceMotion`: `system` | `on` | `off` (default `system`)

**Compose agent system** (excluded from global chrome dyeing): each agent stores `accent` in `.agent.md` / Settings → Agents. Composer injects `--composer-mode-accent` and derived `--composer-effort-*` for shell edge glow, energy ring, mode pill, send, and Effort/Max. Global Appearance accent must not recolor compose Effort; Light/Dark only modulates luminance of agent-derived colors. Context breakdown (`--token-context-*`) remains global.
...
Provider lifecycle is a truthful admission contract, not a progress label. A provider is `stable` and available only when its configured authentication mode, route, discovery path, request translation, and deterministic contract fixtures are complete. A provider whose upstream login or catalog contract cannot be verified is unavailable with one shared reason in Settings and Runtime; it must not register a guessed flow, expose a dead control, or remain as a user-visible half implementation. Connection Test validates credentials and writes the same credential-scoped live discovery layer that Connect/Refresh use into `EffectiveCatalogService` (no second HTTP discovery after a successful Test payload); Connect persists secrets and connection state. Settings model-row badges reflect Effective Catalog `availability` only: show Loading while `refreshing`, never flash Unverified during that window.
...
The headless browser-session process and the visible Electron desktop are parallel surfaces over the same canonical runtime state. Headless mode must not own Electron's visible desktop single-instance lock; its localhost bridge port is its singleton boundary. All `pnpm run start:*` scripts and the platform wrappers `scripts/start-rdc-agent.cmd|.sh` / `scripts/run-rdc-launcher.cmd|.sh` share `scripts/launch-rdc-agent.mjs`; the wrappers only locate Node and forward arguments. Visible desktop launchers must always open or focus a real app even while Browser Use QA is running.
