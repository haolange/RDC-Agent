...
| **目标态** | 与 Wave 1 拓扑相同。durable handoff 状态机已落地。三条 Mission 方法面已接到 Skill / Hook / Capsule。Investigation schema / Service / 工具、15 个垂直方法 Skill、4 个 builtin Hook 模板与 Session rail 五卡已落地。不新增 `mission` / `orchestratorType` / `investigationMode` 等 Profile 领域字段。产品级 Browser QA 与本机 ColdData 真实验收仍待 Wave 6。 |
...
| **迁移门禁** | 禁止把上述字段写入 `TaskRecord` / `AgentProfile` / `ConversationMessage`。垂直记录只能引用 task id。禁止新建平台级 Graph Service。`pnpm run check:investigation-system` ratchet 已建立；schema / Service / contract suite / 五卡已清零，15 Skill / 4 Hook 已落地，三条 Mission 方法面已接到 Skill / Hook / Capsule，债务只减不增。产品级 Browser QA 与本机 ColdData 真实验收仍待 Wave 6。 |
