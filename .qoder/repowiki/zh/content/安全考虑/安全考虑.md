# 安全考虑

<cite>
**本文引用的文件**
- [AgentPermissionPolicy.ts](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts)
- [ShellCommandRiskAnalyzer.ts](file://src/main/agent-runtime/permissions/ShellCommandRiskAnalyzer.ts)
- [shellFileToolBypass.ts](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts)
- [IpcPayloadGuard.ts](file://src/main/ipc/validation/IpcPayloadGuard.ts)
- [bridgeSecurity.ts](file://src/main/browserAppBridge/bridgeSecurity.ts)
- [SecretStorageService.ts](file://src/main/settings/SecretStorageService.ts)
- [secretRedaction.ts](file://src/main/runtime/secretRedaction.ts)
- [AttachmentStagingService.ts](file://src/main/conversation/AttachmentStagingService.ts)
- [ToolResultArtifactizer.test.ts](file://src/main/agent-runtime/tools/ToolResultArtifactizer.test.ts)
- [ShellTool.ts](file://src/main/agent-runtime/tools/primitives/ShellTool.ts)
- [EditFileTool.ts](file://src/main/agent-runtime/tools/primitives/EditFileTool.ts)
- [WriteFileTool.ts](file://src/main/agent-runtime/tools/primitives/WriteFileTool.ts)
- [fileReadAccess.ts](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts)
- [EffectiveCatalogService.ts](file://src/main/settings/EffectiveCatalogService.ts)
- [permissions.md](file://docs/contracts/permissions.md)
</cite>

## 更新摘要
**变更内容**
- 新增 shell 命令绕过检测机制，防止通过常见文件操作命令绕过专用工具
- 实现读前写验证机制，确保编辑和覆盖文件前必须先成功读取同一真实路径
- 新增 SHELL_FILE_TOOL_BYPASS、RDC_TOOL_VIA_COMMAND_DENIED、READ_BEFORE_EDIT_REQUIRED 等错误代码
- 增强文件访问控制，在权限策略层拦截潜在的命令注入和工具绕过尝试

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与安全权衡](#性能与安全权衡)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：配置与审计清单](#附录配置与审计清单)

## 简介
本文件面向 RDC-Agent 的安全设计，聚焦权限模型、沙箱机制、输入验证、输出清理、敏感信息处理、网络安全、数据传输加密与存储安全，并提供安全配置、漏洞扫描与安全审计的最佳实践，以及常见威胁防护方法与应急响应流程。文档基于仓库中已实现的安全边界与策略进行说明，避免臆测未实现的特性。

**更新** 本次更新重点增强了 shell 命令绕过检测和文件读取访问控制，包括新的错误代码和读前写验证机制。

## 项目结构
RDC-Agent 的安全能力分布在主进程（main）的多个子系统：
- 权限与策略：AgentPermissionPolicy、ShellCommandRiskAnalyzer、PolicyCompiler（编译策略）
- Shell 绕过检测：shellFileToolBypass（命令路由表、嵌套命令深度限制）
- IPC 校验：IpcPayloadGuard（Zod 模式 + 大小限制）
- 浏览器桥安全：bridgeSecurity（通道能力矩阵、令牌与同源校验）
- 密钥与凭据：SecretStorageService（safeStorage 加密）、secretRedaction（脱敏）
- 会话附件与工具输出：AttachmentStagingService（文件名清洗、Base64 严格校验）、ToolResultArtifactizer（大输出隔离与引用）
- 文件访问控制：fileReadAccess（读前写验证、successfulFileReads 跟踪）
- 网络与提供者：EffectiveCatalogService（发现条目净化）
- 协议与契约：permissions.md 定义稳定安全边界

```mermaid
graph TB
subgraph "权限与策略"
P["AgentPermissionPolicy"]
SRA["ShellCommandRiskAnalyzer"]
SFTB["shellFileToolBypass"]
end
subgraph "文件访问控制"
FRA["fileReadAccess"]
EFT["EditFileTool"]
WFT["WriteFileTool"]
end
subgraph "IPC 校验"
IPG["IpcPayloadGuard"]
end
subgraph "浏览器桥"
BS["bridgeSecurity"]
end
subgraph "凭据与脱敏"
SS["SecretStorageService"]
SR["secretRedaction"]
end
subgraph "附件与输出"
AS["AttachmentStagingService"]
TA["ToolResultArtifactizer"]
end
subgraph "网络与提供者"
EC["EffectiveCatalogService"]
end
P --> SRA
P --> SFTB
SFTB --> FRA
FRA --> EFT
FRA --> WFT
IPG --> P
BS --> IPG
SS --> P
SR --> P
AS --> P
TA --> P
EC --> P
```

**图表来源**
- [AgentPermissionPolicy.ts:428-446](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L428-L446)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [EditFileTool.ts:71-71](file://src/main/agent-runtime/tools/primitives/EditFileTool.ts#L71-L71)
- [WriteFileTool.ts:74-75](file://src/main/agent-runtime/tools/primitives/WriteFileTool.ts#L74-L75)

章节来源
- [permissions.md:78-84](file://docs/contracts/permissions.md#L78-L84)
- [permissions.md:126-126](file://docs/contracts/permissions.md#L126-L126)

## 核心组件
- 权限策略与风险分类
  - AgentPermissionPolicy：对工具调用与 shell 命令进行风险评估，输出 allow / auto_review / ask_user / deny 决策，并维护临时路径根白名单。非沙箱边界，真正强制由硬拒绝列表、OS 权限、Electron 沙箱与 IPC 校验完成。
  - ShellCommandRiskAnalyzer：解析命令结构、匹配高危模式与危险命令名，用于审批路由。
  - **新增** shellFileToolBypass：检测 shell 命令中的文件操作绕过尝试，将 cat/grep/find/sed 等命令重定向到专用工具。
- IPC 载荷校验
  - IpcPayloadGuard：对所有 IPC handler 参数进行 Zod 模式校验与字节长度限制，非法即 fail-closed。
- 浏览器桥安全
  - bridgeSecurity：通道能力矩阵、Bearer/Cookie 鉴权、同源校验、高影响操作需显式环境变量放行。
- 凭据与脱敏
  - SecretStorageService：使用 safeStorage 加密存储凭据，损坏隔离，文件权限收紧；对外仅暴露 has/masked 预览。
  - secretRedaction：递归脱敏请求体与日志中的敏感键与文本模式，记录脱敏轨迹。
- 附件与工具输出
  - AttachmentStagingService：文件名清洗、保留字符过滤、Windows 保留名拒绝、严格 Base64 解码。
  - ToolResultArtifactizer：超大工具输出转存为受控 artifact，通过引用与哈希精确读取，防止内存与传输膨胀。
- 文件访问控制
  - **新增** fileReadAccess：跟踪 successfulFileReads，实现 READ_BEFORE_EDIT_REQUIRED 验证。
  - **新增** EditFileTool/WriteFileTool：在编辑或覆盖文件前验证是否已成功读取同一真实路径。
- 网络与提供者
  - EffectiveCatalogService：持久化目录状态净化，过滤不受信任的发现条目。

章节来源
- [AgentPermissionPolicy.ts:428-446](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L428-L446)
- [ShellCommandRiskAnalyzer.ts:1-7](file://src/main/agent-runtime/permissions/ShellCommandRiskAnalyzer.ts#L1-L7)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [EditFileTool.ts:71-71](file://src/main/agent-runtime/tools/primitives/EditFileTool.ts#L71-L71)
- [WriteFileTool.ts:74-75](file://src/main/agent-runtime/tools/primitives/WriteFileTool.ts#L74-L75)

## 架构总览
下图展示从渲染器到主进程的关键安全路径：渲染器通过 preload 暴露受限 API，所有 IPC 经 IpcPayloadGuard 校验；主进程根据权限策略与 shell 风险分析决定执行或审批；浏览器桥在 QA/调试模式下提供受限 HTTP/SSE 入口，具备通道能力与同源校验；凭据通过 safeStorage 加密存储，输出与日志经脱敏。**新增** 的 shell 绕过检测和文件访问控制在权限策略层和工具执行层双重保障。

```mermaid
sequenceDiagram
participant UI as "渲染器/浏览器"
participant Preload as "Preload"
participant Bridge as "BrowserAppBridge"
participant IPC as "IPC Handlers"
participant Policy as "AgentPermissionPolicy"
participant Bypass as "shellFileToolBypass"
participant Access as "fileReadAccess"
participant Store as "SecretStorageService"
participant Redact as "secretRedaction"
UI->>Preload : 调用受限API
Preload->>IPC : 发送IPC消息(带Zod校验)
IPC->>Policy : 评估工具/命令风险
Policy->>Bypass : 检测shell命令绕过
Bypass-->>Policy : SHELL_FILE_TOOL_BYPASS/RDC_TOOL_VIA_COMMAND_DENIED
Policy->>Access : 验证文件访问权限
Access-->>Policy : READ_BEFORE_EDIT_REQUIRED
Policy-->>IPC : allow/auto_review/ask_user/deny
IPC->>Store : 获取凭据(加密)
IPC->>Redact : 脱敏日志/响应
IPC-->>UI : 返回结果(已脱敏)
Note over Bridge,IPC : 浏览器桥通过通道能力与同源校验访问相同主进程能力
```

**图表来源**
- [IpcPayloadGuard.ts:39-57](file://src/main/ipc/validation/IpcPayloadGuard.ts#L39-L57)
- [AgentPermissionPolicy.ts:428-446](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L428-L446)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [SecretStorageService.ts:182-227](file://src/main/settings/SecretStorageService.ts#L182-L227)
- [secretRedaction.ts:31-102](file://src/main/runtime/secretRedaction.ts#L31-L102)
- [bridgeSecurity.ts:16-31](file://src/main/browserAppBridge/bridgeSecurity.ts#L16-L31)

## 详细组件分析

### 权限策略与审批流
- 决策强度格：allow < auto_review < ask_user < deny。策略层与模式共同决定最终动作。
- 工作区边界：只读工具允许在工作区内自动；外部路径需显式许可或审批。
- Shell 命令：**新增** 绕过检测后，cat/grep/find/sed 等命令被重定向到专用工具，防止绕过安全控制。
- 临时路径根：针对外部读写的临时授权仅在当次上下文有效。

```mermaid
flowchart TD
Start(["进入 evaluate()"]) --> Mode{"权限模式"}
Mode --> |full-access| AllowAll["允许(仍受硬约束)"]
Mode --> |auto-review| AutoReview["自动审查"]
Mode --> |custom| Custom["按配置根/命令集"]
Mode --> |default| Default["默认策略"]
Default --> ReadTools{"只读文件工具?"}
ReadTools --> |是| CheckRoots["检查工作区/知识读根"]
CheckRoots --> |在内| AllowRead["允许"]
CheckRoots --> |在外| AskOrAllow["请求审批或允许(在白名单内)"]
ReadTools --> |否| ShellCheck{"是否 shell?"}
ShellCheck --> |是| BypassCheck["检测命令绕过"]
BypassCheck --> |绕过| Redirect["重定向到专用工具"]
BypassCheck --> |正常| Risk["风险分析/危险模式检测"]
Risk --> |高| DenyHigh["拒绝或要求审批"]
Risk --> |低/中| Routine{"是否例行命令?"}
Routine --> |是| AllowRoutine["允许"]
Routine --> |否| AskCmd["请求审批"]
ShellCheck --> |否| Mutation{"是否变更/破坏性?"}
Mutation --> |是| WriteRoots["检查写根/外部路径"]
WriteRoots --> |允许| AllowWrite["允许(可能标记临时根)"]
WriteRoots --> |不允许| AskWrite["请求审批"]
Mutation --> |否| Network{"是否网络工具?"}
Network --> |是| AskNet["请求审批(非 custom)"]
Network --> |否| End(["允许"])
```

**图表来源**
- [AgentPermissionPolicy.ts:428-446](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L428-L446)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)

章节来源
- [AgentPermissionPolicy.ts:323-476](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L323-L476)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)

### Shell 命令绕过检测
**新增功能** shellFileToolBypass 模块检测常见的文件操作命令绕过尝试：

- **命令路由表**：将 cat/type/get-content/head/tail/rg/grep/find/sed/awk/set-content 等命令映射到专用工具
- **嵌套命令检测**：支持 pwsh/cmd/bash/sh/zsh 的嵌套调用，深度限制为 8 层
- **RDC 命令拦截**：阻止通过命令行直接调用 rdc CLI，强制使用冻结的安装和租约
- **引号保护**：正确识别被引用的命令名，避免误判 echo "cat file" 等场景

```mermaid
flowchart TD
Cmd["shell.command"] --> Tokenize["命令分词"]
Tokenize --> CheckDepth{"嵌套深度 > 8?"}
CheckDepth --> |是| Reject["SHELL_FILE_TOOL_BYPASS: 无法验证嵌套命令"]
CheckDepth --> |否| MatchCmd{"匹配文件命令?"}
MatchCmd --> |是| MapTool["映射到专用工具"]
MapTool --> ToolAvailable{"工具可用?"}
ToolAvailable --> |是| Redirect["SHELL_FILE_TOOL_BYPASS: 使用专用工具"]
ToolAvailable --> |否| PassThrough["继续正常处理"]
MatchCmd --> |否| CheckRDC{"是否为 RDC 命令?"}
CheckRDC --> |是| DenyRDC["RDC_TOOL_VIA_COMMAND_DENIED"]
CheckRDC --> |否| Continue["继续正常处理"]
```

**图表来源**
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)

章节来源
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- [shellFileToolBypass.test.ts:1-32](file://src/main/agent-runtime/permissions/shellFileToolBypass.test.ts#L1-L32)

### 读前写验证机制
**新增功能** 文件编辑和覆盖操作前必须成功读取同一真实路径：

- **successfulFileReads 跟踪**：在 ToolExecutionContext 中维护已成功读取的文件路径集合
- **realpath 验证**：使用 fs.realpathSync 获取真实路径，处理符号链接和平台差异
- **跨 turn 持久化**：在同一 session 内跨 turn 保持读取状态，但重启或子 session 不继承
- **新建文件豁免**：创建新文件不需要先读，但覆盖现有文件需要验证

```mermaid
sequenceDiagram
participant Caller as "调用方"
participant EditTool as "EditFileTool"
participant Access as "fileReadAccess"
participant FS as "文件系统"
Caller->>EditTool : edit_file(path, old_text, new_text)
EditTool->>Access : requireSuccessfulFileRead(realpath)
Access->>Access : 检查 successfulFileReads
alt 未找到读取记录
Access-->>EditTool : 抛出 READ_BEFORE_EDIT_REQUIRED
else 找到读取记录
Access-->>EditTool : 验证通过
EditTool->>FS : 读取文件内容
EditTool->>FS : 写入修改后的内容
EditTool-->>Caller : 编辑成功
end
```

**图表来源**
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [EditFileTool.ts:71-71](file://src/main/agent-runtime/tools/primitives/EditFileTool.ts#L71-L71)
- [WriteFileTool.ts:74-75](file://src/main/agent-runtime/tools/primitives/WriteFileTool.ts#L74-L75)

章节来源
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [EditFileTool.ts:71-71](file://src/main/agent-runtime/tools/primitives/EditFileTool.ts#L71-L71)
- [WriteFileTool.ts:74-75](file://src/main/agent-runtime/tools/primitives/WriteFileTool.ts#L74-L75)
- [fileReadAccess.test.ts:17-38](file://src/main/agent-runtime/tools/primitives/fileReadAccess.test.ts#L17-L38)

### IPC 输入验证与尺寸保护
- 所有 IPC handler 必须通过 parseIpcArgs 进行 Zod 模式校验与字节上限限制，失败关闭。
- 提供字符串、ID、数组等常用安全构造器，统一长度与格式约束。
- 支持可选尾部参数补齐，确保零参调用也能被 schema 接受。

```mermaid
flowchart TD
A["收到 IPC 参数"] --> B["padIpcArgs(补齐可选位)"]
B --> C["assertIpcPayloadBytes(字节上限)"]
C --> D["schema.safeParse(Zod)"]
D --> |成功| E["执行业务逻辑"]
D --> |失败| F["抛出 IPC_VALIDATION_ERROR"]
```

**图表来源**
- [IpcPayloadGuard.ts:26-57](file://src/main/ipc/validation/IpcPayloadGuard.ts#L26-L57)
- [IpcPayloadGuard.ts:67-87](file://src/main/ipc/validation/IpcPayloadGuard.ts#L67-L87)

章节来源
- [IpcPayloadGuard.ts:1-88](file://src/main/ipc/validation/IpcPayloadGuard.ts#L1-L88)

### 浏览器桥安全（QA/调试）
- 通道能力矩阵：read/mutation 默认允许；high-impact 需要显式环境变量；desktop-only 永远拒绝。
- 认证：支持 Bearer 头或 HttpOnly Cookie；Cookie 路径与 SameSite 严格；Origin 校验对 cookie 认证接口强制同源。
- 入口：/qa 一次性引导后签发 Cookie，重定向至干净 /app；/invoke、/events、/api/* 需同源。

```mermaid
sequenceDiagram
participant Browser as "浏览器"
participant Bridge as "BrowserAppBridge"
participant Auth as "bridgeSecurity"
participant Handler as "IPC 处理器"
Browser->>Bridge : GET /qa?qaBootstrap=...
Bridge->>Auth : 生成并设置 rdcBridgeToken(Cookie)
Auth-->>Bridge : 重定向到 /app
Browser->>Bridge : POST /invoke (携带 Cookie/Bearer)
Bridge->>Auth : isOriginAllowed + 通道能力检查
Auth-->>Bridge : 允许/拒绝
Bridge->>Handler : 转发 IPC 调用
Handler-->>Bridge : 返回结果
Bridge-->>Browser : JSON 响应
```

**图表来源**
- [bridgeSecurity.ts:16-31](file://src/main/browserAppBridge/bridgeSecurity.ts#L16-L31)
- [bridgeSecurity.ts:46-76](file://src/main/browserAppBridge/bridgeSecurity.ts#L46-L76)
- [bridgeSecurity.ts:99-119](file://src/main/browserAppBridge/bridgeSecurity.ts#L99-L119)

章节来源
- [bridgeSecurity.ts:6-15](file://src/main/browserAppBridge/bridgeSecurity.ts#L6-L15)
- [bridgeSecurity.ts:16-31](file://src/main/browserAppBridge/bridgeSecurity.ts#L16-L31)
- [bridgeSecurity.ts:46-76](file://src/main/browserAppBridge/bridgeSecurity.ts#L46-L76)
- [bridgeSecurity.ts:99-119](file://src/main/browserAppBridge/bridgeSecurity.ts#L99-L119)

### 凭据存储与脱敏
- 存储：使用 safeStorage 加密，不可用则 fail-closed；损坏文件隔离；文件权限 0600/ACL；写入原子化（临时文件+rename）。
- 访问：对外仅提供 has/masked 预览，不返回明文；支持多类密钥引用（provider/account/connection）。
- 脱敏：递归替换敏感键与文本模式（Bearer、JWT、Google Key、前缀密钥等），记录脱敏路径与哈希。

```mermaid
classDiagram
class SecretStorageService {
+getSecret(ref) string
+setSecret(ref, plaintext) void
+hasSecret(ref) boolean
+maskSecretPreview(plaintext) string?
-readSecretMap() SecretMap
-writeSecretMap(map) void
}
class SecretRecord {
+encoding : "safeStorage"
+payload : string
+updatedAt : string
}
SecretStorageService --> SecretRecord : "读写"
```

**图表来源**
- [SecretStorageService.ts:72-227](file://src/main/settings/SecretStorageService.ts#L72-L227)
- [SecretStorageService.ts:249-258](file://src/main/settings/SecretStorageService.ts#L249-L258)

章节来源
- [SecretStorageService.ts:18-42](file://src/main/settings/SecretStorageService.ts#L18-L42)
- [SecretStorageService.ts:182-227](file://src/main/settings/SecretStorageService.ts#L182-L227)
- [secretRedaction.ts:22-29](file://src/main/runtime/secretRedaction.ts#L22-L29)
- [secretRedaction.ts:31-102](file://src/main/runtime/secretRedaction.ts#L31-L102)

### 附件与工具输出安全
- 附件上传：文件名清洗，拒绝控制字符与 Windows 保留名；严格 Base64 校验，非法立即拒绝。
- 工具输出：超大输出转存为 artifact，通过 ref/hash 精确读取；无 session 时拒绝；错误输出保留关键尾部片段。

```mermaid
flowchart TD
In["接收附件/工具输出"] --> Sanitize["文件名清洗/校验"]
Sanitize --> Valid{"合法?"}
Valid --> |否| Reject["拒绝并报错"]
Valid --> |是| Size{"是否超大?"}
Size --> |是| Offload["转存为artifact<br/>生成ref/hash"]
Size --> |否| Return["直接返回内容"]
Offload --> ReturnRef["返回引用与哈希"]
```

**图表来源**
- [AttachmentStagingService.ts:43-70](file://src/main/conversation/AttachmentStagingService.ts#L43-L70)
- [ToolResultArtifactizer.test.ts:108-130](file://src/main/agent-runtime/tools/ToolResultArtifactizer.test.ts#L108-L130)

章节来源
- [AttachmentStagingService.ts:43-70](file://src/main/conversation/AttachmentStagingService.ts#L43-L70)
- [ToolResultArtifactizer.test.ts:108-130](file://src/main/agent-runtime/tools/ToolResultArtifactizer.test.ts#L108-L130)

### Shell 执行与输出裁剪
- 执行：通过 ShellInvocationService 调用本机 shell；exitCode 规范化。
- 输出：合并 stdout/stderr，超过阈值截断，标记 truncated；错误路径同样裁剪并保留诊断信息。

```mermaid
sequenceDiagram
participant Caller as "调用方"
participant Shell as "ShellTool"
participant OS as "操作系统"
Caller->>Shell : 执行命令
Shell->>OS : 启动子进程
OS-->>Shell : 流式输出(stdout/stderr)
Shell->>Shell : 合并输出/判断失败
Shell->>Shell : 截断超限输出
Shell-->>Caller : 结构化结果(含details)
```

**图表来源**
- [ShellTool.ts:245-264](file://src/main/agent-runtime/tools/primitives/ShellTool.ts#L245-L264)
- [ShellTool.ts:286-307](file://src/main/agent-runtime/tools/primitives/ShellTool.ts#L286-L307)

章节来源
- [ShellTool.ts:245-264](file://src/main/agent-runtime/tools/primitives/ShellTool.ts#L245-L264)
- [ShellTool.ts:286-307](file://src/main/agent-runtime/tools/primitives/ShellTool.ts#L286-L307)

### 网络与提供者数据净化
- 发现条目净化：对 catalog 持久化状态进行 sanitize，移除不受信任的模型条目，保证最小暴露面。

章节来源
- [EffectiveCatalogService.ts:35-66](file://src/main/settings/EffectiveCatalogService.ts#L35-L66)

## 依赖关系分析
- 松耦合与职责分离：权限策略专注风险评估，IPC 负责输入边界，桥负责通道能力与同源，凭据与脱敏独立于业务流。
- 强约束点：
  - IPC 校验：所有跨进程通信必经 Zod + 字节限制。
  - 权限策略：工具与 shell 审批的核心决策点。
  - **新增** 绕过检测：shellFileToolBypass 在权限策略层拦截命令绕过。
  - **新增** 文件访问控制：fileReadAccess 在工具执行层验证读前写。
  - 浏览器桥：通道能力矩阵与环境变量开关。
  - 凭据存储：safeStorage 与文件权限。
- 潜在循环依赖：未发现明显循环；各模块通过明确接口交互。

```mermaid
graph LR
IPG["IpcPayloadGuard"] --> POL["AgentPermissionPolicy"]
POL --> SRA["ShellCommandRiskAnalyzer"]
POL --> SFTB["shellFileToolBypass"]
SFTB --> FRA["fileReadAccess"]
FRA --> EFT["EditFileTool"]
FRA --> WFT["WriteFileTool"]
BR["bridgeSecurity"] --> IPG
SS["SecretStorageService"] --> POL
SR["secretRedaction"] --> POL
AS["AttachmentStagingService"] --> POL
EC["EffectiveCatalogService"] --> POL
```

**图表来源**
- [IpcPayloadGuard.ts:39-57](file://src/main/ipc/validation/IpcPayloadGuard.ts#L39-L57)
- [AgentPermissionPolicy.ts:428-446](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L428-L446)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [EditFileTool.ts:71-71](file://src/main/agent-runtime/tools/primitives/EditFileTool.ts#L71-L71)
- [WriteFileTool.ts:74-75](file://src/main/agent-runtime/tools/primitives/WriteFileTool.ts#L74-L75)

## 性能与安全权衡
- IPC 字节限制：防止超大 payload 导致内存压力与 DoS；建议结合业务场景调整 maxBytes。
- 工具输出裁剪：避免大输出阻塞渲染与传输；必要时使用 artifact 引用按需读取。
- 权限审批：默认更保守，减少误操作风险；可通过 policy 与 mode 精细调优。
- 凭据加密：safeStorage 增加少量开销，但显著提升安全性；损坏隔离避免级联失败。
- **新增** 绕过检测：shellFileToolBypass 增加命令解析开销，但防止严重的安全绕过。
- **新增** 读前写验证：successfulFileReads 集合占用内存，但确保文件操作的完整性。

## 故障排查指南
- IPC 校验失败
  - 现象：抛出 IPC_VALIDATION_ERROR，包含字段路径与问题摘要。
  - 排查：确认 schema 定义与实际参数一致；检查 padTo 与可选参数；核对字符串/ID/数组长度限制。
  - 参考：[IpcPayloadGuard.ts:39-57](file://src/main/ipc/validation/IpcPayloadGuard.ts#L39-L57)
- 权限拒绝或审批
  - 现象：工具或 shell 被 deny/ask_user/auto_review。
  - 排查：查看策略模式、工作区根、读写根、命令白/黑名单、policy floor；确认是否为灾难 shell 硬拒绝。
  - 参考：[AgentPermissionPolicy.ts:323-476](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L323-L476)、[ShellCommandRiskAnalyzer.ts:83-121](file://src/main/agent-runtime/permissions/ShellCommandRiskAnalyzer.ts#L83-L121)
- **新增** Shell 命令绕过检测
  - 现象：命令被重定向到专用工具，返回 SHELL_FILE_TOOL_BYPASS。
  - 排查：检查命令是否在文件命令路由表中；确认专用工具是否可用；验证嵌套命令深度。
  - 参考：[shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- **新增** 读前写验证失败
  - 现象：edit_file 或 write_file 覆盖操作返回 READ_BEFORE_EDIT_REQUIRED。
  - 排查：确认是否在同一 session 中成功读取过同一 realpath；检查符号链接处理；验证 successfulFileReads 集合。
  - 参考：[fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)、[fileReadAccess.test.ts:17-38](file://src/main/agent-runtime/tools/primitives/fileReadAccess.test.ts#L17-L38)
- 浏览器桥鉴权失败
  - 现象：401/403；/invoke、/events、/api/* 需同源。
  - 排查：确认 Cookie/Bearer 正确；Origin 与 bridge origin 一致；high-impact 通道需环境变量；未知 channel 会被拒绝。
  - 参考：[bridgeSecurity.ts:16-31](file://src/main/browserAppBridge/bridgeSecurity.ts#L16-L31)、[bridgeSecurity.ts:99-119](file://src/main/browserAppBridge/bridgeSecurity.ts#L99-L119)
- 凭据解密失败
  - 现象：无法解密或返回空值。
  - 排查：确认 safeStorage 可用；检查密钥引用是否正确；查看损坏文件隔离位置。
  - 参考：[SecretStorageService.ts:182-227](file://src/main/settings/SecretStorageService.ts#L182-L227)
- 附件上传失败
  - 现象：文件名非法或 Base64 无效。
  - 排查：检查文件名清洗规则；确认 Base64 严格校验通过。
  - 参考：[AttachmentStagingService.ts:43-70](file://src/main/conversation/AttachmentStagingService.ts#L43-L70)

章节来源
- [IpcPayloadGuard.ts:39-57](file://src/main/ipc/validation/IpcPayloadGuard.ts#L39-L57)
- [AgentPermissionPolicy.ts:323-476](file://src/main/agent-runtime/permissions/AgentPermissionPolicy.ts#L323-L476)
- [shellFileToolBypass.ts:39-80](file://src/main/agent-runtime/permissions/shellFileToolBypass.ts#L39-L80)
- [fileReadAccess.ts:9-17](file://src/main/agent-runtime/tools/primitives/fileReadAccess.ts#L9-L17)
- [fileReadAccess.test.ts:17-38](file://src/main/agent-runtime/tools/primitives/fileReadAccess.test.ts#L17-L38)
- [bridgeSecurity.ts:16-31](file://src/main/browserAppBridge/bridgeSecurity.ts#L16-L31)
- [bridgeSecurity.ts:99-119](file://src/main/browserAppBridge/bridgeSecurity.ts#L99-L119)
- [SecretStorageService.ts:182-227](file://src/main/settings/SecretStorageService.ts#L182-L227)
- [AttachmentStagingService.ts:43-70](file://src/main/conversation/AttachmentStagingService.ts#L43-L70)

## 结论
RDC-Agent 采用分层安全架构：以 IPC 校验为第一道边界，权限策略为核心决策点，**新增的 shell 绕过检测和文件访问控制** 提供额外的安全防护层，浏览器桥通道能力与环境变量作为二次控制，凭据与脱敏保障敏感数据安全，附件与工具输出通过严格清洗与隔离降低风险。配合策略编译与契约文档，形成可审计、可演进的安全体系。

**更新** 本次更新显著增强了 shell 命令安全性和文件操作完整性，通过 SHELL_FILE_TOOL_BYPASS、RDC_TOOL_VIA_COMMAND_DENIED 和 READ_BEFORE_EDIT_REQUIRED 等错误代码提供了明确的违规反馈。

## 附录：配置与审计清单
- 安全配置要点
  - 权限模式：Default/Auto-review/Full access/Custom，按项目需求选择；Custom 需明确读写根与命令集。
  - 浏览器桥：仅在 QA/调试启用；high-impact 通道需显式环境变量；/invoke、/events、/api/* 强制同源。
  - IPC：统一使用 parseIpcArgs；合理设置 maxBytes；为可选参数配置 padTo。
  - 凭据：确保 safeStorage 可用；定期轮换密钥；监控损坏隔离文件。
  - 输出：启用 artifact 转存；限制 shell 输出大小；保留关键错误尾部。
  - **新增** 文件访问：确保 successfulFileReads 集合正确初始化；监控 READ_BEFORE_EDIT_REQUIRED 错误频率。
- 漏洞扫描与审计
  - 静态扫描：关注 IPC schema 覆盖、权限策略变更、shell 硬拒绝表更新。
  - 动态测试：模拟非法 IPC、越界路径、危险 shell、桥鉴权绕过。
  - **新增** 绕过测试：验证 cat/grep/find/sed 等命令被正确重定向；测试嵌套命令深度限制。
  - **新增** 文件访问测试：验证 read-before-edit 机制；测试符号链接处理；验证跨 turn 状态持久化。
  - 审计项：通道能力矩阵、origin 白名单、cookie 属性、凭据存储权限、脱敏覆盖率。
- 常见威胁防护
  - 注入与命令执行：通过 shellHardDeny 与风险分类拦截；禁止 eval/base64|sh 等高危链。
  - **新增** 命令绕过：通过 shellFileToolBypass 检测常见文件操作命令；限制嵌套命令深度。
  - **新增** 文件篡改：通过 READ_BEFORE_EDIT_REQUIRED 确保编辑前已读取；防止未授权修改。
  - 路径穿越与越权：工作区边界与读写根校验；临时路径根绑定单次上下文。
  - 数据泄露：脱敏敏感键与令牌；artifact 精确引用；附件严格校验。
  - 网络攻击：桥同源校验；通道能力矩阵；禁用 desktop-only 通道。
- 应急响应流程
  - 事件分级：权限滥用、凭据泄露、桥劫持、恶意 shell、文件篡改。
  - **新增** 响应措施：监控 SHELL_FILE_TOOL_BYPASS 和 READ_BEFORE_EDIT_REQUIRED 错误；分析绕过尝试模式。
  - 处置步骤：立即阻断通道/权限；隔离凭据文件；回滚策略；取证与复盘。
  - 恢复措施：重置策略与凭据；修复 schema 与通道能力；补充测试用例。

章节来源
- [permissions.md:78-84](file://docs/contracts/permissions.md#L78-L84)
- [permissions.md:126-126](file://docs/contracts/permissions.md#L126-L126)