...
- 新增 SHELL_FILE_TOOL_BYPASS、RDX_VIA_COMMAND_DENIED、READ_BEFORE_EDIT_REQUIRED 等错误代码
...
Bypass-->>Policy : SHELL_FILE_TOOL_BYPASS/RDX_VIA_COMMAND_DENIED
...
- **RDX 命令拦截**：阻止通过命令行直接调用 rdx CLI，强制使用冻结的安装和租约
...
MatchCmd --> |否| CheckRDX{"是否为 RDX 命令?"}
CheckRDX --> |是| DenyRDX["RDX_VIA_COMMAND_DENIED"]
CheckRDX --> |否| Continue["继续正常处理"]
```
...
**更新** 本次更新显著增强了 shell 命令安全性和文件操作完整性，通过 SHELL_FILE_TOOL_BYPASS、RDX_VIA_COMMAND_DENIED 和 READ_BEFORE_EDIT_REQUIRED 等错误代码提供了明确的违规反馈。
