...
## 简介
RDC-Agent 是一个面向 RenderDoc .rdc 捕获文件的 Electron 桌面应用。它将通用 Agent 协作入口与可审计的图形调试流程整合在同一工作台中，并通过用户配置的外部 RDX CLI 执行本机 RenderDoc 能力。其目标是让开发者在一个统一的界面里完成 .rdc 管理、AI Agent 协作、Provider 选择与工具生态调用，同时保持严格的权限、安全与审计边界。
...
## 核心组件
- 工作台与窗口生命周期：主进程创建 BrowserWindow、安装 CSP、注册快捷键与菜单、处理单实例锁与 userData 隔离，并在启动时初始化设置、存储、IPC、RDX CLI 编目、回放设备等服务。
- 会话与对话：会话持久化、分支、上下文日志、消息合并、附件暂存、任务投影与右侧栏投影。
- Provider 系统：声明式目录、发现与覆盖合并为有效目录；模型路由、请求计划与适配器统一管线；认证、配额与能力探测。
- 工具生态：Shell 解析、RDX CLI 调用、MCP 连接、技能与钩子、权限策略与审批策略。
...
## 架构总览
RDC-Agent 的数据流遵循“渲染层 → 预加载 → 主进程 IPC → 运行时服务 → 外部能力（RDX CLI）”的路径，并以 Trace 投影回渲染层展示。
...
- 初始化设置、存储、IPC、RDX CLI 编目、回放设备等服务。
- 在退出阶段协调关闭对话、子进程、MCP、RDX 上下文与回放设备。
...
App->>Main : whenReady()
Main->>Main : 安装CSP/权限
Main->>Svc : 初始化设置/存储/IPC/RDX/回放
Main->>Win : 创建并加载渲染页面
Win-->>Main : ready-to-show
Main->>App : 显示窗口
App->>Main : before-quit
Main->>Svc : 有序关闭(对话/子进程/MCP/RDX/回放)
```
...
### 调试器工作流（RDC/RDX 垂直能力）
调试器是 RDC/RDX 导向的可执行 profile，用户打开项目与 .rdc 后选择 Debugger profile，提供目标，审查 Work Process 事件、工具调用、审批与诊断，由已批准的入口通过配置的 RDX shell action / 系统 CLI 执行，最终查看 trace、证据、制品与报告输出。
...
- RDX 调用失败：核对 Settings 中 RDX CLI 路径与 action 配置；确认会话租约与权限；查看 Trace 与诊断行。
