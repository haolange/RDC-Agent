...
- [isolatedRdcTools.ts](file://src/main/testing/isolatedRdcTools.ts)
...
- 方法与工具
  ...
  - 通过 isolatedRdcTools 将原生 CLI 工具以符号链接方式挂载到隔离目录，避免污染真实环境。
- 示例
  - 契约测试矩阵：为每个协议构建最小 RequestPlan，断言安全头、协议契约、流式解析结果。
  - 工具隔离：在临时目录中建立 tools 子树，注入 RDC_TOOL_ROOT，使测试可独立运行。
...
- [isolatedRdcTools.ts:1-15](file://src/main/testing/isolatedRdcTools.ts#L1-L15)

### 端到端（E2E）与验收测试策略
- Browser QA 表面
  - 通过一次性 /qa 引导建立同源 cookie，随后访问 /app；支持 dev 模式反向代理与 HMR。
  - 严格限制 payload 大小，除特定通道外默认上限 1 MiB。
  - 验收需显式指定 userData 或使用隔离临时目录，避免触碰真实 ~/.rdc-agent。
...
- 原生工具不可用
  - 确认 isolatedRdcTools 已正确创建符号链接并注入 RDC_TOOL_ROOT。
