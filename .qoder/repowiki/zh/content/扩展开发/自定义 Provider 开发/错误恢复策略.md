...
## 更新摘要
**所做更改**
- 扩展了错误分类体系从7个类别到9个类别，新增了'request'和'model'失败类型
- 添加了isRequestSchemaFailure()函数用于请求模式验证错误检测
- 添加了isModelUnavailableFailure()函数用于模型访问问题检测
- 更新了ConversationRoutePreflight.ts中的错误处理逻辑以支持新的错误分类
- 增强了用户友好的错误消息生成机制
...
**更新** 错误分类体系已扩展至9个类别，新增了对请求模式验证错误和模型访问问题的专门处理。
...
## 核心组件
- **增强的错误分类体系**
  - Provider 侧统一分类：将未知错误映射为标准错误信息，支持按 HTTP 状态码与消息模式分类，识别网络、超时、限流、认证、上下文溢出、流协议违规、请求模式验证错误和模型访问问题等。
...
**更新** 错误分类体系已从7个类别扩展到9个类别，新增了对请求模式验证错误和模型访问问题的专门处理。

- **Provider 错误分类器**
  - 通过 HTTP 状态码与消息模式识别限流、网络、超时、认证、上下文溢出、流协议违规、请求拒绝和模型源问题等。
  - 支持 Abort 与 Provider 未找到等特殊情况。
- **会话侧类别推断**
  - 结合状态码与消息内容推断 LLM 失败类别，用于生成用户可见的诊断信息与后续处理。
  - 新增对请求模式验证错误（如工具参数格式、schema验证失败）和模型访问问题（如模型不存在、无权限访问）的专门识别。
...
StreamCheck --> |否| RequestCheck{"是否请求模式验证错误?"}
RequestCheck --> |是| RequestErr["返回 request"]
RequestCheck --> |否| ModelCheck{"是否模型访问问题?"}
ModelCheck --> |是| ModelErr["返回 model"]
ModelCheck --> |否| Fallback["返回 unknown"]
```
...
### 错误恢复管理器与重试算法
- **错误类别与优先级**
  - 优先处理输出长度限制（先压缩再续写），其次限流、过载、上下文过长、令牌上限、认证错误、网络与服务端错误、空流、流协议违规、请求模式验证错误和模型访问问题。
- **重试与退避**
...
- **常见错误类别与处理**
  ...
  - **新增** 请求模式验证错误：检查工具参数格式和schema定义，修正请求结构。
  - **新增** 模型访问问题：确认模型可用性和访问权限，更换可用模型。
- **诊断与日志**
...
## 结论
RDC-Agent 的错误恢复策略通过统一的错误分类、可配置的恢复决策、指数退避与断路器保护，构建了健壮的错误处理机制。**更新后的系统现在能够更精确地识别和处理请求模式验证错误和模型访问问题**，结合配额管理与成本计算，系统在面临网络异常、API 限流与服务不可用时能够自适应调整，保障用户体验与资源效率。通过完善的诊断与日志记录，问题定位与优化更加高效。
...
- **关键术语**
  ...
  - **新增** 请求模式验证错误：指由于工具参数格式不正确或schema验证失败导致的请求被拒绝。
  - **新增** 模型访问问题：指由于模型不存在、无权限访问或模型下线导致的访问失败。
- **最佳实践**
  - 明确错误分类与重试策略，避免盲目重试。
  - 设置合理的超时与断路器阈值，平衡可用性与性能。
  - 记录结构化诊断信息，便于分析与监控。
  - 结合配额与成本，优化模型选择与请求规划。
  - **新增** 针对请求模式验证错误，应检查工具schema定义和参数格式的正确性。
  - **新增** 对于模型访问问题，应建立模型可用性监控和自动切换机制。