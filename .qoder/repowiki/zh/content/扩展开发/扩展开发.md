...
## 简介
本指南面向希望在 RDC-Agent 中开发自定义 Agent、Skill、Tool 与 Provider 的工程师。内容覆盖扩展点识别、开发框架、配置格式、测试方法、生命周期管理、权限控制、错误处理机制、调试技巧与性能优化建议。RDC-Agent 以 Electron 主进程为核心，通过声明式 Manifest、编译期 Catalog 与运行时解析链路，将 LLM Provider、Agent 行为、工具执行与外部能力（如 RDX CLI）统一编排，并通过会话、追踪与审计保障可观测性与安全性。
...
- 外部能力：通过 Settings 配置的 shell actions 与 RDX CLI 执行本机 RenderDoc 能力。
...
- 工具令牌：read、search、web、shell、write、edit、git、askUser、agent、handoff、task、memory、planArtifact、skills/skill、mcp、subagent、tool_search、rdxContext。
- 角色边界：Mission（debugger/analyzer/optimizer）为 plan-only；General 为执行编排者，可在策略允许时使用 shell/write/edit/rdxContext。
...
SkipTasks --> Profile
Profile --> |是| PlanOnly["限制为只读 rdx_probe/rdx_context"]
...
- 可见性规则：Mission 角色对冲突技能（如 rdx-cli-shell）不可见或受限；General 可使用通用技能。
...
### Hook 与审计
- Hook 清单与实现：resources/agent-runtime/hooks 下提供 .hook.yml 与 .mjs 配对，用于工件完整性、计划交接检查、RDX shell 审计与报告契约。
...
## 依赖关系分析
- 模块职责映射：Agent 工作区、Provider 系统、RDX shell actions、会话/捕获、报告/证据等模块通过 IPC/API 表面协作。
- 构建与验证：脚本链串联 provider-system、tool-system、work-process 等门禁，确保契约一致。

```mermaid
graph LR
A["Agent 工作区"] --> B["Provider 系统"]
A --> C["RDX Shell Actions"]
B --> D["会话/捕获"]
A --> E["报告/证据"]
F["脚本门禁"] --> B
F --> A
