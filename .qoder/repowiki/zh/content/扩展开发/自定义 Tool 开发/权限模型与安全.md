...
- 强制层：跨平台 shell 灾难模式硬拒绝、Electron Sandbox、IPC Schema 校验、Secret 隔离、RDX 上下文租约等。
...
## 结论
RDC-Agent 采用“策略编译 + 权限判定 + 风险分类 + 硬拒绝 + 路径/文件门禁”的多层防御体系，配合 Electron Sandbox、IPC Schema 校验、Secret 隔离与 RDX 上下文租约，形成从声明到执行的闭环安全边界。建议在生产环境启用严格策略与沙箱，最小化 Full access 的使用范围，并通过契约测试持续验证安全矩阵。
