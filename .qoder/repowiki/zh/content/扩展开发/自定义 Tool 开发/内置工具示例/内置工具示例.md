...
### Shell 工具（终端命令）
- 功能特性
  - 支持两种模式：直接执行 shell 命令或执行原生 RDC 操作（rd.shader/perf/event/pipeline/resource/export）。
...
- 使用场景
  ...
  - 调用 RDC 原生操作进行捕获/分析/导出。
- 配置选项
  - command：要执行的命令字符串。
  - rdc：原生操作对象（operation、args、experimentId）。
...
Agent->>Tool : execute(command/rdc-tool, timeout)
Tool->>ST : 读取/写入会话 cwd
Tool->>SR : 构建非交互参数
