...
- [src/main/tools/RdcProbeLifecycle.ts](file://src/main/tools/RdcProbeLifecycle.ts)
- [src/main/tools/RdcTurnBindings.ts](file://src/main/tools/RdcTurnBindings.ts)
...
## 更新摘要
**变更内容**
- 移除了对已删除的 RdcNativeLifecycle.test.ts 文件的引用
...
- 工具与执行：Shell 工具、rdc-tool CLI 调用、会话上下文与租约校验
...
Main --> Tools["工具与执行<br/>ShellTool / executeRdcShell"]
Tools --> Child
Main --> Probe["探针生命周期<br/>RdcProbeLifecycle"]
Probe --> Sessions["会话服务<br/>RdcSessionService"]
```
...
- [src/main/tools/RdcProbeLifecycle.ts:7-29](file://src/main/tools/RdcProbeLifecycle.ts#L7-L29)
...
- 工具与执行：Shell 工具封装命令执行与 rdc 操作，基于会话锁与租约校验；rdc-tool CLI 调用前进行参数白名单与身份一致性校验。
...
- [src/main/tools/executeRdcShell.ts:32-54](file://src/main/tools/executeRdcShell.ts#L32-L54)
...
App->>Exec : 解析执行绑定与能力
Exec-->>App : 能力决议与约束
App->>Tools : 执行工具/Shell/rdc-tool CLI
...
- **租约管理**：使用 `RdcLeaseIdentity` 管理执行上下文的生命周期，确保会话隔离
- **能力决议**：在执行前验证能力的可用性和授权状态，防止未授权操作
- **探针生命周期**：通过 `RdcProbeLifecycle` 管理捕获的生命周期，确保安全的上下文访问
...
**图表来源**
- [src/main/tools/RdcTurnBindings.ts:5-16](file://src/main/tools/RdcTurnBindings.ts#L5-L16)
- [src/main/tools/RdcProbeLifecycle.ts:7-29](file://src/main/tools/RdcProbeLifecycle.ts#L7-L29)
...
### 运行与执行
- 工具执行：ShellTool 封装命令执行与 rdc 操作，基于会话锁确保并发安全；executeRdcShell 对参数进行白名单校验与身份一致性检查，防止越权。
...
- 会话与租约：executeRdcShell 校验 replay 身份与上下文租约，防止跨会话越权。
- 工具白名单：ShellTool 与 executeRdcShell 对参数进行白名单过滤，禁止敏感字段注入。
