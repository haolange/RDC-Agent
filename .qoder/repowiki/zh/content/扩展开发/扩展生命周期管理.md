...
- [src/main/tools/RdxNativeLifecycle.test.ts](file://src/main/tools/RdxNativeLifecycle.test.ts)
...
- Hook 与 Session 生命周期：在创建/删除会话前后派发运行时钩子，用于审计、完整性检查等。
...
## 架构总览
下图展示扩展从加载到运行的关键路径：清单与服务发现 → 目录解析与缓存 → 工具与进程执行 → 会话与钩子 → 统一关闭。
...
- 原生捕获生命周期：RdxNativeLifecycle.test 展示了通过 actions 打开/关闭捕获的完整流程，便于复现问题。
...
- [src/main/hooks/sessionLifecycle.test.ts:1-39](file://src/main/hooks/sessionLifecycle.test.ts#L1-L39)
- [src/main/tools/RdxNativeLifecycle.test.ts:25-42](file://src/main/tools/RdxNativeLifecycle.test.ts#L25-L42)

## 结论
RDC-Agent 通过清单驱动的服务发现、严格的版本校验、统一的进程管理与分阶段关闭协调，构建了稳健的扩展生命周期体系。依赖解析与缓存机制保障了性能与一致性，安全策略与权限验证确保了扩展执行的受控性。结合前端性能探针与结构化日志，提供了完善的监控与调试能力。运维层面，启动脚本与原子写入、回滚机制支撑了可靠的打包发布与更新恢复。
