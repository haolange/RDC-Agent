...
- handoffs 允许 Agent 将对话转交给其他 Agent，并附带标签、目标 Agent、提示词、是否发送、是否显示继续按钮以及可选的模型覆盖。
- 内置 Agent 之间形成协作网络：General 可向 Debugger/Analyzer/Optimizer 交接；Mission 类 Agent 通常向 General 交接执行。
...
## 结论
Agent 清单是 RDC-Agent 的行为契约入口，通过严格的解析与校验、清晰的能力展开、完善的模型路由与交接机制，确保不同 Agent 类型在安全、可控的前提下协同工作。遵循本文的字段规范、验证规则与最佳实践，可显著提升清单的可维护性与运行稳定性。
