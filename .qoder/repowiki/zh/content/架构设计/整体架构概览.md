...
## 简介
RDC-Agent 是基于 Electron 的桌面工作台，面向 RenderDoc .rdc 捕获文件的分析与调试。它将通用 Agent 协作入口与可审计的图形调试流程整合在同一界面中，并通过用户配置的外部 rdc-tool CLI 执行本机能力。应用采用四层代码结构：主进程、预加载脚本、渲染器、共享层，通过 IPC 进行受控通信，并以会话管理、设置管理、Agent 编排为核心服务，驱动对话、追踪与外部工具调用。
...
- 会话管理：RdcSessionService 管理 RDC 运行时上下文、捕获打开/预览、设备连接与生命周期。
...
- [src/renderer/main.tsx:1-16](file://src/renderer/main.tsx#L1-L16)
- [src/shared/index.ts:1-5](file://src/shared/index.ts#L1-L5)
- [src/main/sessions/RdcSessionService.ts:53-200](file://src/main/sessions/RdcSessionService.ts#L53-L200)
...
participant Act as "RDC动作"
participant CLI as "系统rdc-tool CLI"
...
- 初始化服务（设置、rdc-tool CLI 目录、IPC、回放设备等）。
...
### 会话管理（RdcSessionService）
会话服务负责：
- 打开本地或远程捕获，维护捕获描述符与状态。
- 通过 rdcShellActionService 调用配置的 shell actions（openCapture/openRemoteCapture/connectRemote/openPreview/closeRuntime）。
- 解析返回数据，构造稳定的 RdcRuntimeContext，更新 opened capture 与预览。
...
图表来源
- [docs/architecture/data-flow.md:33-63](file://docs/architecture/data-flow.md#L33-L63)
- [src/main/sessions/RdcSessionService.ts:69-200](file://src/main/sessions/RdcSessionService.ts#L69-L200)
...
- 规范化 Provider、Agent、UI、窗口布局、RDC 动作等。
...
- 失败收敛：RDC 动作失败返回结构化诊断，避免仅透传 stderr。
...
- [src/main/index.ts:180-235](file://src/main/index.ts#L180-L235)
- [src/main/index.ts:641-667](file://src/main/index.ts#L641-L667)
- [src/main/sessions/RdcSessionService.ts:159-186](file://src/main/sessions/RdcSessionService.ts#L159-L186)
...
- RDC 动作失败：查看结构化诊断 message/classification/fix_hint，确认 shell action 配置与系统 CLI 可用。
