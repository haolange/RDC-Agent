...
- [withRdxHostRuntimeEnv.ts](file://src/main/tools/withRdxHostRuntimeEnv.ts)
...
## 更新摘要
**所做更改**
- 新增桌面运行时环境注入功能章节，详细说明 RDX_INTERMEDIATE_ROOT 环境变量注入机制
...
## 简介
本文件面向需要理解并正确使用 RDX CLI 调用器的开发者与运维人员，系统性解析 RdxCliInvokerService 的核心能力：工具目录加载、命令参数构建、执行流程管理、可用性检查、运行时元数据生成、错误处理策略、配置与环境变量处理、工作目录管理等。**最新更新**：集成了新的桌面运行时环境注入功能，确保CLI调用尊重隔离的运行时路径，同时保持与现有配置的兼容性。文档同时提供调用示例与最佳实践，帮助在 Agent 或上层工具链中安全、稳定地调用本地 RDX CLI。
...
- withRdxHostRuntimeEnv：**新增** 桌面运行时环境注入器，自动设置 RDX_INTERMEDIATE_ROOT 环境变量以实现运行时隔离。
- AppPathService：提供用户级 RDX 路径解析，包括 rdxIntermediateRoot 等隔离路径。
...
B --> C["withRdxHostRuntimeEnv<br/>运行时环境注入"]
C --> D["AppPathService<br/>用户路径解析"]
B --> E["ShellInvocationService"]
B --> F["resolveRdxBatchInvocation"]
B --> G["RdxNativeProtocol"]
B --> H["SettingsService<br/>tooling.rdxCli"]
E --> I["操作系统进程<br/>RDX CLI<br/>带隔离运行时环境"]
```

**图表来源**
- [RdxCliInvokerService.ts:19-164](file://src/main/tools/RdxCliInvokerService.ts#L19-L164)
- [withRdxHostRuntimeEnv.ts:10-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L10-L16)
...
## 核心组件
- RdxCliInvokerService：对外暴露 loadCatalog、getRuntimeSummary、call、executeCLI、onInvocationTrace、abortRun、terminateAll 等方法；内部实现工具目录发现、参数构建、执行与结果标准化，**新增** 集成桌面运行时环境注入。
- ShellInvocationService：统一进程调度、超时控制、环境注入、孤儿进程检测与终止。
- withRdxHostRuntimeEnv：**新增** 桌面运行时环境注入器，自动设置 RDX_INTERMEDIATE_ROOT 环境变量到用户级隔离路径。
- AppPathService：提供用户级 RDX 路径解析，包括 rdxIntermediateRoot 等隔离路径。
...
**章节来源**
- [RdxCliInvokerService.ts:21-334](file://src/main/tools/RdxCliInvokerService.ts#L21-L334)
- [withRdxHostRuntimeEnv.ts:10-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L10-L16)
- [AppPathService.ts:108-123](file://src/main/runtime/AppPathService.ts#L108-L123)

## 架构总览
RdxCliInvokerService 作为"适配器+编排器"，屏蔽了底层进程调用的复杂性，向上提供统一的工具调用接口。**更新**：现在集成了桌面运行时环境注入功能，确保每次 CLI 调用都使用隔离的用户级运行时路径。其关键流程如下：
- 可用性检查：基于 settings.tooling.rdxCli.enabled 与 command 是否存在且可访问，快速失败。
- **运行时环境注入**：**新增** 通过 withRdxHostRuntimeEnv 自动设置 RDX_INTERMEDIATE_ROOT 环境变量到用户级隔离路径。
...
**图表来源**
- [RdxCliInvokerService.ts:161-190](file://src/main/tools/RdxCliInvokerService.ts#L161-L190)
- [withRdxHostRuntimeEnv.ts:10-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L10-L16)
- [ShellInvocationService.ts:32-105](file://src/main/tools/ShellInvocationService.ts#L32-L105)

## 详细组件分析

### 桌面运行时环境注入：withRdxHostRuntimeEnv()
**新增功能** - 桌面运行时环境注入器
- 行为
  - 检查当前设置的 env 中是否已存在 RDX_INTERMEDIATE_ROOT 环境变量。
  - 如果不存在或为空，则从 AppPathService 获取用户级隔离路径并注入。
  - 返回新的 settings 对象，包含注入的环境变量。
- 隔离路径
  - 默认路径：~/.rdx/rdx-intermediate（可通过 RDC_AGENT_HOME 环境变量覆盖）
  - 每个用户拥有独立的隔离运行时空间，避免不同用户间的冲突。
- 设计要点
  - 支持显式覆盖：如果用户在 Settings 中明确设置了 RDX_INTERMEDIATE_ROOT，则优先使用用户配置。
...
- 行为
  - 调用 withRdxHostRuntimeEnv 获取注入后的 settings。
  - 提取 RDX_INTERMEDIATE_ROOT 环境变量值作为 intermediateRoot。
...
**章节来源**
- [RdxCliInvokerService.ts:33-49](file://src/main/tools/RdxCliInvokerService.ts#L33-L49)
...
**章节来源**
- [RdxCliInvokerService.ts:74-102](file://src/main/tools/RdxCliInvokerService.ts#L74-L102)
...
**章节来源**
- [RdxCliInvokerService.ts:121-144](file://src/main/tools/RdxCliInvokerService.ts#L121-L144)

### 执行流程：executeCLI()
**更新** - 集成运行时环境注入
- 行为
  - **新增** 调用 withRdxHostRuntimeEnv 注入 RDX_INTERMEDIATE_ROOT 环境变量。
...
**章节来源**
- [RdxCliInvokerService.ts:146-191](file://src/main/tools/RdxCliInvokerService.ts#L146-L191)
- [withRdxHostRuntimeEnv.ts:10-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L10-L16)
...
**章节来源**
- [RdxCliInvokerService.ts:218-325](file://src/main/tools/RdxCliInvokerService.ts#L218-L325)
...
**章节来源**
- [RdxCliInvokerService.ts:33-49](file://src/main/tools/RdxCliInvokerService.ts#L33-L49)
- [RdxCliInvokerService.ts:104-119](file://src/main/tools/RdxCliInvokerService.ts#L104-L119)

### 工作目录与环境变量
- 工作目录优先级：options.cwd > settings.workingDirectory > 默认（undefined 表示继承父进程）。
- 环境变量合并：**更新** 首先通过 withRdxHostRuntimeEnv 注入 RDX_INTERMEDIATE_ROOT，然后叠加 settings.env 与 options.env；ShellInvocationService 还会注入 PYTHONIOENCODING=utf-8。

**章节来源**
- [RdxCliInvokerService.ts:161-184](file://src/main/tools/RdxCliInvokerService.ts#L161-L184)
- [withRdxHostRuntimeEnv.ts:10-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L10-L16)
...
- [RdxCliInvokerService.ts:290-325](file://src/main/tools/RdxCliInvokerService.ts#L290-L325)
...
- **新增依赖**：withRdxHostRuntimeEnv 提供桌面运行时环境注入，依赖 AppPathService 解析用户级路径。
...
class withRdxHostRuntimeEnv {
+withRdxHostRuntimeEnv(settings) RdxCliInvokerSettings
+hostRdxIntermediateRoot() string
}
class AppPathService {
+getUserRdxPaths() UserRdxPaths
+getRdxIntermediateRoot() string
}
...
RdxCliInvokerService --> withRdxHostRuntimeEnv : "注入运行时环境"
RdxCliInvokerService --> AppPathService : "解析用户路径"
...
- **新增**：运行时路径问题 - 检查 RDX_INTERMEDIATE_ROOT 环境变量是否正确设置到用户级隔离路径；确认 ~./rdx/rdx-intermediate 目录存在且有适当权限。

**章节来源**
- [RdxCliInvokerService.ts:51-67](file://src/main/tools/RdxCliInvokerService.ts#L51-L67)
- [ShellInvocationService.ts:67-97](file://src/main/tools/ShellInvocationService.ts#L67-L97)
- [RdxNativeProtocol.ts:12-34](file://src/main/tools/RdxNativeProtocol.ts#L12-L34)
- [withRdxHostRuntimeEnv.ts:10-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L10-L16)

## 结论
RdxCliInvokerService 提供了稳定、可观测、可配置的 RDX CLI 调用能力。**最新更新**：通过集成桌面运行时环境注入功能，确保了每次 CLI 调用都使用隔离的用户级运行时路径，增强了多用户环境下的安全性和稳定性。通过清晰的参数构建、严格的协议校验、完善的错误处理与追踪机制，上层工具可以专注于业务逻辑，而无需关心进程管理与平台差异。建议在生产环境中：
...
- **新增**：信任默认的 RDX_INTERMEDIATE_ROOT 注入机制，或在特殊需求时显式覆盖该环境变量。
...
- env：环境变量键值对，会与进程环境与 options.env 合并。**新增**：可显式设置 RDX_INTERMEDIATE_ROOT 覆盖默认的用户级隔离路径。
- timeoutMs：默认超时毫秒数。
...
- **新增**：运行时环境自动注入 - 每次调用时自动设置 RDX_INTERMEDIATE_ROOT 到用户级隔离路径。
...
**章节来源**
- [RdxCliInvokerService.ts:218-325](file://src/main/tools/RdxCliInvokerService.ts#L218-L325)
- [RdxCliInvokerService.test.ts:122-127](file://src/main/tools/RdxCliInvokerService.test.ts#L122-L127)
...
- **新增**：信任默认的运行时环境注入机制，仅在特殊需求时显式覆盖 RDX_INTERMEDIATE_ROOT。
- **新增**：在多用户环境中，确保每个用户都有独立的 ~./rdx/rdx-intermediate 目录权限。