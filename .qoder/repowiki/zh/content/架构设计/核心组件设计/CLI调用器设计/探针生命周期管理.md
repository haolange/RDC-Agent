...
## 简介
本文件围绕 RDC 探针的生命周期管理，系统性说明探针从启动、运行监控到优雅关闭的完整流程。重点覆盖以下方面：
- 进程状态管理与租约（Lease）机制
- 健康检查、自动重启策略与异常恢复
- 资源泄漏检测与隔离
- 状态同步、事件通知与诊断信息收集
...
- 工具层：RdcProbeTool 负责编译参数、校验所有权、执行 CLI 并封装结果
- 会话服务：RdcSessionService 负责打开/关闭捕获、设备准备、预览与健康检查
- 运行时上下文：RdcRuntimeContextRegistry 维护 per-session 的 RDC 上下文租约与委派
...
```mermaid
graph TB
ProbeTool["RdcProbeTool<br/>参数编译/所有权校验/CLI执行"] --> LeaseMgr["RdcRuntimeContextRegistry<br/>租约/委派/隔离"]
ProbeTool --> SessionSvc["RdcSessionService<br/>打开/关闭捕获/预览/健康检查"]
...
**图示来源**
- [RdcProbeTool.ts:24-127](file://src/main/workflow/debugger/RdcProbeTool.ts#L24-L127)
- [RdcRuntimeContextRegistry.ts:61-154](file://src/main/sessions/RdcRuntimeContextRegistry.ts#L61-L154)
- [RdcSessionService.ts:69-200](file://src/main/sessions/RdcSessionService.ts#L69-L200)
...
## 核心组件
- RdcProbeLifecycle：封装探针租约的打开与关闭，复用产品捕获生命周期，禁止为任意 contextId 创建租约
- RdcProbeTool：将探针动作编译为 CLI 调用，进行所有权校验、上下文绑定、结果解析与截断输出
- RdcRuntimeContextRegistry：维护 per-session 的 RDC 上下文租约、委派、隔离与版本控制
- RdcSessionService：打开/关闭捕获、远程/本地回放设备准备、预览与健康检查、错误诊断格式化
...
**章节来源**
- [RdcProbeLifecycle.ts:1-45](file://src/main/tools/RdcProbeLifecycle.ts#L1-L45)
- [RdcProbeTool.ts:24-127](file://src/main/workflow/debugger/RdcProbeTool.ts#L24-L127)
- [RdcRuntimeContextRegistry.ts:61-268](file://src/main/sessions/RdcRuntimeContextRegistry.ts#L61-L268)
- [RdcSessionService.ts:69-200](file://src/main/sessions/RdcSessionService.ts#L69-L200)
- [ProcessSupervisor.ts:140-425](file://src/main/runtime/ProcessSupervisor.ts#L140-L425)
- [ResourceExecutionLifetime.ts:1-21](file://src/main/runtime/ResourceExecutionLifetime.ts#L1-L21)
- [ShutdownCoordinator.ts:35-113](file://src/main/lifecycle/ShutdownCoordinator.ts#L35-L113)
...
participant Tool as "RdcProbeTool"
...
### 探针租约与上下文管理（RdcProbeLifecycle + RdcRuntimeContextRegistry）
- 打开租约（openProbeLease）
  - 校验项目与捕获输入是否已注册且匹配当前会话
  - 禁止为任意 contextId 创建租约，必须复用产品捕获生命周期
  - 通过 ReplayDeviceService 获取本地设备，再调用 RdcSessionService.openProjectInput 完成上下文绑定
- 关闭租约（closeProbeLease）
  - 防止在委派执行未结束或父捕获被误关时关闭
  - 调用 RdcSessionService.clearOpenedCaptureForSession 清理会话级捕获状态
- 租约模型（RdcContextLease）
  ...
  - 提供 assertRdcContextLeaseOwnership 进行严格的所有权验证
...
**图示来源**
- [RdcProbeLifecycle.ts:7-32](file://src/main/tools/RdcProbeLifecycle.ts#L7-L32)
- [RdcRuntimeContextRegistry.ts:61-154](file://src/main/sessions/RdcRuntimeContextRegistry.ts#L61-L154)

**章节来源**
- [RdcProbeLifecycle.ts:7-44](file://src/main/tools/RdcProbeLifecycle.ts#L7-L44)
- [RdcRuntimeContextRegistry.ts:61-268](file://src/main/sessions/RdcRuntimeContextRegistry.ts#L61-L268)

### 探针工具执行与所有权校验（RdcProbeTool）
- 参数编译与动作枚举：将高层 action 编译为底层 CLI 命令与参数
- 所有权与上下文校验：
  - 校验租约归属、contextId 一致性、委派状态与隔离原因
  - 若检测到 RDC 运行时身份变化，返回 reprepareRequired 提示上层重新准备 turn
- 执行与结果：
...
```mermaid
sequenceDiagram
participant T as "RdcProbeTool"
participant L as "RdcRuntimeContextRegistry"
participant S as "RdcSessionService"
participant P as "ProcessSupervisor"
T->>L : getRdcContextLease(sessionId)
alt 需要打开租约
...
T->>T : parseRdcNativeResult + truncateOutput
T-->>T : 构造details(worldStateStamp/reprepareRequired)
```

**图示来源**
- [RdcProbeTool.ts:24-127](file://src/main/workflow/debugger/RdcProbeTool.ts#L24-L127)
- [RdcRuntimeContextRegistry.ts:103-154](file://src/main/sessions/RdcRuntimeContextRegistry.ts#L103-L154)
- [RdcSessionService.ts:69-200](file://src/main/sessions/RdcSessionService.ts#L69-L200)
- [ProcessSupervisor.ts:167-394](file://src/main/runtime/ProcessSupervisor.ts#L167-L394)
...
### 会话服务与设备管理（RdcSessionService + ReplayDeviceService）
- 打开捕获（openProjectInput）
  - 检查是否有未确认的子进程退出，避免恢复阻塞
  - 区分本地与远程回放：远程需 ensureReplayDeviceReady 与 consumePreparedRemote
  - 调用 rdcShellActionService.runAction 执行 openCapture/openRemoteCapture
...
## 依赖关系分析
- RdcProbeTool 依赖 RdcRuntimeContextRegistry 进行所有权校验，依赖 RdcSessionService 进行捕获打开/关闭，依赖 ProcessSupervisor 执行 CLI
- RdcSessionService 依赖 ReplayDeviceService 进行设备准备与观察租约管理
...
```mermaid
graph LR
Tool["RdcProbeTool"] --> Reg["RdcRuntimeContextRegistry"]
Tool --> Svc["RdcSessionService"]
...
- [RdcProbeTool.ts:106-123](file://src/main/workflow/debugger/RdcProbeTool.ts#L106-L123)
- [ReplayDeviceService.ts:187-199](file://src/main/captures/ReplayDeviceService.ts#L187-L199)

## 故障排查指南
- 常见错误码与含义
  - RDC_TOOL_PROBE_CAPTURE_REQUIRED：未先打开注册的项目捕获
  - RDC_TOOL_PROBE_OWNER：会话/项目不匹配、上下文不合法、委派冲突或租约变更
  - RDC_TOOL_PROBE_DEVICE_REQUIRED：未选择回放设备
  - RDC_TOOL_LEASE_DUAL_OWNER：委派执行未结束或双重拥有冲突
  - LOCAL_REPLAY_UNSUPPORTED：本地回放不支持或失败
- 排查步骤
  - 检查租约所有权与上下文一致性（assertRdcContextLeaseOwnership）
...
**章节来源**
- [RdcProbeLifecycle.ts:10-44](file://src/main/tools/RdcProbeLifecycle.ts#L10-L44)
- [RdcRuntimeContextRegistry.ts:134-154](file://src/main/sessions/RdcRuntimeContextRegistry.ts#L134-L154)
- [RdcSessionService.ts:142-186](file://src/main/sessions/RdcSessionService.ts#L142-L186)
- [ProcessSupervisor.ts:18-72](file://src/main/runtime/ProcessSupervisor.ts#L18-L72)
- [RuntimeLogService.ts:94-156](file://src/main/runtime/RuntimeLogService.ts#L94-L156)

## 结论
RDC 探针生命周期以“租约为中心、会话服务为枢纽、进程监管为保障、关机协调为兜底”，形成安全、可观测、可恢复的执行闭环。通过严格的租约所有权校验、设备准备与观察租约管理、进程超时与孤儿检测、以及结构化日志与诊断输出，系统能够在复杂环境下保持稳健运行并快速定位问题。
