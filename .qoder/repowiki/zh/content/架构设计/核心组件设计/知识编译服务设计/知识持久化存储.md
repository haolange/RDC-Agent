...
## 更新摘要
**所做更改**
- 更新了会话草稿管理机制，新增 `leftoverDrafts` 字段分离
- 添加了 `listLeftoverDrafts` 和 `replaceLeftoverDrafts` 方法
- 增强了 `parseKnowledgeStatePayload` 和 `serializeKnowledgeStateDocument` 函数以支持向后兼容
- 更新了数据模型和序列化逻辑以支持新的草稿管理策略
...
**更新** 会话草稿管理已显著改进，现在将"遗留草稿"（leftover drafts）与主文档结构分离，提供更好的数据完整性和迁移支持。
...
## 核心组件
- KnowledgeDurableStore：按会话维度持久化草稿、候选、评审三类记录；提供内容哈希、版本冲突检测、原子落盘与跨进程锁保护。**更新** 现在支持独立的遗留草稿管理。
...
- Schema 层：Zod 校验与迁移定义，确保存储格式演进兼容性与运行时强类型约束。**更新** 支持遗留草稿的分离存储。
...
- 存储目标：每个会话一个 knowledge-state.json，包含 candidates、reviews 数组，以及可选的 leftoverDrafts 数组。
...
**更新** 新增了遗留草稿管理功能：
- `listLeftoverDrafts(sessionId)`：列出会话中的遗留草稿
- `replaceLeftoverDrafts(sessionId, leftoverDrafts)`：批量替换遗留草稿
- 遗留草稿与主文档结构分离，提供更好的数据完整性保障
...
### 数据模型与序列化
- 状态文档：
  - schemaVersion、candidates、reviews 三个必需数组，以及可选的 leftoverDrafts 数组。
  - 每项包含 revision、contentHash 与业务对象。
- 卡片记录：
...
**更新** 数据模型已增强以支持遗留草稿分离：
- `parseKnowledgeStatePayload` 函数现在返回 `{document, leftoverDrafts}` 结构
- `serializeKnowledgeStateDocument` 函数接受 document 和 leftoverDrafts 参数
- 遗留草稿仅在存在时才序列化到文件中，减少不必要的开销
...
- 容量规划建议：
  - 估算每会话状态文件大小（候选/评审数量 × 平均记录大小 + 遗留草稿数量 × 平均草稿大小），结合会话数评估磁盘占用。
  - 索引快照大小与卡片总数线性增长，定期清理过期快照或归档历史。

**更新** 遗留草稿分离带来的性能影响：
- 遗留草稿独立存储减少了主文档的大小
- 批量操作 `replaceLeftoverDrafts` 提高了草稿管理的效率
- 条件序列化（仅在存在遗留草稿时）减少了不必要的 I/O 操作
...
- 遗留草稿问题：
  - 现象：遗留草稿无法正确加载或保存。
  - 原因：数据结构不兼容或迁移失败。
  - 处理：检查 schemaVersion 兼容性，使用 `parseKnowledgeStatePayload` 进行解析。
...
## 结论
KnowledgeDurableStore 提供了会话级、强一致、跨进程安全的知识持久化能力，配合 StorageIo 的原子写与损坏恢复、directoryFileLock 的并发控制、KnowledgeWriteService 的人类确认与路径安全、KnowledgeCandidateService 的冷数据处理与草稿/候选管理，以及 KnowledgeIndexService 的索引构建与失效检测，形成了一套完整、健壮的知识存储体系。**更新** 新的遗留草稿分离机制进一步提升了数据完整性和迁移支持能力。在生产环境中，应关注锁超时、索引重建成本与容量规划，并结合业务需求引入缓存与批处理优化。

## 附录：迁移与备份恢复
- 数据迁移：
  - 状态文档与索引快照均通过 Migration 列表声明 schemaVersion 与校验器，升级时需向后兼容；未知高版本会快速失败，避免误用。
  - **更新** 新的 `parseKnowledgeStatePayload` 和 `serializeKnowledgeStateDocument` 函数确保了遗留草稿的正确迁移和处理。
