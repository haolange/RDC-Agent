...
## 更新摘要
**变更内容**
- 新增 workflowStore 组件分析，包含深度相等检查优化
- 更新性能考虑章节，增加工作流存储的变更检测机制说明
- 扩展 Store 依赖关系图，包含 workflowStore 的集成
- 增强故障排查指南，涵盖工作流状态更新的调试方法
...
## 简介
本文件聚焦于渲染进程中的 Zustand 状态管理模式，系统梳理各 Store 的职责边界、状态结构设计、更新模式与选择器最佳实践。重点覆盖 conversationStore、sessionStore、appSettingsStore、layoutStore、projectStore 以及新增的 workflowStore 等核心模块，并补充 captureStore、evidenceStore、terminalStore 的协同方式。文档还包含状态持久化机制、跨组件共享方案、调试与性能优化建议，以及状态流转图与 Store 依赖关系图，帮助读者快速理解并高效使用这套状态体系。
...
- [workflowStore.ts:1-44](file://src/renderer/stores/workflowStore.ts#L1-L44)
...
- workflowStore：管理工作流状态和追踪呈现数据，通过深度相等检查优化性能，防止不必要的重渲染。
...
## 架构总览
Zustand 在各 Store 中以 create 创建独立状态切片，通过选择器订阅最小数据片段，减少重渲染范围。跨 Store 协作遵循"单一职责 + 显式依赖"原则：sessionStore 作为编排中心，组合 conversationStore、captureStore、evidenceStore、workflowStore 与 projectStore 的能力；appSettingsStore 负责与主进程设置 API 交互；layoutStore 将 UI 布局同步至设置；terminalStore 通过 Electron API 拉取日志；workflowStore 提供工作流状态管理和追踪呈现数据的深度相等检查优化。
...
LS->>LS : persistLayout()
LS-->>UI : 布局已更新并持久化
UI->>WS : setTracePresentation()
WS->>WS : sameTracePresentation() 深度检查
WS-->>UI : 跳过重复更新或应用新状态
```
...
- [workflowStore.ts:31-43](file://src/renderer/stores/workflowStore.ts#L31-L43)
...
### workflowStore：工作流状态与追踪呈现
- 状态结构
  - workflowState：工作流状态对象
  - tracePresentation：Agent 运行呈现数据
- 更新模式
  - setWorkflowState：直接设置工作流状态
  - setTracePresentation：智能更新，通过 sameTracePresentation 进行深度相等检查
  - reset：重置所有状态
- 深度相等检查优化
  - sameRightPanel：检查右侧面板视图模型的四个关键字段（progress、artifacts、outputs、context）
  - sameTracePresentation：检查 sessionId、updatedAt 和时间戳相同的 rightPanel
  - 性能收益：避免相同状态的重复更新，减少 React 重渲染周期

```mermaid
flowchart TD
Input["setTracePresentation(presentation)"] --> Check["sameTracePresentation(current, next)"]
Check --> Equal{"状态相同?"}
Equal --> |是| Skip["跳过更新，返回原状态"]
Equal --> |否| Update["应用新状态"]
Skip --> Output["无重渲染"]
Update --> Output
```

**图表来源**
- [workflowStore.ts:15-29](file://src/renderer/stores/workflowStore.ts#L15-L29)
- [workflowStore.ts:31-43](file://src/renderer/stores/workflowStore.ts#L31-L43)
...
- workflowStore 提供工作流状态管理，被 sessionStore 在重置时调用。
...
- **工作流状态深度相等检查**：workflowStore 通过 sameRightPanel 和 sameTracePresentation 函数实现复杂对象的深度相等检查，避免相同状态的重复更新，显著减少 React 重渲染周期。该机制特别适用于高频更新的追踪呈现数据，通过比较 sessionId、updatedAt 和 rightPanel 的关键字段来优化性能。
...
- **工作流状态更新问题**
  - 现象：追踪呈现数据频繁触发重渲染或状态更新不生效。
  - 排查：检查 sameTracePresentation 函数的比较逻辑，确认 sessionId、updatedAt 和 rightPanel 字段的比较是否正确；验证 deep equality 检查是否正常工作。
  - 参考路径：[workflowStore.ts:15-29](file://src/renderer/stores/workflowStore.ts#L15-L29)、[workflowStore.ts:31-43](file://src/renderer/stores/workflowStore.ts#L31-L43)
...
## 结论
本项目采用 Zustand 构建清晰、可维护的前端状态体系。各 Store 职责明确、更新模式规范，结合选择器实现细粒度订阅与高性能渲染。通过乐观更新与回滚策略保障设置类操作的可靠性；通过单调停止与消息合并保障对话更新的正确性；通过布局持久化提升用户体验；通过工作流状态的深度相等检查优化性能。建议在新增功能时遵循现有模式：拆分领域 Store、使用选择器订阅最小切片、在写入边界进行排序与投影、对异步操作进行竞态与回滚处理、对复杂状态对象实施深度相等检查以减少不必要的重渲染。
