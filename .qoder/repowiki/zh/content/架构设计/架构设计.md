...
## 结论
RDC-Agent 通过主进程服务化、渲染器投影化与事件驱动通信，实现了清晰的职责边界与高可观测性。AgentOrchestrator、SettingsService、RdxSessionService、RdxRuntimeService 分别承担 Agent 编排、设置与提供者管理、RDX 运行时上下文与捕获生命周期、作用域资源与信任管理的核心职责。IPC 层作为稳定契约面，屏蔽了主进程复杂逻辑，使渲染器专注于 UI 与交互。数据流与事件流贯穿整个系统，确保状态一致性与可追溯性。未来扩展可通过新增 Provider、Shell Action、IPC 通道与资源类型实现，同时保持向后兼容与安全边界。
