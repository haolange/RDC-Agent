...
- [src/main/runtime/RdxRuntimeService.ts](file://src/main/runtime/RdxRuntimeService.ts)
...
- [docs/workflows/first-use-and-release-readiness.md](file://docs/workflows/first-use-and-release-readiness.md)
...
## 更新摘要
**变更内容**   
- 更新版本信息至 0.6.0-rc.8 发布候选版本
- 增强提供者系统架构描述，包含完整的 manifest 编译器和运行时解析机制
- 添加发布工作流章节，说明 rc.1-rc.8 的发布流程和验证门禁
- 更新提供者适配器列表，包含新增的 Bedrock Converse Stream 和 Azure OpenAI Responses
- 强化提供者配置管理和凭证处理机制
...
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
12. [附录：关键数据流与事件契约](#附录：关键数据流与事件契约)

## 简介
RDC-Agent 是一个基于 Electron 的桌面工作台，面向 RenderDoc `.rdc` capture 的调试与分析。整体采用主进程/渲染器分离、服务导向架构与事件驱动通信的设计原则：
- 主进程负责应用生命周期、IPC 路由、会话与运行时上下文管理、设置持久化、外部工具链（RDX CLI）调用编排、工作流协调与可观测性。
...
当前版本 **0.6.0-rc.8** 是发布候选版本，已完成 Windows x64 未签名预发布，包含成熟的提供者系统架构和完善的发布工作流。
...
Prov["Provider System<br/>Manifest Compiler / Registry"]
Ext["External Tools<br/>RDX CLI / Shell"]
...
- RdxSessionService：RDX 运行时上下文与捕获生命周期管理；封装本地/远程回放设备接入、预览窗口、运行时租约与清理。
- RdxRuntimeService：用户/项目作用域资源清单、验证、导入与信任管理（Agent/Skill/MCP/Hook/Policy/Knowledge/Memory）。
...
- [src/main/runtime/RdxRuntimeService.ts:24-90](file://src/main/runtime/RdxRuntimeService.ts#L24-L90)
...
RT->>PROV : compile manifest & resolve adapter
...
### RdxSessionService：RDX 运行时上下文与捕获生命周期
- 职责：打开/关闭捕获、切换活动捕获、打开/关闭人类预览窗口、管理本地/远程回放设备与会话租约。
- 关键流程：
  - openProjectInput：根据设备类型选择本地或远程动作，调用 RDX shell action，提取运行时上下文并建立租约。
...
**章节来源**
- [src/main/sessions/RdxSessionService.ts:1-781](file://src/main/sessions/RdxSessionService.ts#L1-L781)

### RdxRuntimeService：作用域资源与信任管理
...
**图表来源**
- [src/main/runtime/RdxRuntimeService.ts:177-208](file://src/main/runtime/RdxRuntimeService.ts#L177-L208)
- [src/main/runtime/RdxRuntimeService.ts:283-293](file://src/main/runtime/RdxRuntimeService.ts#L283-L293)

**章节来源**
- [src/main/runtime/RdxRuntimeService.ts:1-302](file://src/main/runtime/RdxRuntimeService.ts#L1-L302)
...
RDC-Agent 的提供者系统采用声明式架构，通过 JSON manifest 定义 Provider、Model、Route 和能力，经过编译期验证后在运行时解析。

### Manifest 分层架构
- **identities/*.json**：固定 models.dev identity、上游 revision/hash、原始 provenance
- **profiles/*.json**：可复用的 protocol/auth/discovery/wire mechanics 引用
- **surfaces/*.json**：RDC 展示 surface、分类、operator、endpoint、route、models、controls、bindings

### 编译与加载流程
```mermaid
flowchart TD
A["Manifest JSON"] --> B["Schema Validation"]
B --> C["Identity Coverage Check"]
C --> D["Route/Adapter Coverage"]
D --> E["Surface Closure Check"]
E --> F["Model Matrix Validation"]
F --> G["Control Resolution"]
G --> H["Catalog Revision Hash"]
H --> I["Compiled Index"]
I --> J["Runtime Registry"]
```

**图表来源**
- [docs/architecture/provider-architecture.md:36-57](file://docs/architecture/provider-architecture.md#L36-L57)

### 支持的协议与适配器
当前实现包含 **16 个适配器**，支持多种协议：
...
### 凭证管理与安全
- 凭证通过 `ProviderAuth` 桥接层统一管理，支持环境变量和注入解析器
- Secret 材料保存在安全存储中，不在 IPC 或 UI 中明文传输
- 每次发送前冻结 Provider 配置和短期凭证，避免并发修改

**章节来源**
- [docs/architecture/provider-architecture.md:1-263](file://docs/architecture/provider-architecture.md#L1-L263)
- [src/main/settings/SettingsProviderOps.ts:1-547](file://src/main/settings/SettingsProviderOps.ts#L1-L547)
- [src/main/agent-runtime/providers/ProviderAuth.ts:1-32](file://src/main/agent-runtime/providers/ProviderAuth.ts#L1-L32)

## 发布工作流

RDC-Agent 采用严格的发布工作流，支持多轮发布候选版本和完整的验证门禁。

### 版本历史
当前版本 **0.6.0-rc.8** 是最新的发布候选版本，已完成 Windows x64 未签名预发布。
...
**图表来源**
- [docs/workflows/first-use-and-release-readiness.md:1-174](file://docs/workflows/first-use-and-release-readiness.md#L1-L174)

### 验证门禁
- **check:provider-system**：编译后的语义对象、Resolver、Planner、Discovery 与最终 wire 契约
- **check:agent-runtime**：冻结 RequestPlan/credential lease、PromptPlan 与 adapter 边界
- **check:gates**：完整的质量门禁集合
- **verify-package.mjs**：验证包完整性、资源数量和哈希值

### 发布资产
每个发布候选版本包含以下资产：
- NSIS 安装包（Windows x64）
- ZIP 压缩包
- Blockmap 文件
...
Main --> Sessions["RdxSessionService"]
...
- 可扩展点：
  ...
  - 新增 Shell Action 与 RDX CLI 命令，通过 Settings 配置，无需硬编码。
...
- RDX 会话：
  - 打开捕获失败时，关注结构化诊断（message/classification/fix_hint），优先检查 RDX CLI 配置与设备连接。
...
- [src/main/sessions/RdxSessionService.ts:106-173](file://src/main/sessions/RdxSessionService.ts#L106-L173)
- [src/main/settings/SettingsService.ts:141-206](file://src/main/settings/SettingsService.ts#L141-L206)

## 结论
RDC-Agent 通过主进程服务化、渲染器投影化与事件驱动通信，实现了清晰的职责边界与高可观测性。AgentOrchestrator、SettingsService、RdxSessionService、RdxRuntimeService 分别承担 Agent 编排、设置与提供者管理、RDX 运行时上下文与捕获生命周期、作用域资源与信任管理的核心职责。IPC 层作为稳定契约面，屏蔽了主进程复杂逻辑，使渲染器专注于 UI 与交互。

当前版本 0.6.0-rc.8 代表了系统的成熟度，具备完整的提供者系统架构、严格的发布工作流和全面的验证门禁。未来扩展可通过新增 Provider、Shell Action、IPC 通道与资源类型实现，同时保持向后兼容与安全边界。

## 附录：关键数据流与事件契约
- Agent Turn 数据流：渲染器 -> Preload -> IPC -> ConversationService -> AgentOrchestrator -> Agent Runtime -> Provider/Tools -> Trace/Logs -> IPC -> 渲染器。
- RDX Shell Actions 数据流：渲染器 -> IPC -> RdxSessionService -> RdxShellActionService -> ShellInvocationService -> RDX CLI -> 解析结果 -> IPC -> 渲染器。
