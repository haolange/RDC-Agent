# 主进程架构

<cite>
**本文引用的文件**
- [src/main/index.ts](file://src/main/index.ts)
- [src/main/workflow/debugger/AgentOrchestrator.ts](file://src/main/workflow/debugger/AgentOrchestrator.ts)
- [src/main/settings/SettingsService.ts](file://src/main/settings/SettingsService.ts)
- [src/main/sessions/RdcSessionService.ts](file://src/main/sessions/RdcSessionService.ts)
- [src/main/ipc/handlers.ts](file://src/main/ipc/handlers.ts)
- [src/main/ipc/workbenchHandlers.ts](file://src/main/ipc/workbenchHandlers.ts)
- [src/main/ipc/invokeRegistry.ts](file://src/main/ipc/invokeRegistry.ts)
- [src/main/browserAppBridge/rendererEventHub.ts](file://src/main/browserAppBridge/rendererEventHub.ts)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts](file://src/main/sessions/OwnedRdcDaemonRegistry.ts)
</cite>

## 更新摘要
**变更内容**
- 新增守护进程收割功能：在主进程初始化期间集成遗留守护进程检查与清理机制
- 更新服务初始化流程：在 rdc-tool CLI 调用器初始化后执行守护进程收割
- 增强错误处理：对守护进程收割失败进行详细日志记录
- 完善资源管理：确保上次运行遗留的守护进程得到正确清理

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向 Electron 主进程，系统性解析 RDC-Agent 的主进程架构与关键业务逻辑。重点覆盖：
- AgentOrchestrator 任务编排引擎：消息路由、提示词计划、工具装配与执行、子代理与后台任务、状态与事件发布。
- SettingsService 配置管理服务：设置持久化、提供者凭据管理、LLM 配置注入、窗口布局与工具链配置。
- RdcSessionService 会话生命周期管理：捕获打开/关闭、本地/远端回放设备、运行时上下文与预览窗口、错误恢复与清理。
- **守护进程收割机制**：主进程启动时自动检测并清理上次运行遗留的 RDC 守护进程，防止资源泄漏和端口冲突。
- IPC 处理器架构与消息路由：统一注册、渲染器通道对等性校验、SSE 事件总线、跨窗口广播。
- 工作流编排、工具执行、外部命令调用：rdc-tool CLI 调用、Shell Action 编排、工具追踪与审计。
- 服务注册发现、依赖注入、事件总线：集中式初始化、模块按需加载、事件驱动解耦。

## 项目结构
主进程入口负责应用生命周期、安全策略、单实例锁、用户数据目录、窗口创建、IPC 注册与服务初始化；各子系统通过模块化方式组织，按职责划分到 workflow、settings、sessions、ipc、tools 等目录。**新增的守护进程收割功能**在 initializeServices 函数中集成，确保应用启动时的资源清理。

```mermaid
graph TB
A["主进程入口<br/>src/main/index.ts"] --> B["IPC 注册中心<br/>src/main/ipc/workbenchHandlers.ts"]
A --> C["设置服务<br/>src/main/settings/SettingsService.ts"]
A --> D["会话服务<br/>src/main/sessions/RdcSessionService.ts"]
A --> E["守护进程收割器<br/>src/main/sessions/OwnedRdcDaemonRegistry.ts"]
B --> F["Agent 编排器<br/>src/main/workflow/debugger/AgentOrchestrator.ts"]
B --> G["RDC 运行时处理器<br/>src/main/ipc/rdcRuntimeHandlers.ts"]
B --> H["其他领域处理器<br/>conversation/workflow/memory/knowledge 等"]
B --> I["IPC 注册表<br/>src/main/ipc/invokeRegistry.ts"]
B --> J["渲染器事件总线<br/>src/main/browserAppBridge/rendererEventHub.ts"]
E --> K["rdc-tool CLI 调用器<br/>src/main/tools/RdcCliInvokerService.ts"]
E --> L["运行时日志服务<br/>src/main/runtime/RuntimeLogService.ts"]
```

**图表来源**
- [src/main/index.ts:560-628](file://src/main/index.ts#L560-L628)
- [src/main/index.ts:683-741](file://src/main/index.ts#L683-L741)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)

章节来源
- [src/main/index.ts:1-747](file://src/main/index.ts#L1-L747)

## 核心组件
- AgentOrchestrator：对外暴露 sendMessage/sendProfileMessage/prepareTurnContext 等能力，内部组合 TurnPreparationService、AgentTurnRunner、SubagentRunner、ToolExecutorFactory、PromptPlanForTurn、McpConnectionCoordinator 等，完成从消息到 LLM 调用的完整编排。
- SettingsService：提供 getAll/setAll/getLlmConfig 等接口，负责设置读取/写入、提供者凭据存储、窗口布局保存、LLM 配置编译与注入。
- RdcSessionService：管理捕获的打开/关闭、本地/远端回放设备连接、运行时上下文（contextId、ownerLeaseId）、人类预览窗口生命周期，以及与 rdcShellActionService 和 rdcCliInvokerService 的交互。
- **OwnedRdcDaemonRegistry**：**新增的守护进程收割组件**，负责检测并清理上次运行遗留的 RDC 守护进程，包括进程终止、注册表清理和资源释放。
- IPC 层：workbenchHandlers 作为组合根，注册各域处理器；invokeRegistry 提供统一的 ipcMain.handle 包装与通道对等性校验；rendererEventHub 提供 SSE 事件总线用于跨窗口广播。

章节来源
- [src/main/workflow/debugger/AgentOrchestrator.ts:75-145](file://src/main/workflow/debugger/AgentOrchestrator.ts#L75-L145)
- [src/main/settings/SettingsService.ts:94-139](file://src/main/settings/SettingsService.ts#L94-L139)
- [src/main/sessions/RdcSessionService.ts:53-78](file://src/main/sessions/RdcSessionService.ts#L53-L78)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)
- [src/main/ipc/workbenchHandlers.ts:43-101](file://src/main/ipc/workbenchHandlers.ts#L43-L101)
- [src/main/ipc/invokeRegistry.ts:1-51](file://src/main/ipc/invokeRegistry.ts#L1-L51)
- [src/main/browserAppBridge/rendererEventHub.ts:1-53](file://src/main/browserAppBridge/rendererEventHub.ts#L1-L53)

## 架构总览
主进程启动流程：
- 安装渲染器安全策略（CSP、权限拒绝）。
- 计算并锁定 userData 目录，确保单实例运行。
- 初始化设置服务、存储适配器、rdc-tool CLI 编目、ReplayDeviceService。
- **执行守护进程收割**：检查上次运行遗留的 RDC 守护进程并进行清理。
- 注册 IPC 处理器，恢复中断运行，预加载 LLM 配置。
- 可选启动 BrowserAppBridge（Headless/QA），或创建主窗口。
- 监听窗口事件、菜单快捷键、导航白名单。

```mermaid
sequenceDiagram
participant App as "Electron 应用"
participant Main as "主进程入口"
participant Settings as "设置服务"
participant Harvest as "守护进程收割器"
participant IPC as "IPC 注册中心"
participant Bridge as "浏览器桥接"
participant Win as "主窗口"
App->>Main : whenReady()
Main->>Main : 安装渲染器安全策略
Main->>Settings : initialize()
Main->>Main : 初始化存储/CLI/回放设备
Main->>Harvest : harvestOwnedRdcDaemons()
Harvest->>Harvest : 扫描遗留守护进程
Harvest->>Harvest : 终止进程/清理注册表
Harvest-->>Main : 返回清理结果
Main->>IPC : registerIPCHandlers()
IPC->>IPC : installIpcInvokeRegistry()
IPC->>IPC : register*Handlers()
IPC->>IPC : assertRendererIpcParity()
alt Headless/QA
Main->>Bridge : startBrowserAppBridge()
else 桌面模式
Main->>Win : createMainWindow()
end
```

**图表来源**
- [src/main/index.ts:124-161](file://src/main/index.ts#L124-L161)
- [src/main/index.ts:560-628](file://src/main/index.ts#L560-L628)
- [src/main/index.ts:683-741](file://src/main/index.ts#L683-L741)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)

## 详细组件分析

### AgentOrchestrator 任务编排引擎
- 角色与职责：聚合 AgentSlotRegistry、MCP 连接协调、工具装配与执行、提示词计划、轮次准备与执行、子代理与后台任务、内存 UI、状态更新与事件发布。
- 关键流程：
  - sendMessage：选择有效模型与提供者、刷新凭据、构建提示词计划、准备上下文、执行轮次、记录助手消息、更新状态并发布。
  - sendProfileMessage：支持传入已准备的轮次、冻结请求计划、额外提示段、终端上下文回调等高级场景。
  - 工具与 MCP：通过 RuntimeToolAssembly 与 ToolExecutorFactory 动态装配工具，支持延迟激活与允许列表控制；MCP 服务器状态查询与断开。
  - 状态与事件：updateAgentStatus 将状态变更写入 slots，并通过 runtimeLogService 与 workflowProjectionPublisher 发布。

```mermaid
classDiagram
class AgentOrchestrator {
+sendMessage(agentId, content, context, options) Promise~string~
+sendProfileMessage(agentId, content, options) Promise~string~
+prepareTurnContext(input) Promise~PreparedAgentTurnContext~
+configureAgent(agentId, config) void
+getAgentState(sessionOrScopeId, agentId) AgentState
+applyLlmConfig(config) void
+disconnectAllMcpServers() Promise~void~
}
class AgentSlotRegistry
class McpConnectionCoordinator
class RuntimeToolAssembly
class ToolExecutorFactory
class TurnPreparationService
class ProfileTurnPreparation
class AgentTurnRunner
class SubagentRunner
class PromptPlanForTurn
class TokenizerService
class OrchestratorMemoryUi
AgentOrchestrator --> AgentSlotRegistry : "管理槽位/状态"
AgentOrchestrator --> McpConnectionCoordinator : "MCP 连接"
AgentOrchestrator --> RuntimeToolAssembly : "工具装配"
AgentOrchestrator --> ToolExecutorFactory : "工具执行"
AgentOrchestrator --> TurnPreparationService : "轮次准备"
AgentOrchestrator --> ProfileTurnPreparation : "配置轮次准备"
AgentOrchestrator --> AgentTurnRunner : "执行 LLM 轮次"
AgentOrchestrator --> SubagentRunner : "子代理"
AgentOrchestrator --> PromptPlanForTurn : "提示词计划"
AgentOrchestrator --> TokenizerService : "分词统计"
AgentOrchestrator --> OrchestratorMemoryUi : "内存UI"
```

**图表来源**
- [src/main/workflow/debugger/AgentOrchestrator.ts:75-145](file://src/main/workflow/debugger/AgentOrchestrator.ts#L75-L145)
- [src/main/workflow/debugger/AgentOrchestrator.ts:195-366](file://src/main/workflow/debugger/AgentOrchestrator.ts#L195-L366)
- [src/main/workflow/debugger/AgentOrchestrator.ts:384-634](file://src/main/workflow/debugger/AgentOrchestrator.ts#L384-L634)

```mermaid
sequenceDiagram
participant Client as "调用方"
participant Orchestrator as "AgentOrchestrator"
participant Prep as "TurnPreparationService"
participant Runner as "AgentTurnRunner"
participant Tools as "RuntimeToolAssembly"
participant MCP as "McpConnectionCoordinator"
Client->>Orchestrator : sendMessage(agentId, content, context, options)
Orchestrator->>Orchestrator : 刷新凭据/选择模型/构建提示词计划
Orchestrator->>Prep : prepare(...)
Prep-->>Orchestrator : preparedBundle
Orchestrator->>Runner : runAgentTurn(preparedBundle)
Runner->>Tools : resolve/create tool executors
Tools->>MCP : 必要时建立/复用连接
Runner-->>Orchestrator : responseText
Orchestrator->>Orchestrator : 记录助手消息/更新状态/发布事件
Orchestrator-->>Client : 最终内容
```

**图表来源**
- [src/main/workflow/debugger/AgentOrchestrator.ts:195-366](file://src/main/workflow/debugger/AgentOrchestrator.ts#L195-L366)
- [src/main/workflow/debugger/AgentOrchestrator.ts:384-634](file://src/main/workflow/debugger/AgentOrchestrator.ts#L384-L634)

章节来源
- [src/main/workflow/debugger/AgentOrchestrator.ts:75-145](file://src/main/workflow/debugger/AgentOrchestrator.ts#L75-L145)
- [src/main/workflow/debugger/AgentOrchestrator.ts:195-366](file://src/main/workflow/debugger/AgentOrchestrator.ts#L195-L366)
- [src/main/workflow/debugger/AgentOrchestrator.ts:384-634](file://src/main/workflow/debugger/AgentOrchestrator.ts#L384-L634)
- [src/main/workflow/debugger/AgentOrchestrator.ts:682-688](file://src/main/workflow/debugger/AgentOrchestrator.ts#L682-L688)

### SettingsService 配置管理服务
- 职责：读取/合并/持久化设置；提供者凭据存储与解密；窗口布局增量保存；LLM 配置编译与注入；Agent 定义与提供者定义的提交与版本控制。
- 关键点：
  - initialize 时重建并迁移设置，清理过期密钥引用。
  - setAll 中处理 API Key 与 OAuth 账户凭据，生成/删除 secretRef，并返回运行时设置。
  - getLlmConfig 过滤已启用且验证通过的提供者，组装 provider 列表与 agentRoutes。
  - persistWindowLayout/persistWindowLayoutAsync 针对高频窗口事件做窄写优化。

```mermaid
flowchart TD
Start(["调用 setAll(patch)"]) --> Read["读取当前持久化设置"]
Read --> Normalize["归一化提供者/凭据视图"]
Normalize --> PatchProviders{"是否包含提供者补丁?"}
PatchProviders -- 是 --> HandleSecrets["处理 API Key/OAuth 凭据<br/>生成/删除 secretRef"]
PatchProviders -- 否 --> MergeUI["合并 UI 偏好/布局/工具链"]
HandleSecrets --> MergeUI
MergeUI --> Write["写入持久化设置"]
Write --> Cleanup["清理过期密钥引用"]
Cleanup --> Return["返回运行时设置(getAll)"]
```

**图表来源**
- [src/main/settings/SettingsService.ts:249-383](file://src/main/settings/SettingsService.ts#L249-L383)
- [src/main/settings/SettingsService.ts:458-487](file://src/main/settings/SettingsService.ts#L458-L487)

章节来源
- [src/main/settings/SettingsService.ts:94-139](file://src/main/settings/SettingsService.ts#L94-L139)
- [src/main/settings/SettingsService.ts:249-383](file://src/main/settings/SettingsService.ts#L249-L383)
- [src/main/settings/SettingsService.ts:458-487](file://src/main/settings/SettingsService.ts#L458-L487)

### RdcSessionService 会话生命周期管理
- 职责：打开/关闭捕获、本地/远端回放设备准备、运行时上下文（contextId、ownerLeaseId）管理、人类预览窗口生命周期、错误诊断与恢复。
- 关键点：
  - openProjectInput：根据设备类型走本地或远端路径，调用 rdcShellActionService.runAction('openCapture'/'openRemoteCapture')，提取运行时上下文并应用。
  - teardownRuntime：优先通过 closeRuntime action 优雅关闭，失败则抛出错误以保留所有权；resetRuntimeState 清理所有状态。
  - humanPreview：open/close 通过 rdcShellActionService 或 rdcCliInvokerService 控制预览开关，维护 HumanPreviewSnapshot。
  - 远程设备：ensureReplayDeviceReady 保证设备处于可用状态，必要时激活设备。

```mermaid
sequenceDiagram
participant Caller as "调用方"
participant Svc as "RdcSessionService"
participant Shell as "rdcShellActionService"
participant Cli as "rdcCliInvokerService"
participant Replay as "ReplayDeviceService"
Caller->>Svc : openProjectInput(request, options)
alt 远端回放
Svc->>Replay : ensureReplayDeviceReady(device)
Svc->>Shell : runAction("openRemoteCapture", env)
else 本地回放
Svc->>Shell : runAction("openCapture", env)
end
Shell-->>Svc : result(data)
Svc->>Svc : extractRuntimeContext / applyRuntimeContext
Svc-->>Caller : OpenedCaptureState
Caller->>Svc : closeOrReplaceOpenedCapture(options)
Svc->>Shell : runAction("closeRuntime", env)
Svc->>Svc : resetRuntimeState()
```

**图表来源**
- [src/main/sessions/RdcSessionService.ts:69-212](file://src/main/sessions/RdcSessionService.ts#L69-L212)
- [src/main/sessions/RdcSessionService.ts:710-753](file://src/main/sessions/RdcSessionService.ts#L710-L753)
- [src/main/sessions/RdcSessionService.ts:231-300](file://src/main/sessions/RdcSessionService.ts#L231-L300)

章节来源
- [src/main/sessions/RdcSessionService.ts:53-78](file://src/main/sessions/RdcSessionService.ts#L53-L78)
- [src/main/sessions/RdcSessionService.ts:69-212](file://src/main/sessions/RdcSessionService.ts#L69-L212)
- [src/main/sessions/RdcSessionService.ts:710-753](file://src/main/sessions/RdcSessionService.ts#L710-L753)

### 守护进程收割机制
**新增功能**：主进程初始化期间自动检测和清理上次运行遗留的 RDC 守护进程，防止资源泄漏和端口冲突。

- **核心职责**：
  - 扫描中间目录中的遗留守护进程状态文件
  - 验证进程所有权和有效性
  - 优雅停止守护进程或强制终止残留进程
  - 清理守护进程注册表记录
  - 详细的错误处理和日志记录

- **工作流程**：
  - 扫描 `intermediateRoot/runtime/rdc_cli` 目录查找守护进程状态文件
  - 合并现有注册表记录与发现的守护进程
  - 对每个遗留守护进程尝试优雅停止（clear + stop）
  - 如果优雅停止失败，验证并强制终止相关进程
  - 清理成功或失败的记录，返回清理结果

```mermaid
flowchart TD
Start(["守护进程收割开始"]) --> Scan["扫描遗留守护进程状态文件"]
Scan --> Merge["合并注册表记录"]
Merge --> ProcessLoop{"处理每个守护进程"}
ProcessLoop -- 是 --> TryStop["尝试优雅停止"]
TryStop --> StopSuccess{"停止成功?"}
StopSuccess -- 是 --> CleanRecord["清理注册表记录"]
CleanRecord --> Next["处理下一个"]
StopSuccess -- 否 --> ForceKill["强制终止进程"]
ForceKill --> KillSuccess{"终止成功?"}
KillSuccess -- 是 --> CleanRecord
KillSuccess -- 否 --> LogError["记录错误日志"]
LogError --> Next
Next --> ProcessLoop
ProcessLoop -- 否 --> Return["返回清理结果"]
Return --> End(["收割完成"])
```

**图表来源**
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:138-181](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L138-L181)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:183-203](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L183-L203)

章节来源
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:138-181](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L138-L181)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:183-203](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L183-L203)

### IPC 处理器架构与消息路由
- 组合根：workbenchHandlers 集中注册各域处理器（conversation、workflow、memory、knowledge、investigation、command、shell、web、toolEvidence、trace、rdcRuntime 等），并安装 invokeRegistry。
- 通道注册：invokeRegistry 包装 ipcMain.handle，收集已注册通道，支持 assertRendererIpcParity 校验渲染器侧声明的通道是否全部在主进程实现。
- 事件总线：rendererEventHub 基于 SSE 向连接的客户端推送事件，workbenchHandlers 通过 broadcastToRenderer 将运行状态、主题变化、工具执行结果等广播至所有窗口。

```mermaid
sequenceDiagram
participant Renderer as "渲染器"
participant IPC as "invokeRegistry"
participant WB as "workbenchHandlers"
participant Hub as "rendererEventHub"
Renderer->>IPC : ipc.invoke(channel, args)
IPC->>WB : 分发到对应处理器
WB->>WB : 执行业务逻辑(如会话/工作流/设置)
WB->>Hub : emit(channel, payload)
Hub-->>Renderer : SSE 推送事件
```

**图表来源**
- [src/main/ipc/invokeRegistry.ts:9-21](file://src/main/ipc/invokeRegistry.ts#L9-L21)
- [src/main/ipc/invokeRegistry.ts:32-49](file://src/main/ipc/invokeRegistry.ts#L32-L49)
- [src/main/ipc/workbenchHandlers.ts:79-101](file://src/main/ipc/workbenchHandlers.ts#L79-L101)
- [src/main/browserAppBridge/rendererEventHub.ts:15-51](file://src/main/browserAppBridge/rendererEventHub.ts#L15-L51)

章节来源
- [src/main/ipc/handlers.ts:1-7](file://src/main/ipc/handlers.ts#L1-L7)
- [src/main/ipc/workbenchHandlers.ts:43-101](file://src/main/ipc/workbenchHandlers.ts#L43-L101)
- [src/main/ipc/workbenchHandlers.ts:259-281](file://src/main/ipc/workbenchHandlers.ts#L259-L281)
- [src/main/ipc/invokeRegistry.ts:1-51](file://src/main/ipc/invokeRegistry.ts#L1-L51)
- [src/main/browserAppBridge/rendererEventHub.ts:1-53](file://src/main/browserAppBridge/rendererEventHub.ts#L1-L53)

### 工作流编排、工具执行、外部命令调用
- 工作流：workbenchHandlers 中的 selectCurrentProject、setRunLifecycleState、appendActionEvent 等函数驱动运行生命周期，并与 storageAdapter 同步状态，同时广播给渲染器。
- 工具执行：通过 rdcCliInvokerService 的 onInvocationTrace 订阅工具执行结果，转换为 ActionEvent 持久化并广播；AgentOrchestrator 通过 RuntimeToolAssembly/ToolExecutorFactory 动态装配与执行工具。
- 外部命令：RdcSessionService 使用 rdcShellActionService.runAction 与 rdcCliInvokerService.executeCLI 调用 RDC 原生能力（打开捕获、预览开关、关闭运行时等），并处理诊断信息与错误恢复。

```mermaid
flowchart TD
A["工作流触发"] --> B["selectCurrentProject / setRunLifecycleState"]
B --> C["storageAdapter 更新运行状态"]
C --> D["broadcastToRenderer 通知渲染器"]
A --> E["工具执行 trace"]
E --> F["appendActionEvent 持久化证据"]
E --> G["runtimeLogService 记录日志"]
A --> H["RDC 外部命令"]
H --> I["rdcShellActionService.runAction"]
H --> J["rdcCliInvokerService.executeCLI"]
I --> K["解析结果/错误/诊断"]
J --> K
```

**图表来源**
- [src/main/ipc/workbenchHandlers.ts:136-178](file://src/main/ipc/workbenchHandlers.ts#L136-L178)
- [src/main/ipc/workbenchHandlers.ts:190-237](file://src/main/ipc/workbenchHandlers.ts#L190-L237)
- [src/main/sessions/RdcSessionService.ts:231-300](file://src/main/sessions/RdcSessionService.ts#L231-L300)
- [src/main/sessions/RdcSessionService.ts:710-753](file://src/main/sessions/RdcSessionService.ts#L710-L753)

章节来源
- [src/main/ipc/workbenchHandlers.ts:136-178](file://src/main/ipc/workbenchHandlers.ts#L136-L178)
- [src/main/ipc/workbenchHandlers.ts:190-237](file://src/main/ipc/workbenchHandlers.ts#L190-L237)
- [src/main/sessions/RdcSessionService.ts:231-300](file://src/main/sessions/RdcSessionService.ts#L231-L300)
- [src/main/sessions/RdcSessionService.ts:710-753](file://src/main/sessions/RdcSessionService.ts#L710-L753)

## 依赖关系分析
- 主进程入口依赖：settingsService、rdcCliInvokerService、replayDeviceService、runtimeLogService、browserAppBridge、shutdownCoordinator、agentOrchestrator、turnCoordinator、processSupervisor、**harvestOwnedRdcDaemons**。
- IPC 层依赖：workbenchHandlers 组合多个领域处理器，invokeRegistry 提供通道注册与对等性校验，rendererEventHub 提供事件广播。
- 服务间耦合：AgentOrchestrator 依赖 settingsService、storageAdapter、providerRuntimeCredentialService、workflowProjectionPublisher；RdcSessionService 依赖 rdcShellActionService、rdcCliInvokerService、replayDeviceService、settingsService；**守护进程收割器依赖 rdcCliInvokerService、settingsService、runtimeLogService**。

```mermaid
graph LR
Main["主进程入口"] --> Settings["设置服务"]
Main --> IPC["IPC 注册中心"]
Main --> Harvest["守护进程收割器"]
IPC --> Agents["Agent 编排器"]
IPC --> Sessions["会话服务"]
IPC --> Tools["工具/CLI"]
IPC --> Events["事件总线"]
Agents --> Settings
Agents --> Storage["存储适配器"]
Sessions --> Tools
Sessions --> Replay["回放设备服务"]
Harvest --> Tools
Harvest --> Settings
Harvest --> Logs["运行时日志服务"]
```

**图表来源**
- [src/main/index.ts:560-628](file://src/main/index.ts#L560-L628)
- [src/main/index.ts:683-741](file://src/main/index.ts#L683-L741)
- [src/main/ipc/workbenchHandlers.ts:259-281](file://src/main/ipc/workbenchHandlers.ts#L259-L281)
- [src/main/workflow/debugger/AgentOrchestrator.ts:75-145](file://src/main/workflow/debugger/AgentOrchestrator.ts#L75-L145)
- [src/main/sessions/RdcSessionService.ts:53-78](file://src/main/sessions/RdcSessionService.ts#L53-L78)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)

章节来源
- [src/main/index.ts:560-628](file://src/main/index.ts#L560-L628)
- [src/main/index.ts:683-741](file://src/main/index.ts#L683-L741)
- [src/main/ipc/workbenchHandlers.ts:259-281](file://src/main/ipc/workbenchHandlers.ts#L259-L281)
- [src/main/workflow/debugger/AgentOrchestrator.ts:75-145](file://src/main/workflow/debugger/AgentOrchestrator.ts#L75-L145)
- [src/main/sessions/RdcSessionService.ts:53-78](file://src/main/sessions/RdcSessionService.ts#L53-L78)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L93-L136)

## 性能考量
- 设置写入优化：persistWindowLayout/persistWindowLayoutAsync 仅写入 layout.window，避免全量归一化与凭据水合，降低高频窗口事件开销。
- 工具执行追踪：onInvocationTrace 批量追加证据并广播，减少重复 IO；日志记录附带 sessionId/projectId/runId 便于定位。
- 会话清理：teardownRuntime 优先优雅关闭，失败快速回滚并抛错，避免资源泄漏；resetRuntimeState 集中清理状态。
- **守护进程收割优化**：采用异步并行处理多个遗留守护进程，支持 AbortSignal 中断，避免阻塞主进程启动。
- 事件广播：rendererEventHub 使用 SSE 心跳保持连接，避免频繁轮询；多窗口广播通过遍历窗口 webContents.send。

[本节为通用指导，不直接分析具体文件]

## 故障排查指南
- 设置初始化失败：检查 readJsonFile/readJsonFileAsync 与 writeSettings/writeSettingsAsync 的异常路径；确认 schemaVersion 与迁移逻辑。
- 提供者凭据问题：hasProviderSecret/getProviderOAuthSecret 返回空或掩码预览为空，需检查 secretStorageService 与 secretRef 映射。
- **守护进程收割失败**：检查 harvestOwnedRdcDaemons 返回的 failed 数组，查看具体的 contextId 和错误信息；确认 rdc-tool CLI 配置和进程权限。
- 会话打开失败：openProjectInput 抛出 LOCAL_REPLAY_UNSUPPORTED 或 RDC action failed，查看 diagnostic 与 error 字段；确认 replayDevice 状态与 activateDevice 结果。
- 运行时关闭失败：teardownRuntime 抛出 RDC_TOOL_CLOSE_FAILED，表示 closeRuntime 未确认；检查 shellInvocationService.hasUnconfirmedProcesses 与上下文租约。
- IPC 通道缺失：assertRendererIpcParity 抛出缺少处理器通道错误，需在 workbenchHandlers 中补充对应 register*Handlers。

章节来源
- [src/main/settings/SettingsService.ts:103-139](file://src/main/settings/SettingsService.ts#L103-L139)
- [src/main/settings/SettingsService.ts:141-206](file://src/main/settings/SettingsService.ts#L141-L206)
- [src/main/sessions/OwnedRdcDaemonRegistry.ts:122-133](file://src/main/sessions/OwnedRdcDaemonRegistry.ts#L122-L133)
- [src/main/sessions/RdcSessionService.ts:69-212](file://src/main/sessions/RdcSessionService.ts#L69-L212)
- [src/main/sessions/RdcSessionService.ts:710-753](file://src/main/sessions/RdcSessionService.ts#L710-L753)
- [src/main/ipc/invokeRegistry.ts:45-49](file://src/main/ipc/invokeRegistry.ts#L45-L49)

## 结论
RDC-Agent 主进程采用模块化、事件驱动与服务化设计：
- AgentOrchestrator 将提示词计划、工具装配、LLM 调用与子代理编排整合为统一入口，支持测试模式与凭据刷新。
- SettingsService 提供强一致的设置持久化与凭据管理，支撑 LLM 配置与工具链动态切换。
- RdcSessionService 封装 RDC 运行时上下文与设备生命周期，保障捕获打开/关闭的可靠性与可恢复性。
- **守护进程收割机制**：**新增的关键功能**确保应用启动时自动清理上次运行遗留的 RDC 守护进程，防止资源泄漏和端口冲突，提升系统稳定性。
- IPC 层通过 invokeRegistry 与 rendererEventHub 实现通道对等性与跨窗口事件广播，提升可观测性与调试效率。
整体架构清晰、职责分离明确，适合扩展新的领域处理器与工具能力。

[本节为总结性内容，不直接分析具体文件]

## 附录
- 关键入口与导出：
  - 主进程入口导出 rdcSessionService，供外部模块使用。
  - IPC handlers 通过 workbenchHandlers 统一注册，避免分散维护。
- 环境变量与模式：
  - HEADLESS/BROWSER_QA/TEST_MODE 影响窗口创建与安全策略。
  - USER_DATA/HOME 控制用户数据目录与隔离环境。
- 安全策略：
  - CSP 限制脚本与样式来源，禁止 object-src，限制 connect-src 到受信任域名。
  - 默认拒绝权限请求，导航白名单仅允许开发服务器与 file 协议。
- **守护进程收割配置**：
  - 通过 settingsService.getAll().tooling.rdc-agentCli 获取 rdc-tool CLI 配置。
  - 中间目录路径由 withRdcHostRuntimeEnv 确定。
  - 支持自定义 execute、readCommandLine、killPid 函数进行测试和调试。

[本节为补充信息，不直接分析具体文件]