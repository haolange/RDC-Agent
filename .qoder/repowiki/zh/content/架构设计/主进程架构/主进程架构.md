...
- [src/main/sessions/OwnedRdxDaemonRegistry.ts](file://src/main/sessions/OwnedRdxDaemonRegistry.ts)
</cite>

## 更新摘要
**变更内容**
- 新增守护进程收割功能：在主进程初始化期间集成遗留守护进程检查与清理机制
- 更新服务初始化流程：在 RDX CLI 调用器初始化后执行守护进程收割
...
- **守护进程收割机制**：主进程启动时自动检测并清理上次运行遗留的 RDX 守护进程，防止资源泄漏和端口冲突。
...
A --> E["守护进程收割器<br/>src/main/sessions/OwnedRdxDaemonRegistry.ts"]
B --> F["Agent 编排器<br/>src/main/workflow/debugger/AgentOrchestrator.ts"]
B --> G["RDX 运行时处理器<br/>src/main/ipc/rdxRuntimeHandlers.ts"]
...
E --> K["RDX CLI 调用器<br/>src/main/tools/RdxCliInvokerService.ts"]
...
- [src/main/sessions/OwnedRdxDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L93-L136)
...
- **OwnedRdxDaemonRegistry**：**新增的守护进程收割组件**，负责检测并清理上次运行遗留的 RDX 守护进程，包括进程终止、注册表清理和资源释放。
...
- **执行守护进程收割**：检查上次运行遗留的 RDX 守护进程并进行清理。
...
Main->>Harvest : harvestOwnedRdxDaemons()
...
### 守护进程收割机制
**新增功能**：主进程初始化期间自动检测和清理上次运行遗留的 RDX 守护进程，防止资源泄漏和端口冲突。
...
- **工作流程**：
  - 扫描 `intermediateRoot/runtime/rdx_cli` 目录查找守护进程状态文件
  - 合并现有注册表记录与发现的守护进程
...
**图表来源**
- [src/main/sessions/OwnedRdxDaemonRegistry.ts:93-136](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L93-L136)
- [src/main/sessions/OwnedRdxDaemonRegistry.ts:138-181](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L138-L181)
- [src/main/sessions/OwnedRdxDaemonRegistry.ts:183-203](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L183-L203)
...
## 依赖关系分析
- 主进程入口依赖：settingsService、rdxCliInvokerService、replayDeviceService、runtimeLogService、browserAppBridge、shutdownCoordinator、agentOrchestrator、turnCoordinator、processSupervisor、**harvestOwnedRdxDaemons**。
- IPC 层依赖：workbenchHandlers 组合多个领域处理器，invokeRegistry 提供通道注册与对等性校验，rendererEventHub 提供事件广播。
- 服务间耦合：AgentOrchestrator 依赖 settingsService、storageAdapter、providerRuntimeCredentialService、workflowProjectionPublisher；RdxSessionService 依赖 rdxShellActionService、rdxCliInvokerService、replayDeviceService、settingsService；**守护进程收割器依赖 rdxCliInvokerService、settingsService、runtimeLogService**。
...
- **守护进程收割失败**：检查 harvestOwnedRdxDaemons 返回的 failed 数组，查看具体的 contextId 和错误信息；确认 RDX CLI 配置和进程权限。
...
- [src/main/sessions/OwnedRdxDaemonRegistry.ts:122-133](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L122-L133)
- [src/main/sessions/RdxSessionService.ts:69-212](file://src/main/sessions/RdxSessionService.ts#L69-L212)
- [src/main/sessions/RdxSessionService.ts:710-753](file://src/main/sessions/RdxSessionService.ts#L710-L753)
...
- **守护进程收割机制**：**新增的关键功能**确保应用启动时自动清理上次运行遗留的 RDX 守护进程，防止资源泄漏和端口冲突，提升系统稳定性。
...
- **守护进程收割配置**：
  - 通过 settingsService.getAll().tooling.rdxCli 获取 RDX CLI 配置。
  - 中间目录路径由 withRdxHostRuntimeEnv 确定。
