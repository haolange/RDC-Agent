...
A --> E["resolveRdcBatchInvocation<br/>Windows rdc.bat 适配"]
...
- resolveRdcBatchInvocation：在 Windows 平台将 rdc.bat 调用转发至 PowerShell 脚本以增强可观测性与一致性。
...
- 批量调用适配：在 Windows 上可将 rdc.bat 转发到 PowerShell 脚本执行。
...
BuildArgs --> ResolveWin["Windows rdc.bat 适配"]
...
- 批量调用适配：Windows 上将 rdc.bat 转为 PowerShell 脚本执行，提升一致性与可观测性。
