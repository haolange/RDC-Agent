...
- 深入解析 RdxCliInvokerService 的外部命令调用机制，包括进程管理、参数传递与输出处理。
...
- tools：封装外部 CLI 与 shell 执行能力（RdxCliInvokerService、ShellInvocationService）。
...
```mermaid
graph TB
A["RdxCliInvokerService<br/>外部命令编排"] --> B["ShellInvocationService<br/>shell 执行入口"]
B --> C["ProcessSupervisor<br/>子进程注册/终止/超时"]
A --> D["RdxNativeProtocol<br/>协议校验"]
A --> E["resolveRdxBatchInvocation<br/>Windows rdx.bat 适配"]
...
## 核心组件
- RdxCliInvokerService：负责将上层工具调用转换为外部 RDX CLI 命令，完成参数归一化、上下文注入、超时控制、结果解析与追踪事件上报。
...
- RdxNativeProtocol：对 RDX CLI 的标准 JSON 信封进行严格校验，确保 ok:true、result_kind 与 data 存在，并校验 context_id 归属。
- resolveRdxBatchInvocation：在 Windows 平台将 rdx.bat 调用转发至 PowerShell 脚本以增强可观测性与一致性。
...
Invoker->>Resolver : resolveRdxBatchInvocation(command, args)
...
Protocol-->>Invoker : RdxNativeEnvelope
...
### RdxCliInvokerService：外部命令编排与结果处理
...
- 结果解析：使用 RdxNativeProtocol 校验标准信封，区分成功与失败路径，构造 ToolCallResult 并触发追踪事件。
...
InvokeShell --> ParseRes["解析 RDX 信封"]
ParseRes --> Ok{"ok:true?"}
...
图表来源
- [RdxCliInvokerService.ts:49-221](file://src/main/tools/RdxCliInvokerService.ts#L49-L221)
- [RdxCliInvokerService.ts:248-355](file://src/main/tools/RdxCliInvokerService.ts#L248-L355)
- [resolveRdxBatchInvocation.ts:1-21](file://src/main/tools/resolveRdxBatchInvocation.ts#L1-L21)
- [RdxNativeProtocol.ts:12-34](file://src/main/tools/RdxNativeProtocol.ts#L12-L34)
...
## 依赖关系分析
- RdxCliInvokerService 依赖 ShellInvocationService 执行外部命令，依赖 RdxNativeProtocol 校验响应，依赖 resolveRdxBatchInvocation 做 Windows 兼容。
...
A --> C["RdxNativeProtocol"]
A --> D["resolveRdxBatchInvocation"]
...
## 故障排查指南
- 命令不可用：检查 RdxCliInvokerService 的可用性判断（enabled、command、workingDirectory、timeoutMs），必要时调整设置。
...
- 协议校验失败：RdxNativeProtocol 会校验 exitCode、JSON 格式、ok 字段、result_kind 与 data；失败时检查外部命令输出是否符合规范。
...
- RdxCliInvokerService 负责高层编排与协议校验，确保调用语义清晰、结果可追溯。
...
participant T as "RdxCliInvokerService"
...
```mermaid
flowchart TD
A["调用方"] --> B["工具执行入口<br/>RdxCliInvokerService"]
