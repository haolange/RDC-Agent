...
- [RdcCliInvokerService.test.ts](file://src/main/tools/RdxCliInvokerService.test.ts)
- [ShellInvocationService.test.ts](file://src/main/tools/ShellInvocationService.test.ts)
</cite>

## 更新摘要
**所做更改**
- 将所有 RDX 到 RDC 的引用更新为 RDC，包括 executeRdxShell 到 executeRdcShell、RdxProbeLifecycle 到 RdcProbeLifecycle
- 更新了工具执行组件中的品牌重命名，确保所有相关文件都使用新的 RDC 命名约定
- 保持了文档的整体结构和一致性，同时反映了代码库中的实际变更
...
A --> E["resolveRdcBatchInvocation<br/>Windows rdx.bat 适配"]
...
## 核心组件
- RdcCliInvokerService：负责将上层工具调用转换为外部 RDC CLI 命令，完成参数归一化、上下文注入、超时控制、结果解析与追踪事件上报。
...
- RdcNativeProtocol：对 RDC CLI 的标准 JSON 信封进行严格校验，确保 ok:true、result_kind 与 data 存在，并校验 context_id 归属。
- resolveRdcBatchInvocation：在 Windows 平台将 rdx.bat 调用转发至 PowerShell 脚本以增强可观测性与一致性。
...
- 批量调用适配：在 Windows 上可将 rdx.bat 转发到 PowerShell 脚本执行。
...
BuildArgs --> ResolveWin["Windows rdx.bat 适配"]
...
图表来源
- [RdcCliInvokerService.ts:52-76](file://src/main/tools/RdcCliInvokerService.ts#L52-L76)
- [RdcCliInvokerService.ts:171-224](file://src/main/tools/RdcCliInvokerService.ts#L171-L224)
- [resolveRdcBatchInvocation.ts:1-7](file://src/main/tools/resolveRdcBatchInvocation.ts#L1-L7)
- [RdcNativeProtocol.ts:12-43](file://src/main/tools/RdcNativeProtocol.ts#L12-L43)
...
章节来源
- [RdcCliInvokerService.ts:22-370](file://src/main/tools/RdcCliInvokerService.ts#L22-L370)
- [RdcCliInvokerService.test.ts:1-48](file://src/main/tools/RdxCliInvokerService.test.ts#L1-L48)
...
### executeRdcShell：结构化 RDC 壳执行
- 输入验证：使用 Zod schema 验证操作类型和参数，支持 discovery 和 operation 两种模式。
- 权限检查：验证会话所有权、项目绑定、上下文租约和隔离状态。
- 操作授权：通过 RdcOperationPolicy 验证操作的合法性和副作用范围。
- 执行协调：使用 RdcOperationCoordinator 管理并发执行和状态一致性。
- 证据验证：对实验性操作进行证据收集和验证，确保操作的可追溯性。
- 收据生成：为重要操作生成持久化的执行收据，支持审计和恢复。

章节来源
- [executeRdcShell.ts:23-133](file://src/main/tools/executeRdcShell.ts#L23-L133)

### RdcProbeLifecycle：RDC 探针生命周期管理
- 租约管理：复用产品捕获生命周期，不为任意上下文 ID 创建租约。
- 捕获验证：验证捕获路径是否为已注册的项目输入。
- 设备要求：确保选择了有效的回放设备。
- 会话集成：通过 rdcSessionService 打开和关闭项目输入。
- 所有权检查：防止委托租约关闭父捕获。
...
- 批量调用适配：Windows 上将 rdx.bat 转为 PowerShell 脚本执行，提升一致性与可观测性。
