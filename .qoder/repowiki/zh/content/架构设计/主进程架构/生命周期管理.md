...
- src/main/sessions：RDX 守护进程所有权管理
...
- [ResourceExecutionLifetime.ts:1-21](file://src/main/runtime/ResourceExecutionLifetime.ts#L1-L21)
- [OwnedRdxDaemonRegistry.ts:1-252](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L1-L252)
...
- OwnedRdxDaemonRegistry
  - 职责：管理应用拥有的 RDX 守护进程；记录守护进程元数据；提供守护进程的发现、停止和清理功能。
...
- [OwnedRdxDaemonRegistry.ts:16-34](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L16-L34)

## 架构总览
应用生命周期分为"运行期"和"关闭期"。运行期由各个子系统独立管理自身资源；关闭期由 ShutdownCoordinator 统一调度，按阶段依次清理，确保依赖倒置与有序释放。

**更新** 新增了 `release_owned_runtimes` 阶段，确保在终止进程前正确释放 RDX 守护进程。

```mermaid
sequenceDiagram
participant App as "应用"
participant SC as "ShutdownCoordinator"
participant ORD as "OwnedRdxDaemonRegistry"
...
- 阶段定义与顺序
  ...
  - 阶段顺序即"依赖倒序"，先停止接受新任务，再中止进行中任务，再等待生产者结束，**然后释放拥有的 RDX 守护进程**，最后终止进程与刷新存储。
- 超时与强制退出
...
**更新** 新增的 `release_owned_runtimes` 阶段确保在终止进程前先释放 RDX 守护进程，防止守护进程成为孤儿进程。
...
ForEachPhase --> |release_owned_runtimes| RunRelease["释放 RDX 守护进程"]
...
### OwnedRdxDaemonRegistry：RDX 守护进程管理
- 守护进程所有权跟踪
  - rememberOwnedRdxDaemon：记录应用拥有的 RDX 守护进程信息，包括 contextId、PID、启动时间等。
  - forgetOwnedRdxDaemon：在守护进程成功停止后移除记录。
  - listOwnedRdxDaemons：列出所有记录的守护进程。
- 守护进程发现与清理
  - harvestOwnedRdxDaemons：扫描中间目录发现守护进程，尝试优雅停止，失败时强制终止。
  - stopOwnedDaemon：通过 CLI 命令停止守护进程，验证上下文匹配性。
  - killVerifiedOwnedPids：验证并强制终止残留的守护进程和 worker 进程。
- 安全验证
  - isAppRdxContextId：验证 contextId 格式符合应用拥有的守护进程规范。
  - 命令行验证：确保目标 PID 确实是对应的守护进程或 worker。

**新增** 此组件在 `release_owned_runtimes` 阶段被调用，确保 RDX 守护进程在进程终止前正确关闭。
...
## 依赖关系分析
- ShutdownCoordinator 与 OwnedRdxDaemonRegistry
  - 关闭阶段 release_owned_runtimes 调用 OwnedRdxDaemonRegistry.harvestOwnedRdxDaemons，确保 RDX 守护进程在进程终止前正确释放。
...
**更新** 新增了 ShutdownCoordinator 与 OwnedRdxDaemonRegistry 之间的依赖关系。

```mermaid
graph LR
SC["ShutdownCoordinator"] --> ORD["OwnedRdxDaemonRegistry"]
...
- 守护进程批量处理：OwnedRdxDaemonRegistry 使用批量方式处理多个守护进程，提高清理效率。
...
- 守护进程释放的可靠性
  - OwnedRdxDaemonRegistry 提供双重保障：先尝试优雅停止，失败时强制终止残留进程。
...
**更新** 增强了守护进程释放的可靠性说明，确保 RDX 守护进程不会成为孤儿进程。
...
- **OwnedRdxDaemonRegistry 确保 RDX 守护进程在进程终止前正确释放，防止守护进程成为孤儿进程。**
...
**更新** 新增了 OwnedRdxDaemonRegistry 在关闭流程中的作用。
