...
## 更新摘要
**变更内容**
- 新增 ProductLogo 组件：支持动态重着色，根据主题强调色实时渲染产品标志
- 增强主进程窗口图标管理：实现应用图标与主题同步，支持系统主题变化时自动更新
- 改进标题栏集成：在产品品牌区域集成动态 Logo，提升品牌一致性
- 共享像素处理逻辑：在渲染器和主进程中复用相同的 Logo 重着色算法
...
**更新** 新增了 ProductLogo 组件，支持动态重着色功能，能够根据当前主题强调色实时渲染产品标志。主进程窗口图标管理得到增强，现在能够与应用主题保持同步。标题栏集成了新的产品品牌元素，提升了整体品牌一致性。
...
B --> M["命令面板<br/>patterns/CommandPalette"]
B --> N["覆盖层堆栈<br/>lib/overlayStack.ts"]
N --> O["TaskDialog<br/>ui/TaskDialog.tsx"]
N --> P["ModalFocus Hook<br/>lib/useModalFocus.ts"]
L --> Q["Select 组件<br/>ui/Select"]
L --> R["SearchMultiSelect<br/>ui/SearchMultiSelect"]
S["主进程窗口图标<br/>src/main/window/productIcon.ts"] --> T["应用图标管理"]
```
...
- 设置面板 SettingsModal：以多节导航组织通用、外观、工作区、模型、技能、代理、工具、钩子、策略等设置项，支持运行时概览与连接对话框。**更新** 改进了 AgentListItem 布局和增强资源编辑器布局。
...
## 架构总览
渲染层采用"壳 + 面"的分层：AppShell 提供稳定的外壳容器，App 注入业务页面与全局覆盖层；WorkbenchShell 将侧栏、主区域、右侧检查面板与 Composer 组合为工作区；各功能模块通过 stores 与 hooks 共享状态，并通过 IPC 与主进程通信。覆盖层堆栈系统统一管理所有焦点捕获的覆盖层，确保嵌套对话框的正确行为。**更新** 新增了产品品牌系统集成，包括渲染器的 ProductLogo 组件和主进程的窗口图标管理。
...
- **更新** 改进了 AgentListItem 布局，提供更好的视觉层次和信息密度；增强了资源编辑器布局，支持更复杂的资源配置场景。
...
- 性能
  ...
  - **新增** 共享重着色算法，避免代码重复。
  - Select 组件使用智能定位算法，避免菜单溢出视口。
  - SearchMultiSelect 使用高效的搜索过滤算法，支持大数据集。
- 可访问性
