...
## 更新摘要
**所做更改**
- 新增桌面运行时路径隔离功能章节，详细说明RDX_INTERMEDIATE_ROOT环境变量的作用
- 更新AppPathService部分，增加rdxIntermediateRoot属性的说明
- 扩展RdxCliInvokerService部分，描述环境变量注入机制
...
```mermaid
graph TB
A["RdxRuntimeService<br/>运行时服务"] --> B["AppPathService<br/>路径服务"]
...
J["withRdxHostRuntimeEnv<br/>桌面运行时环境"] --> K["RdxCliInvokerService<br/>CLI调用服务"]
B --> J
K --> L["RDX_INTERMEDIATE_ROOT<br/>环境变量注入"]
```

图表来源
- [RdxRuntimeService.ts:24-151](file://src/main/runtime/RdxRuntimeService.ts#L24-L151)
...
- [withRdxHostRuntimeEnv.ts:1-17](file://src/main/tools/withRdxHostRuntimeEnv.ts#L1-L17)
- [RdxCliInvokerService.ts:29-49](file://src/main/tools/RdxCliInvokerService.ts#L29-L49)
...
## 核心组件
- RdxRuntimeService：提供运行时资源的列表、概览、写入、导入、删除；聚合钩子与 MCP 信息，汇总诊断。
...
- withRdxHostRuntimeEnv：桌面运行时环境隔离，注入RDX_INTERMEDIATE_ROOT环境变量防止路径污染。
- RdxCliInvokerService：集成环境变量注入机制，确保CLI调用使用隔离的中间目录。

章节来源
- [RdxRuntimeService.ts:24-302](file://src/main/runtime/RdxRuntimeService.ts#L24-L302)
...
participant Runtime as "RdxRuntimeService"
...
participant Env as "withRdxHostRuntimeEnv"
participant CLI as "RdxCliInvokerService"
Client->>Runtime : overview(projectRoot?)
Runtime->>Paths : initializeRuntime() / getProjectRdxPaths()
...
CLI->>Env : withRdxHostRuntimeEnv(settings)
Env-->>CLI : settings.env.RDX_INTERMEDIATE_ROOT
CLI->>Proc : spawn(command, args, env)
Runtime-->>Client : {userPaths, projectPaths, resources, hooks, mcpServers, diagnostics}
```

图表来源
- [RdxRuntimeService.ts:92-151](file://src/main/runtime/RdxRuntimeService.ts#L92-L151)
...
- [RdxCliInvokerService.ts:161-164](file://src/main/tools/RdxCliInvokerService.ts#L161-L164)

## 详细组件分析

### 桌面运行时路径隔离机制
**新增功能** 桌面运行时路径隔离通过`withRdxHostRuntimeEnv`模块实现，防止桌面用户账户污染CLI的默认中间目录。

- **环境变量注入**
  - `RDX_INTERMEDIATE_ROOT_ENV = 'RDX_INTERMEDIATE_ROOT'`：定义隔离目录的环境变量名
  - `hostRdxIntermediateRoot()`：获取用户作用域的隔离中间目录路径
  - `withRdxHostRuntimeEnv(settings)`：为CLI调用注入隔离的环境变量

- **路径隔离原理**
  - 每个桌面用户拥有独立的`.rdx/rdx-intermediate`目录
  - CLI工具通过`RDX_INTERMEDIATE_ROOT`环境变量访问隔离目录
  - 防止不同用户间的临时文件、缓存和中间产物相互污染

- **集成点**
  - `RdxCliInvokerService.createRuntimeMetadata()`：读取隔离目录路径到运行时元数据
  - `RdxCliInvokerService.executeCLI()`：在所有CLI调用中注入环境变量
  - `ToolRuntimeMetadata.intermediateRoot`：暴露当前使用的隔离目录

```mermaid
flowchart TD
Start(["CLI调用"]) --> Check{"检查环境变量"}
Check --> |未设置| Inject["注入RDX_INTERMEDIATE_ROOT"]
Check --> |已设置| UseExisting["使用现有值"]
Inject --> GetPath["appPathService.getUserRdxPaths().rdxIntermediateRoot"]
GetPath --> SetEnv["env[RDX_INTERMEDIATE_ROOT] = path"]
...
图表来源
- [withRdxHostRuntimeEnv.ts:6-16](file://src/main/tools/withRdxHostRuntimeEnv.ts#L6-L16)
...
- 作用域划分
  ...
  - 用户（user）：位于用户目录下的 .rdx 对应子目录，包括隔离的中间目录
  - 项目（project）：位于项目根 .rdx 下，优先级最高，可覆盖用户配置
- 资源类型与扩展名
...
图表来源
- [RdxRuntimeService.ts:40-90](file://src/main/runtime/RdxRuntimeService.ts#L40-L90)
- [ScopedResourceResolver.ts:30-95](file://src/main/runtime/ScopedResourceResolver.ts#L30-L95)

章节来源
- [RdxRuntimeService.ts:27-90](file://src/main/runtime/RdxRuntimeService.ts#L27-L90)
- [ScopedResourceResolver.ts:29-124](file://src/main/runtime/ScopedResourceResolver.ts#L29-L124)
...
RdxRuntimeService --> McpTrustService : "信任/撤销"
...
- [McpTrustService.ts:43-129](file://src/main/settings/McpTrustService.ts#L43-L129)
- [AgentRuntimeConfigService.ts:120-232](file://src/main/settings/AgentRuntimeConfigService.ts#L120-L232)
- [RdxRuntimeService.ts:153-175](file://src/main/runtime/RdxRuntimeService.ts#L153-L175)
...
## 依赖关系分析
- RdxRuntimeService 依赖 AppPathService 获取路径，依赖 HookEngine 与 AgentRuntimeConfigService 获取钩子与 MCP 信息，依赖 ScopedResourceResolver 进行资源合并
...
- withRdxHostRuntimeEnv 依赖 AppPathService 获取隔离目录路径，被 RdxCliInvokerService 集成使用
- RdxCliInvokerService 通过环境变量注入机制确保CLI调用使用隔离的中间目录

```mermaid
graph LR
R["RdxRuntimeService"] --> P["AppPathService"]
R --> H["HookEngine"]
...
P --> W["withRdxHostRuntimeEnv"]
W --> RC["RdxCliInvokerService"]
RC --> ENV["RDX_INTERMEDIATE_ROOT"]
```
...
- 资源路径
  - 尽量将大文件放在 inputs/artifacts 目录，避免污染 .rdx 配置目录
...
- **路径隔离问题**
  ...
  - 解决：检查`RDX_INTERMEDIATE_ROOT`环境变量是否正确注入；确认`withRdxHostRuntimeEnv`函数被正确调用；验证`ToolRuntimeMetadata.intermediateRoot`字段值
...
- [withRdxHostRuntimeEnv.test.ts:15-33](file://src/main/tools/withRdxHostRuntimeEnv.test.ts#L15-L33)
