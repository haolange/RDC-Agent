...
- AppPathService：统一用户、项目与应用状态路径，自动创建目录，支持内置运行时根定位。
...
## 架构总览
运行时以“作用域资源 + 钩子 + 策略 + 进程监督”为核心，形成可插拔、可审计、可控制的执行环境。
...
## 结论
本运行时通过作用域资源、钩子、策略与进程监督的组合，提供了安全、可控、可扩展的执行环境。开发者可通过钩子与策略定制行为，同时借助信任机制与权限控制保障安全性。建议在项目中合理使用项目作用域覆盖、策略收紧与进程超时控制，以获得最佳的性能与稳定性。