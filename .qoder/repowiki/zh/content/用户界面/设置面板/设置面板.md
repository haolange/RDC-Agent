...
## 更新摘要
**变更内容**
- 增强了模型能力摘要工具，提供更准确的模型功能展示和验证状态
- 更新了 Provider 模型发现机制，改进了与协调后的 Provider 目录数据的集成
- 新增了模型能力探测功能，支持运行时验证模型功能和上下文层级
- 优化了模型路由选择和推理控制的用户界面
- 改进了模型定价信息和配额状态的显示
...
**更新** 本次更新重点增强了模型能力摘要功能，改进了 Provider 模型发现和协调能力矩阵的准确性，为设置面板提供了更精确的模型功能展示和验证机制。
...
R --> S["能力摘要工具<br/>modelCapabilitySummaryUtils.ts"]
```
...
- [src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx:36-293](file://src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx#L36-L293)
...
- **新增** 模型能力摘要组件：提供详细的模型功能展示、验证状态和能力探测功能。

**更新** 新增了 ProviderModelCapabilitySummary 组件和 modelCapabilitySummaryUtils 工具，为模型设置提供了强大的能力展示和验证功能。
...
**更新** 新的架构引入了增强的模型能力摘要系统，通过 ProviderModelCapabilitySummary 组件和 modelCapabilitySummaryUtils 工具提供更精确的模型功能展示和验证。
...
participant RO as "RDC运行时概览<br/>useRdcRuntimeOverview"
...
PMCS->>MCUT : 构建能力芯片和行数据
MCUT-->>PMCS : 返回格式化数据
PMCS->>P : 运行能力探测
P-->>PMCS : 返回探测结果
U->>RO : 获取运行时环境信息
RO->>P : 调用rdcRuntime.getOverview
P-->>RO : 返回运行时信息
RO-->>M : 提供运行时数据
```
...
- 职责：提供模型的详细能力展示、验证状态和能力探测功能。
- 功能特性：
  - 能力芯片：显示模型的关键能力指标，如上下文窗口、推理模式、快速模式等。
  - 上下文层级：展示不同上下文层级的限制、授权状态和成本信息。
  - 定价信息：显示输入、输出、缓存读取和写入的定价详情。
  - 偏好设置：提供路由选择、推理默认值和客户端预算的配置。
  - 能力探测：支持运行能力探测来验证模型的实际功能。
  - 状态指示：显示加载状态、错误状态和刷新状态。

**新增** 这是新引入的模型能力摘要组件，为设置面板提供了强大的模型功能展示和验证能力。
...
**新增** 这是新引入的能力摘要工具模块，为模型能力展示提供了完整的工具函数集。
...
- 渲染层依赖：
  ...
  - **新增** ProviderModelCapabilitySummary 依赖 modelCapabilitySummaryUtils 工具函数。
- 主进程依赖：
...
**更新** 新增了 ProviderModelCapabilitySummary 和 modelCapabilitySummaryUtils 的依赖关系，以及 useRdcRuntimeOverview、scopedSkillSelection 和 AgentListItem 组件的依赖关系。
...
PMCS --> MCUT["modelCapabilitySummaryUtils.ts"]
...
- **新增** 模型能力摘要的计算优化：通过 memoization 避免重复计算能力芯片和行数据。
- **新增** 能力探测的防抖处理：避免频繁触发能力探测请求。

**更新** 增加了针对 RDC 运行时概览、作用域技能选择、Agent 列表和模型能力摘要的性能优化考虑。
...
- **新增** 模型能力摘要加载失败：
  - 现象：模型能力信息为空或显示错误。
  - 排查：检查 ProviderModelCapabilitySummary 组件的状态处理；确认 modelCapabilitySummaryUtils 工具函数的返回值。
  - 参考：[src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx:111-115](file://src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx#L111-L115)
- **新增** 能力探测失败：
  - 现象：能力探测按钮点击后无响应或报错。
  - 排查：检查 testModelCapability 函数的实现；确认 Provider 连接状态和模型可用性。
  - 参考：[src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx:77-106](file://src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx#L77-L106)

**更新** 增加了 RDC 运行时概览、作用域技能选择、Agent 列表和模型能力摘要相关的故障排除指南。
...
- [src/renderer/features/settings/SettingsModal/sections/AgentListItem.tsx:18-38](file://src/renderer/features/settings/SettingsModal/sections/AgentListItem.tsx#L18-L38)
- [src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx:111-115](file://src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx#L111-L115)
- [src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx:77-106](file://src/renderer/features/settings/SettingsModal/sections/ProviderModelCapabilitySummary.tsx#L77-L106)

## 结论
RDC-Agent 设置面板经过重大简化重构后，采用了更加精简和高效的设计。通过移除 RdxActionsFields 组件和生命周期模板配置功能，显著降低了设置界面的复杂度。新增的 RDC 运行时概览集成、作用域技能选择功能、Agent 列表优化和模型能力摘要系统，为用户提供了更加直观和可靠的配置体验。特别是 useRdcRuntimeOverview Hook 提供的准确运行时环境信息，scopedSkillSelection 提供的精确技能管理能力，以及 ProviderModelCapabilitySummary 提供的强大模型功能展示和验证机制，大大提升了设置的准确性和用户体验。建议在新增设置项时遵循现有模式：使用 SettingsPageContent 进行统一渲染，定义清晰的类型和默认值，实现主进程写入与清理逻辑，并在渲染层提供对应的表单与保存流程。

**更新** 本次增强集成的 RDC 运行时概览、作用域技能选择、Agent 列表优化和模型能力摘要系统显著提升了设置面板的功能性和用户体验，为未来的功能扩展奠定了更加稳固的基础。
...
- 模型与 Provider（models）
  ...
  - 采用卡片式布局展示，支持搜索和分组。
  - **新增** 模型能力摘要：显示模型的功能指标、上下文层级、定价信息和能力探测。
- Agent 清单（agents）
...
**更新** 移除了 RDX Actions 和生命周期模板配置相关的说明，新增了 RDC 运行时概览、作用域技能选择、Agent 列表优化和模型能力摘要的描述。
...
- **新增** 查看模型能力：选择模型后查看详细的能力摘要，包括上下文层级、推理模式和定价信息。
- **新增** 运行能力探测：点击能力探测按钮验证模型的实际功能和限制。
- 配置 Agent：编辑清单草稿，自动保存，处理 handoff 冲突。
- 管理 MCP 服务：配置 MCP 服务器，设置信任状态，监控连接状态。
- 查看 Provider 连接摘要：查看协议信息和连接状态。

**更新** 移除了生命周期模板配置相关的场景，新增了 RDC 运行时概览、作用域技能选择、Agent 列表和模型能力摘要相关的场景。
...
- **新增** 模型能力摘要问题：检查 ProviderModelCapabilitySummary 组件和 modelCapabilitySummaryUtils 工具函数。
- **新增** 能力探测失败：检查 testModelCapability 函数和 Provider 连接状态。
- MCP 服务面板问题：检查 useMcpStatus Hook 和连接状态。
- Provider 连接摘要问题：检查连接阶段计算和协议信息。

**更新** 增加了 RDC 运行时概览、作用域技能选择、Agent 列表和模型能力摘要相关的故障排除要点。
...
- **新增** model-capabilities.css：为模型能力摘要组件定义的样式，包括能力芯片、层级显示和探测区域。
...
- [src/renderer/features/settings/SettingsModal/sections/model-capabilities.css:1-100](file://src/renderer/features/settings/SettingsModal/sections/model-capabilities.css#L1-L100)