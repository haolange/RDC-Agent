...
- [src/renderer/features/settings/SettingsModal/sections/RdcCliInvokerSettingsFields.tsx](file://src/renderer/features/settings/SettingsModal/sections/RdcCliInvokerSettingsFields.tsx)
...
- [src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts](file://src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts)
- [src/renderer/features/settings/SettingsModal/sections/AgentHandoffCard.tsx](file://src/renderer/features/settings/SettingsModal/sections/AgentHandoffCard.tsx)
...
- [src/renderer/platform/rdcInstallation.ts](file://src/renderer/platform/rdcInstallation.ts)
...
## 更新摘要
**变更内容**
- 增强了 Agent Handoff 配置的验证机制，支持 send、showContinueOn、model 和 requiredSkillIds 字段的完整验证
- 改进了 Handoff 设置的持久化逻辑，确保简化声明式续传模型的配置正确保存
- 新增了更严格的类型检查和数据清理机制，提升配置可靠性
- 优化了用户界面反馈，提供更清晰的验证错误提示
...
**更新** 本次更新重点增强了 Agent Handoff 配置的验证和持久化机制，特别是针对新的简化声明式续传模型的支持，包括 send、showContinueOn、model 和 requiredSkillIds 字段的完整验证流程。
...
**更新** 增强了 Agent Handoff 配置的验证和持久化机制，特别是针对新的简化声明式续传模型的支持。
...
**更新** 新的架构引入了增强的 Agent Handoff 配置验证机制，提供了更可靠的配置持久化和错误处理。
...
participant V as "RDC安装验证<br/>validateRdcInstallation"
participant P as "主进程设置服务<br/>SettingsService.ts"
U->>M : 打开设置面板
M->>S : 初始化并加载设置内容
S->>P : 获取运行时概览数据
P-->>S : 返回运行时信息
U->>V : 验证rdc-tool CLI安装
V->>P : 检查CLI可用性和版本
P-->>V : 返回安装状态
...
- [src/renderer/features/settings/SettingsModal/SettingsPageContent.tsx:32-174](file://src/renderer/features/settings/SettingsModal/SettingsPageContent.tsx#L32-L174)
- [src/renderer/features/settings/SettingsModal/sections/McpServicesPanel.tsx:53-146](file://src/renderer/features/settings/SettingsModal/sections/McpServicesPanel.tsx#L53-L146)
- [src/renderer/platform/rdcInstallation.ts:5-14](file://src/renderer/platform/rdcInstallation.ts#L5-L14)
...
### Agent Handoff 配置验证（agentHandoffValidation.ts）
- 职责：提供 Agent Handoff 配置的完整验证和持久化逻辑，支持新的简化声明式续传模型。
- 功能特性：
  - 字段验证：验证 label、agent、prompt 等必需字段的有效性。
  - 模型验证：检查 model 字段是否为有效的模型选项。
  - 目标验证：验证目标 Agent 是否存在且已启用。
  - 持久化清理：智能清理未使用的字段，如 send:false 会被省略。
  - 技能ID处理：正确处理 requiredSkillIds 数组的格式化和过滤。

**新增** 这是新引入的增强验证机制，专门支持新的简化声明式续传模型，包括 send、showContinueOn、model 和 requiredSkillIds 字段的完整验证流程。

**章节来源**
- [src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts:35-50](file://src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts#L35-L50)
- [src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts:60-84](file://src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts#L60-L84)

### Agent Handoff 卡片组件（AgentHandoffCard.tsx）
- 职责：提供 Agent Handoff 配置的交互式编辑界面。
- 功能特性：
  - 发送开关：send 字段的布尔值切换，true 表示点击后自动发送。
  - 继续显示：showContinueOn 字段的布尔值切换，false 时隐藏继续按钮。
  - 模型选择：支持从可用模型列表中选择特定模型。
  - 技能ID输入：支持多行文本输入，自动分割为技能ID数组。
  - 实时验证：即时显示配置验证结果和错误提示。

**新增** 这是新引入的增强配置界面，专门支持新的简化声明式续传模型的所有字段。

**章节来源**
- [src/renderer/features/settings/SettingsModal/sections/AgentHandoffCard.tsx:186-212](file://src/renderer/features/settings/SettingsModal/sections/AgentHandoffCard.tsx#L186-L212)

### rdc-tool CLI 安装验证（RdcCliInvokerSettingsFields.tsx）
- 职责：提供 rdc-tool CLI 工具的实时安装验证功能，确保工具配置的正确性。
- 功能特性：
  - 实时验证：点击验证按钮后立即检查 rdc-tool CLI 的安装状态和版本信息。
  - 状态反馈：显示验证中、已验证、不可用等不同状态的提示信息。
  - 错误处理：捕获并显示验证过程中的各种错误情况。
  - 配置同步：确保验证基于最新的配置草稿而非已保存的配置。

**新增** 这是新引入的 rdc-tool CLI 安装验证功能，显著提升了工具配置的可靠性和用户体验。

**章节来源**
- [src/renderer/features/settings/SettingsModal/sections/RdcCliInvokerSettingsFields.tsx:34-43](file://src/renderer/features/settings/SettingsModal/sections/RdcCliInvokerSettingsFields.tsx#L34-L43)
- [src/renderer/features/settings/SettingsModal/sections/RdcCliInvokerSettingsFields.tsx:68-75](file://src/renderer/features/settings/SettingsModal/sections/RdcCliInvokerSettingsFields.tsx#L68-L75)

### RDC 安装验证器（rdcInstallation.ts）
- 职责：封装 rdc-tool CLI 安装的验证逻辑，提供统一的验证接口。
- 功能特性：
  - 配置比较：检查草稿配置与已保存配置是否一致，避免不必要的验证调用。
  - 异步验证：通过 Electron API 调用主进程的 rdc-tool CLI 验证功能。
  - 结果处理：返回详细的工具运行时摘要信息或空值表示需要保存后才能验证。

**新增** 这是新引入的 RDC 安装验证器，为设置面板提供了可靠的安装状态检查能力。
...
**新增** 这是新引入的严格解析器，专门支持新的简化声明式续传模型的所有字段验证。
...
**新增** 这是新引入的续传处理逻辑，专门支持新的简化声明式续传模型。
...
- 关键点：
  ...
  - 工具默认值：rdc-tool CLI、代码解释器、Shell 工具默认启用状态与超时等。

**更新** 移除了 RDC Actions 相关的默认配置，简化了工具默认值结构。
...
**新增** 这是新引入的类型定义，专门支持新的简化声明式续传模型的所有字段。
...
- 渲染层依赖：
  ...
  - RdcCliInvokerSettingsFields 依赖 validateRdcInstallation 进行安装验证。
  - AgentHandoffCard 依赖 agentHandoffValidation 进行配置验证。
- 主进程依赖：
...
**更新** 新增了 Agent Handoff 配置验证功能的依赖关系，移除了 RdcActionsFields 相关的依赖。
...
SS --> STY["settings.ts"]
RSF["RdcCliInvokerSettingsFields.tsx"] --> VI["validateRdcInstallation"]
VI --> EA["Electron API"]
AHC["AgentHandoffCard.tsx"] --> AV["agentHandoffValidation.ts"]
AV --> AMT["agentManifest.ts"]
AMP["agentManifestParse.ts"] --> AMT
ADH["applyDeclaredHandoff.ts"] --> AMT
```
...
- **新增** rdc-tool CLI 安装验证的缓存机制：避免重复验证相同的配置。
- **新增** 验证结果的本地缓存：减少网络请求和系统调用次数。
- **新增** 条件验证：只在配置发生变化时才触发验证，提升响应速度。
- **新增** Agent Handoff 配置的增量验证：只对变化的字段进行重新验证，减少不必要的计算。
- **新增** 持久化数据的智能清理：自动移除未使用的字段，减少存储空间占用。

**更新** 增加了针对 rdc-tool CLI 安装验证和 Agent Handoff 配置验证的性能优化考虑。
...
- 密钥泄露风险：
  ...
  - 参考：[src/main/settings/README.md:42-50](file://src/main/settings/README.md#L42-L50)
- **新增** Agent Handoff 配置验证失败：
  - 现象：保存 Agent Handoff 配置时报错或配置未生效。
  - 排查：检查 label、agent、prompt 是否为空；验证 model 是否为有效选项；确认 requiredSkillIds 格式正确。
  - 参考：[src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts:60-84](file://src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts#L60-L84)
- **新增** Handoff 配置持久化问题：
  - 现象：保存的 Handoff 配置丢失或字段不正确。
  - 排查：检查 persistAgentHandoff 函数的清理逻辑；确认 send:false 被正确省略；验证 showContinueOn 的布尔值处理。
  - 参考：[src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts:35-50](file://src/renderer/features/settings/SettingsModal/sections/agentHandoffValidation.ts#L35-L50)
- **新增** rdc-tool CLI 安装验证失败：
  - 现象：点击验证按钮后显示错误或无法验证。
  - 排查：检查 rdc-tool CLI 命令路径是否正确；确认命令行参数配置；查看 Electron API 是否可用。
  - 参考：[src/renderer/platform/rdcInstallation.ts:5-14](file://src/renderer/platform/rdcInstallation.ts#L5-L14)
- **新增** 生命周期模板配置相关问题：
  - 现象：找不到生命周期模板配置选项。
...
**更新** 增加了 Agent Handoff 配置验证和持久化相关的故障排除指南。
...
## 结论
RDC-Agent 设置面板经过重大简化重构后，采用了更加精简和高效的设计。通过移除 RdcActionsFields 组件和生命周期模板配置功能，显著降低了设置界面的复杂度。新增的 rdc-tool CLI 安装实时验证功能和增强的 Agent Handoff 配置验证机制，为用户提供了更加直观和可靠的配置体验。特别是新的简化声明式续传模型支持，包括 send、showContinueOn、model 和 requiredSkillIds 字段的完整验证流程，大大提升了配置的准确性和用户体验。建议在新增设置项时遵循现有模式：使用 SettingsPageContent 进行统一渲染，定义清晰的类型和默认值，实现主进程写入与清理逻辑，并在渲染层提供对应的表单与保存流程。

**更新** 本次简化重构和增强的验证机制显著提升了设置面板的可维护性和用户体验，为未来的功能扩展奠定了更加稳固的基础。
...
- Agent 清单（agents）
  ...
  - **新增** 增强的 Agent Handoff 配置验证，支持 send、showContinueOn、model 和 requiredSkillIds 字段。
- 技能（skills）
  - 资源范围切换（user/project），技能列表与编辑。
  - 使用RuntimeScopePanel提供统一的管理界面。
- 工具（tools）
  - rdc-tool CLI、代码解释器、Shell 工具配置与保存。
  - 包含MCP服务和信任管理。
  - **新增** rdc-tool CLI 安装实时验证功能。
- 钩子（hooks）
  - 资源范围切换，钩子信任与配置。
  - 使用RuntimeScopePanel提供统一的管理界面。
- 策略（policy）
...
**更新** 移除了 RDC Actions 和生命周期模板配置相关的说明，新增了 rdc-tool CLI 安装验证和增强的 Agent Handoff 配置验证功能的描述。

### 设置持久化与迁移
- 持久化位置：主进程读取/写入 JSON 设置文件。
- 迁移机制：schemaVersion 断言与重建，必要时清理废弃 secret 引用。
- 默认值合并：写入时合并默认外观、布局、个人资料、工具与 Agent 运行时设置。
- **新增** Agent Handoff 配置的智能持久化：自动清理未使用的字段，如 send:false 会被省略。
- **更新** 移除了 RDC Actions 相关的默认配置和迁移逻辑。
...
- 配置工具：启用 rdc-tool CLI/代码解释器/Shell，设置命令与超时。
- **新增** 验证 rdc-tool CLI 安装：配置 rdc-tool CLI 命令路径，点击验证按钮检查安装状态。
- **新增** 配置 Agent Handoff：设置目标 Agent、标签、提示词，可选配置 send、showContinueOn、model 和 requiredSkillIds。
- 配置 Agent：编辑清单草稿，自动保存，处理 handoff 冲突。
- 管理 MCP 服务：配置 MCP 服务器，设置信任状态，监控连接状态。
- 查看 Provider 连接摘要：查看协议信息和连接状态。

**更新** 移除了生命周期模板配置相关的场景，新增了 rdc-tool CLI 安装验证和增强的 Agent Handoff 配置的场景。
...
- **新增** Agent Handoff 配置验证问题：检查必需字段、模型选项和技能ID格式。
- **新增** rdc-tool CLI 安装验证问题：检查命令路径、参数配置和 Electron API 可用性。
- **新增** 生命周期模板配置问题：确认该功能已被移除，检查相关组件引用。
- MCP 服务面板问题：检查 useMcpStatus Hook 和连接状态。
- Provider 连接摘要问题：检查连接阶段计算和协议信息。

**更新** 增加了 Agent Handoff 配置验证和 rdc-tool CLI 安装验证相关的故障排除要点。
