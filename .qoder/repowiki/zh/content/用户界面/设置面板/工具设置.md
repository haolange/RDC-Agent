...
- 工具配置界面与数据模型：代码解释器、rdc-tool CLI 调用器、文件系统（工作目录与环境变量）等。
- 每个工具的可用选项、安全限制与执行环境。
- 工具权限模型与访问控制机制。
- 工具执行日志查看与性能监控要点。
- 最佳实践与常见问题排查。
...
- 主进程工具执行层：rdc-tool CLI 调用器与服务化 Shell 调用封装。
...
```mermaid
graph TB
UI["设置界面<br/>ToolsSettings / RdcCliInvokerSettingsFields / CodeInterpreterSettingsFields / ShellSettingsFields"] --> Settings["设置服务<br/>settingsSanitize / settingsDefaults"]
UI --> RuntimeMeta["运行时代码摘要<br/>RdcCliInvokerService.getRuntimeSummary"]
Settings --> Tooling["工具配置对象<br/>tooling.rdc-agentCli / tooling.codeInterpreter / tooling.shell"]
Tooling --> Invoker["rdc-tool CLI 调用器<br/>RdcCliInvokerService"]
...
## 核心组件
- rdc-tool CLI 调用器服务：负责加载工具清单、组装参数、执行命令、解析结果、生成追踪记录。
...
participant I as "rdc-tool CLI 调用器"
...
图表来源
- [RdcCliInvokerService.ts:178-355](file://src/main/tools/RdcCliInvokerService.ts#L178-L355)
- [ShellInvocationService.ts:32-105](file://src/main/tools/ShellInvocationService.ts#L32-L105)
- [ProcessSupervisor.ts](file://src/main/runtime/ProcessSupervisor.ts)

## 详细组件分析

### rdc-tool CLI 调用器（RdcCliInvokerService）
- 功能职责
...
图表来源
- [RdcCliInvokerService.ts:49-141](file://src/main/tools/RdcCliInvokerService.ts#L49-L141)
- [RdcCliInvokerService.ts:178-355](file://src/main/tools/RdcCliInvokerService.ts#L178-L355)
...
- 工具配置清洗
  - rdc-tool CLI 调用器：enabled/command/argsPrefix/workingDirectory/env/timeoutMs/catalogPath/jsonMode。
...
- 工具设置入口与分组
  ...
  - RdcCliInvokerSettingsFields：rdc-tool CLI 调用器的表单字段。
...
## 依赖关系分析
- 模块耦合
  - RdcCliInvokerService 依赖 ShellInvocationService 与设置服务；
...
A --> E["RdcNativeProtocol"]
A --> F["resolveRdcBatchInvocation"]
```

图表来源
- [RdcCliInvokerService.ts:1-18](file://src/main/tools/RdcCliInvokerService.ts#L1-L18)
...
章节来源
- [RdcCliInvokerService.ts:70-90](file://src/main/tools/RdcCliInvokerService.ts#L70-L90)
- [RdcCliInvokerService.ts:178-355](file://src/main/tools/RdcCliInvokerService.ts#L178-L355)
- [ShellInvocationService.ts:32-105](file://src/main/tools/ShellInvocationService.ts#L32-L105)
...
- [RdcCliInvokerService.ts:248-281](file://src/main/tools/RdcCliInvokerService.ts#L248-L281)
...
章节来源
- [RdcCliInvokerService.ts:223-246](file://src/main/tools/RdcCliInvokerService.ts#L223-L246)
