...
## 更新摘要
**变更内容**
- 新增了计划审查面板功能，支持用户交互式的计划审批和拒绝操作
- 集成了手递选项选择，允许用户在批准计划时选择不同的执行代理
- 增强了待处理请求门控机制，优先显示工具审批、计划审查和用户输入请求
- 改进了标签管理：移除了复杂的模式切换逻辑，采用更简洁的标签组件
...
**最新更新**：Composer组件已进行重大改进，包括新增的计划审查面板功能、简化的标签管理、改进的焦点处理、编辑和预览模式的一致样式，以及限制输入焦点白框处理范围。
...
A --> L["PlanReviewRequestPanel.tsx<br/>计划审查面板"]
B --> M["useComposerPendingGate.tsx<br/>待处理请求门控"]
M --> N["usePlanReviewSubmit.ts<br/>计划审查提交"]
```
...
## 架构总览
下图展示了从用户输入到消息落库的关键路径，包括斜杠命令分流、乐观更新、错误恢复与资源释放，以及新增的计划审查流程。
...
- [usePlanReviewSubmit.ts:5-23](file://src/renderer/features/composer/usePlanReviewSubmit.ts#L5-L23)
- [ComposerMarkdownInput.css:78-81](file://src/renderer/features/composer/composer-chrome-1.css#L78-L81)
...
**最新更新**：最近的改进包括新增的计划审查面板功能、简化的标签管理、改进的焦点处理、编辑和预览模式的一致样式，以及限制输入焦点白框处理范围，进一步提升了用户体验和代码可维护性。计划审查面板为用户提供了交互式的工作流控制能力，增强了系统的灵活性和可控性。
