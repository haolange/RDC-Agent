...
- [userDataPath.ts](file://src/main/runtime/userDataPath.ts)
- [userDataInstanceLock.ts](file://src/main/runtime/userDataInstanceLock.ts)
</cite>

## 更新摘要
**变更内容**   
- 新增启动身份诊断系统章节，详细说明版本、部署载体类型、应用路径、用户数据目录和用户资源位置信息
- 增强安装问题排查部分，包含新的启动诊断信息使用方法
- 更新运行时错误排查，增加启动阶段问题的诊断方法
- 扩展调试技巧，包含启动身份信息的收集和分析
...
**更新** 新增了启动身份诊断系统，提供全面的启动信息包括版本、部署载体类型、应用路径、用户数据目录和用户资源位置，用于排查部署问题。

## 项目结构
RDC-Agent 的故障排除主要围绕以下关键路径：
- 启动器与构建流程：scripts/launch-rdc-agent.mjs
- 主进程入口与启动诊断：src/main/index.ts
...
- 应用路径服务：src/main/runtime/AppPathService.ts
- 用户数据路径解析：src/main/runtime/userDataPath.ts
- 用户数据实例锁：src/main/runtime/userDataInstanceLock.ts

```mermaid
graph TB
A["启动器<br/>scripts/launch-rdc-agent.mjs"] --> B["Electron 主进程<br/>src/main/index.ts"]
...
B --> H["启动身份诊断<br/>版本/载体/路径信息"]
I["渲染端设置API<br/>settings.ts"] --> J["默认设置<br/>defaultAppSettings.ts"]
K["跟踪类型定义<br/>agenticTrace/base.ts"] -.-> B
L["应用路径服务<br/>AppPathService.ts"] --> H
M["用户数据路径<br/>userDataPath.ts"] --> H
N["实例锁机制<br/>userDataInstanceLock.ts"] --> H
```

**图表来源**
- [launch-rdc-agent.mjs:387-530](file://scripts/launch-rdc-agent.mjs#L387-L530)
- [index.ts:202-208](file://src/main/index.ts#L202-L208)
...
- [AppPathService.ts:78-80](file://src/main/runtime/AppPathService.ts#L78-L80)
- [userDataPath.ts:3-8](file://src/main/runtime/userDataPath.ts#L3-L8)
- [userDataInstanceLock.ts:38-77](file://src/main/runtime/userDataInstanceLock.ts#L38-L77)

## 核心组件
- 启动器与构建：负责 Node 版本校验、pnpm 依赖同步、Electron 二进制恢复、应用构建与模式切换（桌面/浏览器/开发）。
- **启动身份诊断**：在应用启动时输出关键身份信息，包括版本号、部署载体类型（打包桌面/源码桌面/浏览器）、应用路径、用户数据目录和用户资源位置。
...
## 架构总览
下图展示了从启动到运行时的关键交互，包括启动身份诊断、日志、错误分类、Shell 执行与能力探测。
...
participant SD as "启动诊断<br/>Startup Identity"
...
### 启动身份诊断系统（新增）
启动身份诊断系统在应用启动时输出关键的环境和部署信息，用于快速识别问题环境。

**启动信息包括：**
- **version**: 应用版本号，用于确认当前运行的版本
- **carrier**: 部署载体类型
  - `packaged-desktop`: 打包的桌面应用
  - `source-desktop`: 源码开发的桌面应用  
  - `browser`: 无头浏览器模式
- **application**: 应用程序路径，显示应用的物理位置
- **userData**: 用户数据目录路径，存储应用配置和状态
- **userResources**: 用户资源根目录，包含自定义配置和扩展

**部署载体检测逻辑：**
- 检查 `RDC_AGENT_HEADLESS` 环境变量判断是否为浏览器模式
- 使用 `app.isPackaged` 区分打包和源码版本
- 自动设置相应的运行模式和权限

**用户数据管理：**
- 支持显式指定用户数据路径（`RDC_AGENT_USER_DATA`）
- 浏览器 QA 模式使用临时隔离的用户数据目录
- 实现实例锁定防止多实例冲突

```mermaid
flowchart TD
Start(["应用启动"]) --> DetectEnv{"检测运行环境"}
DetectEnv --> Headless{"headless模式?"}
Headless --> |是| BrowserCarrier["carrier=browser"]
Headless --> |否| Packaged{"打包应用?"}
Packaged --> |是| DesktopCarrier["carrier=packaged-desktop"]
Packaged --> |否| SourceCarrier["carrier=source-desktop"]
BrowserCarrier --> CollectInfo["收集启动信息"]
DesktopCarrier --> CollectInfo
SourceCarrier --> CollectInfo
CollectInfo --> LogInfo["输出JSON格式启动信息"]
LogInfo --> End(["启动完成"])
```

**图表来源**
- [index.ts:118-125](file://src/main/index.ts#L118-L125)
- [index.ts:183-208](file://src/main/index.ts#L183-L208)
...
- **启动身份诊断**依赖应用路径服务和用户数据路径解析，提供统一的启动信息。
...
Main["主进程"] --> Startup["启动诊断"]
Main --> Log["运行时日志"]
Main --> Err["错误分类器"]
Main --> Shell["Shell 工具"]
Main --> Probe["能力探测"]
Startup --> AppPath["应用路径服务"]
Startup --> UserData["用户数据路径"]
Probe --> Settings["设置服务"]
Log --> Renderer["渲染端"]
```
...
- **启动诊断优化**：启动身份信息仅在启动时输出一次，避免频繁 I/O 操作。

## 故障排除指南

### 启动身份诊断信息收集（新增）
启动时会输出 JSON 格式的启动身份信息，包含以下关键字段：
...
```json
{
  ...
  "application": "应用程序路径", 
  "userData": "用户数据目录路径",
  "userResources": "用户资源根路径"
}
```

**部署载体类型说明：**
- `packaged-desktop`: 打包的桌面应用，从安装包运行
- `source-desktop`: 源码开发的桌面应用，从源代码运行
- `browser`: 无头浏览器模式，用于自动化测试和 QA

**常见启动问题诊断：**
1. **版本不匹配**：检查 `version` 字段确认当前运行版本
2. **部署环境问题**：检查 `carrier` 字段确认运行模式是否符合预期
3. **路径配置问题**：检查 `application`、`userData`、`userResources` 路径是否正确
4. **多实例冲突**：查看是否有实例锁定错误信息
...
### 启动与运行问题
- **启动身份诊断信息异常**
  - 现象：启动时输出的启动信息不完整或格式错误。
  - 诊断：检查控制台输出的 `[RDC-Agent] Startup identity:` 日志。
  - 修复：确认环境变量设置正确，检查应用路径和服务初始化。
  - 参考
    - [index.ts:202-208](file://src/main/index.ts#L202-L208)

- **用户数据目录冲突**
  - 现象：提示 userData is already in use 错误。
  - 诊断：检查 instance.lock 文件和现有进程信息。
  - 修复：终止冲突进程或删除锁文件后重启。
  - 参考
    - [index.ts:215-221](file://src/main/index.ts#L215-L221)
    - [userDataInstanceLock.ts:38-77](file://src/main/runtime/userDataInstanceLock.ts#L38-L77)
...
### 调试技巧
- **使用启动身份诊断**
  - 启动应用时查看控制台输出的 `[RDC-Agent] Startup identity:` 日志。
  - 记录完整的启动信息用于问题报告和版本追踪。
  - 确认部署载体类型符合预期环境。
  - 参考
    - [index.ts:202-208](file://src/main/index.ts#L202-L208)
...
### 问题定位方法
- **收集启动身份信息**：首先记录启动时输出的完整启动信息，包含版本、载体、路径等关键信息。
...
### 社区支持与获取帮助
- **内部渠道**：使用崩溃上报收集生产错误，结合启动身份信息、运行时日志与错误分类器提供完整上下文。
- **团队支持**：提交问题时附上启动身份信息、启动模式、Node/pnpm/Electron 版本、相关日志与错误分类结果。
- **文档与契约**：参考模块映射与设置/提供者契约，确保配置符合预期。

## 结论
通过启动器、启动身份诊断、运行时日志、错误分类器、Shell 工具、能力探测与崩溃上报的组合，RDC-Agent 提供了完整的安装、运行与排障能力。新增的启动身份诊断系统能够快速识别部署环境和版本信息，显著提升问题定位效率。遵循本指南的步骤与最佳实践，可快速定位并解决常见问题，提升稳定性与用户体验。
