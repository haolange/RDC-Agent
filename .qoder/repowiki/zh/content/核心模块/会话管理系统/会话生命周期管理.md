...
## 简介
本文件系统性说明 RDC-Agent 主进程中的"会话生命周期管理"，围绕 RdxSessionService 的会话创建、激活、切换与销毁流程展开。文档重点介绍了新的直接原生协议调用机制，替代了原有的shell动作方式，改进了设备激活流程和连接状态处理，同时移除了生命周期模板相关功能。**最新更新**：RdxSessionRuntime 类现在集成了守护进程所有权跟踪机制，通过 OwnedRdxDaemonRegistry 在会话生命周期中维护准确的拥有记录，确保原生进程的可靠管理和资源清理。覆盖 openProjectInput、closeOrReplaceOpenedCapture、switchActiveCapture 等关键方法，以及捕获文件（.rdc）打开过程、会话状态转换机制、会话上下文与运行时环境配置等功能。

## 项目结构
RDX 会话生命周期由以下模块协作完成：
- 会话服务：RdxSessionService 负责会话级状态管理、操作协调和状态广播
- 会话运行时：RdxSessionRuntime 封装具体的原生协议调用、会话生命周期管理和守护进程所有权跟踪
- 守护进程注册表：OwnedRdxDaemonRegistry 维护应用拥有的原生守护进程记录，支持持久化和恢复
...
Runtime --> DaemonReg["OwnedRdxDaemonRegistry<br/>守护进程所有权跟踪"]
...
- [OwnedRdxDaemonRegistry.ts:16-24](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L16-L24)
...
- **RdxSessionRuntime**：会话运行时，封装原生协议调用、设备连接管理、会话生命周期控制和守护进程所有权跟踪
- **OwnedRdxDaemonRegistry**：守护进程注册表，维护应用拥有的原生守护进程记录，支持持久化存储、进程恢复和资源清理
...
participant DaemonReg as "OwnedRdxDaemonRegistry"
...
Runtime->>DaemonReg : "rememberOwnedRdxDaemon(record)"
alt 远程回放
...
- [OwnedRdxDaemonRegistry.ts:65-72](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L65-L72)
...
### RdxSessionRuntime 原生协议调用与守护进程所有权跟踪
...
- **守护进程所有权管理（ensureOwnedDaemon）**
  ...
  - 记录保存：通过 rememberOwnedRdxDaemon 保存守护进程信息到持久化存储
...
- **守护进程清理（teardownRuntime）**
  ...
  - **所有权移除**：调用 forgetOwnedRdxDaemon 从注册表中移除记录
  - 状态重置：清空所有运行时状态
...
- [RdxSessionRuntime.ts:427-469](file://src/main/sessions/RdxSessionRuntime.ts#L427-L469)
...
### OwnedRdxDaemonRegistry 守护进程所有权跟踪
**新增** 守护进程所有权跟踪系统，确保原生进程的可靠管理和资源清理

- **守护进程记录管理**
  - 记录创建：rememberOwnedRdxDaemon 保存守护进程信息到持久化存储
  - 记录删除：forgetOwnedRdxDaemon 从注册表中移除守护进程记录
  - 记录查询：listOwnedRdxDaemons 获取所有拥有的守护进程列表
  - 上下文验证：isAppRdxContextId 验证上下文ID格式

- **守护进程恢复与清理（harvestOwnedRdxDaemons）**
...
- **持久化存储**
  - 文件格式：JSON 格式的 owned-rdx-daemons.json 文件
...
stateDiagram-v2
[*] --> owned
owned --> released : harvestOwnedRdxDaemons()
...
**图表来源**
- [OwnedRdxDaemonRegistry.ts:65-79](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L65-L79)
...
- [RdxSessionRuntime.ts:271-300](file://src/main/sessions/RdxSessionRuntime.ts#L271-300)
...
Runtime --> DaemonReg["OwnedRdxDaemonRegistry"]
...
- [OwnedRdxDaemonRegistry.ts:1-10](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L1-L10)
...
- **守护进程持久化**：OwnedRdxDaemonRegistry使用文件存储，支持进程恢复和资源清理
...
- **守护进程所有权问题**
  - 现象：RDX_CLOSE_FAILED: daemon start did not bind the application owner_pid
  - 处理：检查守护进程启动参数，确保owner_pid正确绑定到当前进程
  - 现象：守护进程残留无法清理
  - 处理：使用harvestOwnedRdxDaemons进行恢复清理，检查PID验证逻辑

**章节来源**
- [RdxNativeProtocol.ts:12-42](file://src/main/tools/RdxNativeProtocol.ts#L12-L42)
- [RdxSessionRuntime.ts:165-173](file://src/main/sessions/RdxSessionRuntime.ts#L165-L173)
- [RdxSessionRuntime.ts:255-269](file://src/main/sessions/RdxSessionRuntime.ts#L255-269)
- [RdxSessionRuntime.ts:483-493](file://src/main/sessions/RdxSessionRuntime.ts#L483-L493)
- [OwnedRdxDaemonRegistry.ts:138-203](file://src/main/sessions/OwnedRdxDaemonRegistry.ts#L138-L203)
- [RdxRuntimeContextRegistry.ts:164-222](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L164-L222)

## 结论
新的会话生命周期管理架构通过直接原生协议调用替代了shell动作中间层，提供了更高效的会话管理能力。**最新改进**：RdxSessionRuntime 类集成了守护进程所有权跟踪机制，通过 OwnedRdxDaemonRegistry 在会话生命周期中维护准确的拥有记录，确保原生进程的可靠管理和资源清理。RdxSessionService专注于协调和状态管理，RdxSessionRuntime处理具体的原生协议调用、设备连接和守护进程管理。这种分离设计提高了代码的可维护性和性能，同时保持了强大的会话隔离和委派机制。配合改进的设备激活流程和连接状态处理，系统能够更可靠地管理本地和远程回放设备的会话生命周期，并通过守护进程所有权跟踪确保资源的正确释放。
...
### OwnedRdxDaemonRegistry API
**新增** 守护进程所有权跟踪API

- **rememberOwnedRdxDaemon(record, options)**
  - 作用：记录应用拥有的守护进程信息
  - 参数：record（守护进程记录）、options（存储选项）

- **forgetOwnedRdxDaemon(contextId, options)**
  - 作用：从注册表中移除守护进程记录
  - 参数：contextId（上下文ID）、options（存储选项）

- **harvestOwnedRdxDaemons(options)**
  - 作用：恢复和清理遗留的守护进程
  - 参数：options（恢复选项）
...
- **守护进程所有权管理**
  ...
  const result = await harvestOwnedRdxDaemons({
    intermediateRoot: settings.tooling.rdxCli.env[RDX_INTERMEDIATE_ROOT_ENV]
  });
  ```
...
- [RdxSessionRuntime.ts:82-198](file://src/main/sessions/RdxSessionRuntime.ts#L82-L198)
- [OwnedRdxDaemonRegistry.test.ts:52-81](file://src/main/sessions/OwnedRdxDaemonRegistry.test.ts#L52-L81)