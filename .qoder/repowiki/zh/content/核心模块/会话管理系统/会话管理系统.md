...
## 更新摘要
**变更内容**
- 重大架构调整：用声明式续跑模型替换了复杂的状态机交接系统
- 移除了 HandoffStateStore、agent_handoff 工具和自动发送/强制返回 Mission 的逻辑
- 新增 ExecutionOfferStore 用于存储执行提议，仅驱动建议行和计划门按钮
- 通过 applyDeclaredHandoff 函数实现用户驱动的 Agent 切换
- 增强了计划审查状态与执行提议的集成，确保 Skill 预载的安全性
...
- shared/types：跨进程共享的类型定义（会话、对话、分支、执行提议、计划审查等）
...
subgraph "外部依赖"
I["AgentOrchestrator<br/>任务编排"]
J["TraceService<br/>追踪发布"]
K["ReplayDeviceService<br/>回放设备"]
L["SettingsService<br/>设置服务"]
M["StorageIo<br/>存储IO"]
end
A --> C
B --> C
...
- **新架构** ExecutionOfferStore 依赖 storageHost 提供的会话路径查找与 IO 能力，依赖 executionOfferMatches 的状态匹配校验。
...
## 结论
新的会话管理系统采用了声明式交接模型，通过 applyDeclaredHandoff 和 ExecutionOfferStore 实现了更安全、更可控的 Agent 切换机制。**重大架构调整** 移除了复杂的 durable state machine，简化了交接流程，只保留用户驱动的交互模式。系统通过幂等性、原子写入、增量更新、缓存与迁移机制，实现了高可靠、高性能的会话管理能力。建议在应用中遵循其 API 约定，合理使用分支与交接，结合新的计划审查功能，以获得最佳体验与稳定性。
