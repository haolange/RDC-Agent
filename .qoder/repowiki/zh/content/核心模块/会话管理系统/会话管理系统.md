...
## 更新摘要
**变更内容**
- **设备呈现状态增强**：改进了RdxSessionService的设备呈现状态处理，增强了devicePresentation字段的状态管理和验证
...
I["RdxReplayObservation<br/>回放观察"]
J["RdxRuntimeContextRegistry<br/>上下文注册表"]
end
subgraph "知识管理层"
...
- [RdxReplayObservation.ts:36-108](file://src/main/sessions/RdxReplayObservation.ts#L36-L108)
...
- **新增** RdxReplayObservation：增强设备呈现状态跟踪，支持Android原生事件和资源序列号验证。
- **新增** RdxRuntimeContextRegistry：提供会话隔离机制，在检测到上下文问题时自动标记会话为隔离状态。
...
- **新增** 设备呈现状态通过 RdxReplayObservation 进行跟踪，支持Android原生事件和资源序列号验证。
- **新增** 会话隔离机制通过 RdxRuntimeContextRegistry 实现，在检测到上下文问题时自动隔离会话。
...
participant RRO as "RdxReplayObservation"
participant RRCR as "RdxRuntimeContextRegistry"
...
RS->>RRCR : getRdxContextLease(sessionId)
...
- [RdxReplayObservation.ts:36-108](file://src/main/sessions/RdxReplayObservation.ts#L36-L108)
- [RdxRuntimeContextRegistry.ts:262-267](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L262-L267)
...
### RdxRuntimeContextRegistry：会话隔离机制
职责要点
- **新增** 隔离支持：在检测到上下文问题时自动标记会话为隔离状态
- **新增** 上下文租约管理：管理会话的RDX上下文租约，支持父会话和子会话的委托
...
- **新增** RdxReplayObservation 依赖 RdxCliInvokerService 执行CLI命令，依赖 ReplayLivePreviewStore 存储实时预览。
- **新增** RdxRuntimeContextRegistry 提供会话隔离功能，被RdxSessionService和其他组件用于检查上下文状态。
...
RS --> RRO["RdxReplayObservation"]
RS --> RRCR["RdxRuntimeContextRegistry"]
...
- [RdxReplayObservation.ts:1-11](file://src/main/sessions/RdxReplayObservation.ts#L1-L11)
- [RdxRuntimeContextRegistry.ts:1-26](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L1-L26)
...
- **新增** 设备呈现状态缓存：RdxSessionService 缓存设备呈现状态，减少重复的状态查询。
- **新增** 隔离状态快速检查：RdxRuntimeContextRegistry 提供快速的隔离状态检查，避免不必要的上下文操作。
- **新增** Android事件优化：RdxReplayObservation 优化了Android原生事件的验证和处理，减少网络往返。
...
- **新增** 会话隔离问题：检查RdxRuntimeContextRegistry中的quarantineReason，确认会话是否被正确隔离。
- **新增** Android事件验证失败：检查RdxReplayObservation中的事件ID、纹理ID和序列号验证结果。
...
- [KnowledgeImportService.ts:167-186](file://src/main/knowledge/KnowledgeImportService.ts#L167-L186)
- [KnowledgeWriteService.ts:73-113](file://src/main/knowledge/KnowledgeWriteService.ts#L73-L113)
- [RdxReplayObservation.ts:64-74](file://src/main/sessions/RdxReplayObservation.ts#L64-L74)
- [RdxRuntimeContextRegistry.ts:262-267](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L262-L267)
...
- [RdxSessionService.ts:239-266](file://src/main/sessions/RdxSessionService.ts#L239-L266)
...
**章节来源**
- [RdxReplayObservation.ts:27-34](file://src/main/sessions/RdxReplayObservation.ts#L27-L34)
- [RdxReplayObservation.ts:36-108](file://src/main/sessions/RdxReplayObservation.ts#L36-L108)

### RdxRuntimeContextRegistry 关键 API
- setRdxRuntimeContextForSession(sessionId, runtimeContext, options)：设置会话的RDX上下文租约
- getRdxContextLease(sessionId)：获取会话的上下文租约
- assertRdxContextLeaseOwnership(input)：验证会话对上下文的拥有权
- grantDelegatedLease(input)：授予子会话委托租约
- revokeDelegatedLease(childSessionId, options)：撤销子会话的委托租约
- quarantineRdxContext(sessionId, expectedVersion, reason)：隔离会话上下文

使用示例（概念性步骤）
- 在会话打开捕获时调用 setRdxRuntimeContextForSession 设置上下文
- 在执行操作前调用 assertRdxContextLeaseOwnership 验证拥有权
- 在检测到上下文问题时调用 quarantineRdxContext 隔离会话
- 在问题解决后通过重新设置上下文解除隔离

**章节来源**
- [RdxRuntimeContextRegistry.ts:61-101](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L61-L101)
- [RdxRuntimeContextRegistry.ts:103-111](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L103-L111)
...
- [RdxRuntimeContextRegistry.ts:225-242](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L225-L242)
- [RdxRuntimeContextRegistry.ts:262-267](file://src/main/sessions/RdxRuntimeContextRegistry.ts#L262-L267)
