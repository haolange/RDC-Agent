...
## 更新摘要
**所做更改**
- 将所有 RDX 引用更新为 RDC，包括类名、文件路径和用户可见字符串
- 更新所有文件引用以反映从 OwnedRdxDaemonRegistry 到 OwnedRdcDaemonRegistry 的重命名
- 更新所有代码示例和架构图以使用新的 RDC 命名约定
- 保持文档结构一致，同时反映品牌重塑变更
...
**更新** 已将所有 RDX 相关术语更新为 RDC，包括类名、文件路径和用户界面字符串。
...
- 运行时 RdcSessionRuntime 负责与外部 RDC CLI 交互，启动/停止守护进程，维护上下文状态。
...
- 执行层：RdcSessionRuntime 与外部 RDC CLI 交互，启动/停止守护进程，维护上下文快照。
...
participant CLI as "外部 RDC CLI/Daemon"
UI->>Svc : 打开捕获
...
- 错误处理
  - 非法上下文 ID 抛出 RDC_OWNERSHIP_DENIED。
  - 文件损坏或版本不兼容抛出 RDC_OWNERSHIP_CORRUPT / STORAGE_SCHEMA_UNSUPPORTED。
...
**章节来源**
- [RdcProbeTool.ts:64-83](file://src/main/workflow/debugger/RdcProbeTool.ts#L64-83)

## 依赖关系分析
- 组件耦合
  - RdcSessionService 依赖 RdcSessionRuntime、RdcRuntimeContextRegistry、OwnedRdcDaemonRegistry。
  - RdcSessionRuntime 依赖 OwnedRdcDaemonRegistry、RdcRuntimeContextRegistry、外部 RDC CLI。
  - RdcProbeTool 依赖 RdcRuntimeContextRegistry 进行所有权校验。
- 外部依赖
  - 外部 RDC CLI/Daemon：提供守护进程管理与捕获回放能力。
...
**更新** 已完成从 RDX 到 RDC 的品牌重塑，所有相关术语、类名、文件路径和用户界面字符串均已更新。