...
- 会话服务：sessions/RdcSessionService.ts 维护每个会话的回放绑定、生命周期、事件应用与观察结果发布。
- 运行时：sessions/RdcSessionRuntime.ts 封装与外部 rdc-tool CLI/Shell Action 的交互，处理本地/远端打开、关闭、上下文环境注入与错误诊断。
- 观察与事件：sessions/RdcReplayObservation.ts 通过 rdc-tool CLI 调用 rd.session.observe 获取帧图像与元数据，并校验一致性。
...
```mermaid
graph TB
A["ReplayDeviceService<br/>设备发现/激活/广播"] --> B["RdcSessionRuntime<br/>打开/关闭/上下文注入"]
B --> C["RdcSessionService<br/>会话绑定/状态发布"]
C --> D["RdcReplayObservation<br/>rd.session.observe"]
...
- [RdcSessionRuntime.ts:51-525](file://src/main/sessions/RdcSessionRuntime.ts#L51-L525)
- [RdcSessionService.ts:22-271](file://src/main/sessions/RdcSessionService.ts#L22-L271)
...
- RdcSessionRuntime：封装 open/close 生命周期，注入 RDC 上下文环境变量，处理本地/远端打开差异与错误诊断。
- RdcSessionService：会话级编排，保证串行操作、互斥锁、设备占用、事件应用与观察结果的发布与持久化。
- RdcReplayObservation：调用 rdc-tool CLI 的 observe/get_replay_events，严格校验返回结构与图像路径一致性。
...
participant Obs as "RdcReplayObservation"
...
**图示来源**
- [RdcSessionService.ts:83-131](file://src/main/sessions/RdcSessionService.ts#L83-L131)
- [RdcSessionRuntime.ts:65-211](file://src/main/sessions/RdcSessionRuntime.ts#L65-L211)
- [ReplayDeviceService.ts:214-278](file://src/main/captures/ReplayDeviceService.ts#L214-L278)
- [RdcReplayObservation.ts:21-88](file://src/main/sessions/RdcReplayObservation.ts#L21-L88)
...
- 远端激活：通过 rdcShellActionService.runAction('connectRemote') 建立远端上下文，产出 remoteId/contextId，并缓存 PreparedRemoteSurface 用于后续快速打开。
...
### RdcSessionRuntime：会话运行时与上下文管理
- 打开流程：根据设备类型选择本地或远端 openCapture/openRemoteCapture，注入 RDC_TOOL_CONTEXT_ID 等环境变量，解析返回的 runtimeContext。
...
+getCliSettings() RdcCliInvokerSettings
+getContextId() string
}
```
...
**图示来源**
- [RdcSessionService.ts:83-131](file://src/main/sessions/RdcSessionService.ts#L83-L131)
- [RdcSessionService.ts:209-268](file://src/main/sessions/RdcSessionService.ts#L209-L268)
...
### RdcReplayObservation：观察与图像获取
- 调用方式：通过 rdcCliInvokerService.executeCLI('call', 'rd.session.observe'/'rd.session.get_replay_events') 与指定 daemon-context 通信。
...
- RdcSessionRuntime 依赖：设置服务、rdc-tool CLI 调用、Shell Action 服务、会话上下文注册、设备服务。
- RdcSessionService 依赖：存储服务、历史存储、运行时、观察服务、操作协调器。
- ReplayHistoryStore 依赖：路径服务、安全写入、PNG 编解码、配额与树大小统计。
- TraceService 依赖：存储适配器、工作流状态、事件存储、投影构建器、右侧面板服务。

```mermaid
graph LR
Dev["ReplayDeviceService"] --> RT["RdcSessionRuntime"]
RT --> Svc["RdcSessionService"]
Svc --> Obs["RdcReplayObservation"]
...
- 打开失败：RdcSessionRuntime 会格式化诊断信息；LOCAL_REPLAY_UNSUPPORTED 表示本地回放不受支持。
- 观察失败：RdcReplayObservation 会校验 metadata/image_path；错误码如 no_color_output/missing_target 决定重试策略。
- 历史写入失败：ReplayHistoryStore 在配额不足或写入异常时抛出错误；可通过 clearCapture/clearSession 释放空间。
- 上下文锁定：RdcSessionService 在 native 进程退出未确认或代理执行期间会阻止交互；需等待或停止相关会话。

**章节来源**
- [ReplayDeviceService.ts:457-507](file://src/main/captures/ReplayDeviceService.ts#L457-L507)
- [RdcSessionRuntime.ts:159-188](file://src/main/sessions/RdcSessionRuntime.ts#L159-L188)
- [RdcReplayObservation.ts:29-88](file://src/main/sessions/RdcReplayObservation.ts#L29-L88)
- [ReplayHistoryStore.ts:89-94](file://src/main/captures/replay/ReplayHistoryStore.ts#L89-L94)
- [RdcSessionService.ts:46-66](file://src/main/sessions/RdcSessionService.ts#L46-L66)
