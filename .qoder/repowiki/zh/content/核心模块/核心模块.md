...
## 项目结构
RDC-Agent 采用分层组织：主进程入口负责应用生命周期与安全策略；Agent 运行时提供统一的顶层导出；工作流层编排对话与任务；会话层管理持久化与上下文；工具层封装外部 RDX CLI 调用；Provider 层抽象多种 LLM 适配器。
...
B --> E["工具调用服务<br/>RdxCliInvokerService.ts"]
C --> F["会话服务<br/>ConversationService.ts"]
F --> G["存储适配层<br/>StorageAdapter.ts"]
F --> H["RDX 会话服务<br/>RdxSessionService.ts"]
E --> I["外部 RDX CLI"]
...
- [src/main/tools/RdxCliInvokerService.ts:1-120](file://src/main/tools/RdxCliInvokerService.ts#L1-L120)
...
- [src/main/sessions/RdxSessionService.ts:1-70](file://src/main/sessions/RdxSessionService.ts#L1-L70)
...
- 工具系统：封装 RDX CLI 的目录加载、命令组装、执行与结果解析，提供可用性检查与运行元信息。
...
- [src/main/tools/RdxCliInvokerService.ts:42-120](file://src/main/tools/RdxCliInvokerService.ts#L42-L120)
...
Orch->>Tool : 执行工具(如 rdx_probe/rdx_context/shell)
Tool-->>Orch : 工具结果/错误
Orch-->>Conv : 最终回答/完成状态
...
### 工具系统（RdxCliInvokerService）
- 职责边界：加载 RDX CLI 目录、组装命令参数、执行调用、解析结果与错误，提供可用性与运行元信息。
- 关键能力：
...
图表来源
- [src/main/tools/RdxCliInvokerService.ts:84-141](file://src/main/tools/RdxCliInvokerService.ts#L84-L141)
- [src/main/tools/RdxCliInvokerService.ts:178-221](file://src/main/tools/RdxCliInvokerService.ts#L178-L221)
...
### 会话与 RDX 运行时（RdxSessionService 与 StorageAdapter）
- RdxSessionService：管理 RDX 运行时上下文、捕获打开/关闭、预览窗口、远程回放设备激活与状态、环境注入与生命周期清理。
- StorageAdapter：统一管理项目、会话、运行记录与知识，提供会话历史读写、分支状态、上下文日志、附件导入与动作事件追加。

```mermaid
sequenceDiagram
participant UI as "界面/IPC"
participant RDX as "RdxSessionService"
participant Act as "RdxShellActionService"
participant Store as "StorageAdapter"
UI->>RDX : openProjectInput(输入/设备/会话/项目)
RDX->>Act : runAction(openCapture/openRemoteCapture)
Act-->>RDX : 返回上下文/会话ID/预览
RDX->>Store : 应用运行时上下文/快照
UI->>RDX : closeOrReplaceOpenedCapture()
RDX->>Act : runAction(closeRuntime)
Act-->>RDX : 确认关闭
RDX->>Store : 重置运行时状态/清理租约
```

图表来源
- [src/main/sessions/RdxSessionService.ts:69-212](file://src/main/sessions/RdxSessionService.ts#L69-L212)
- [src/main/sessions/RdxSessionService.ts:214-229](file://src/main/sessions/RdxSessionService.ts#L214-L229)
...
- 会话服务依赖存储适配层、追踪服务、RDX 会话服务与后台续跑。
- 工具系统依赖 Shell 调用服务与 RDX CLI 目录/设置。
...
- [src/main/tools/RdxCliInvokerService.ts:1-18](file://src/main/tools/RdxCliInvokerService.ts#L1-L18)
...
- 工具不可用或未激活：RdxCliInvokerService 返回不可用原因或未激活错误；检查设置中的命令、目录与权限。
- RDX 运行时关闭失败：closeRuntime 未确认时会保留租约；需重试或强制清理，并查看诊断信息。

章节来源
- [src/main/conversation/ConversationService.ts:445-543](file://src/main/conversation/ConversationService.ts#L445-L543)
- [src/main/tools/RdxCliInvokerService.ts:70-86](file://src/main/tools/RdxCliInvokerService.ts#L70-L86)
- [src/main/sessions/RdxSessionService.ts:710-753](file://src/main/sessions/RdxSessionService.ts#L710-L753)
...
- 使用示例路径（仅路径，不含代码）：
  ...
  - 调用 RDX CLI：参考 [src/main/tools/RdxCliInvokerService.ts:178-221](file://src/main/tools/RdxCliInvokerService.ts#L178-L221)
  - 打开/关闭捕获：参考 [src/main/sessions/RdxSessionService.ts:69-212](file://src/main/sessions/RdxSessionService.ts#L69-L212) 与 [src/main/sessions/RdxSessionService.ts:214-229](file://src/main/sessions/RdxSessionService.ts#L214-L229)
