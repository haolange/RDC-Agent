...
**所做更改**
- 新增目录投影修订跟踪机制章节
- 更新动态更新机制部分以包含 catalogProjectionRevision 功能
- 增强故障排查指南以涵盖投影修订相关问题
- 更新架构图表以反映新的失效机制
...
A --> H["catalogProjectionRevision<br/>投影修订跟踪"]
H --> I["自动失效检测<br/>确保准确性"]
```

**图表来源**
- [EffectiveCatalogService.ts:76-82](file://src/main/settings/EffectiveCatalogService.ts#L76-L82)
- [EffectiveCatalogService.ts:179-181](file://src/main/settings/EffectiveCatalogService.ts#L179-L181)
...
- **新增**：catalogProjectionRevision：计算目录投影的修订标识符，用于检测编译目录变化。

## 架构总览
有效配置计算采用"分层叠加 + 字段级合并 + 审计证据 + 投影修订跟踪"的架构。各层按固定顺序应用，后一层可覆盖前一层同名字段；每个字段的最终值都附带来源、时间戳与冲突历史，便于追踪与调试。**新增的投影修订跟踪机制确保当编译目录投影发生变化时，缓存的发现结果会自动失效，保证模型可用性信息的准确性。**

```mermaid
sequenceDiagram
participant Client as "调用方"
participant Service as "EffectiveCatalogService"
participant Projection as "catalogProjectionRevision"
...
Service->>Projection : 计算目录投影修订
Projection-->>Service : revisionId
Service->>Store : 读取 discoveries/entitlements/observed
alt 投影修订不匹配
Service->>Store : 标记发现结果为过期
end
...
### 目录投影修订跟踪机制
**新增功能**：系统现在通过 `catalogProjectionRevision()` 函数跟踪编译目录投影的变化。该函数基于 providerId、protocol 和 catalog.models 计算唯一的修订标识符。

- **投影修订计算**：`catalogProjectionRevision(request)` 使用 `revisionFor()` 函数对目录投影内容进行哈希计算
- **自动失效检测**：在 `createSnapshot()` 方法中，系统比较当前请求的投影修订与缓存发现的修订标识符
- **智能刷新触发**：当投影修订不匹配时，系统自动标记发现结果为过期，触发重新发现流程
- **向后兼容性**：对于没有 catalogProjectionRevision 的旧缓存条目，系统会将其视为过期并重新发现

```mermaid
sequenceDiagram
participant Request as "请求"
participant Revision as "catalogProjectionRevision"
participant Cache as "缓存检查"
participant Discovery as "发现加载器"
Request->>Revision : 计算目录投影修订
Revision-->>Request : revisionId
Request->>Cache : 检查缓存发现
alt 修订不匹配或不存在
Cache->>Discovery : 触发重新发现
Discovery-->>Cache : 新发现结果
Cache->>Cache : 存储新发现+修订标识
else 修订匹配
Cache->>Cache : 使用缓存结果
end
```
...
- [EffectiveCatalogService.ts:76-82](file://src/main/settings/EffectiveCatalogService.ts#L76-L82)
- [EffectiveCatalogService.ts:179-181](file://src/main/settings/EffectiveCatalogService.ts#L179-L181)
- [EffectiveCatalogService.ts:272](file://src/main/settings/EffectiveCatalogService.ts#L272)
...
- **新增**：投影修订跟踪：当编译目录投影发生变化时，自动使缓存的发现结果失效，确保模型可用性信息的准确性。
...
Svc->>St : 写入 discoveries/entitlements + 修订标识
...
- **新增**：目录投影变更检测：当编译目录投影发生变化时，自动触发发现刷新，确保模型可用性信息准确。
...
- [EffectiveCatalogService.test.ts:630-641](file://src/main/settings/EffectiveCatalogService.test.ts#L630-641)
- [EffectiveCatalogService.test.ts:689-739](file://src/main/settings/EffectiveCatalogService.test.ts#L689-L739)
...
- **新增**：投影修订缓存：避免重复计算目录投影修订，提升性能。

**章节来源**
- [EffectiveCatalogService.ts:246-298](file://src/main/settings/EffectiveCatalogService.ts#L246-L298)
- [EffectiveCatalogService.ts:365-388](file://src/main/settings/EffectiveCatalogService.ts#L365-388)
...
- **新增**：投影修订问题：如果模型可用性信息不准确，检查 catalogProjectionRevision 是否正确计算；确认目录投影变化后是否触发了正确的刷新流程。
...
## 结论
EffectiveCatalogService 与 mergeEffectiveCatalog 共同实现了安全、可审计、可扩展的有效配置计算体系。通过严格的优先级顺序、字段级合并、安全准入、动态更新机制以及**新增的目录投影修订跟踪**，系统能够在复杂环境中稳定地生成最终配置，并提供完整的证据链以支持排错与合规。**投影修订跟踪机制确保了当编译目录投影发生变化时，系统能够自动检测并刷新缓存，保证模型可用性信息的准确性和一致性。**
...
- **新增**：投影修订：每次发现刷新时存储 catalogProjectionRevision，用于后续失效检测
...
### 目录投影修订跟踪详解
- **修订计算**：`catalogProjectionRevision(request)` 基于 providerId、protocol 和 catalog.models 计算唯一标识符
- **失效检测**：在 `createSnapshot()` 中比较当前请求的投影修订与缓存发现的修订标识符
- **智能处理**：当修订不匹配或缓存缺少修订标识时，自动标记发现结果为过期
- **向后兼容**：对于旧版本的缓存条目，系统会将其视为过期并重新发现
- **性能优化**：修订计算使用高效的哈希算法，避免不必要的重新发现
