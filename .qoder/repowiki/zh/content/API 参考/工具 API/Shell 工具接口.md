...
## 简介
本文件面向 RDC-Agent 的 Shell 工具接口，系统性说明命令执行、管道操作、文件处理与进程控制能力；重点阐述安全沙箱机制、权限控制与资源限制；并覆盖异步执行、输出流处理、错误恢复与性能优化策略。同时提供 Shell 脚本集成与安全使用建议，帮助开发者在 Agent 工作流中安全、可靠地调用系统 Shell 与 RDX CLI。

## 项目结构
围绕 Shell 工具的核心代码主要分布在以下模块：
- 工具层：ShellInvocationService、RdxCliInvokerService、RdxShellActionService、executeRdxShell
...
```mermaid
graph TB
A["Agent 工具调用"] --> B["RdxCliInvokerService"]
B --> C["ShellInvocationService"]
C --> D["ProcessSupervisor"]
D --> E["操作系统子进程"]
B --> F["RdxShellActionService"]
F --> C
G["executeRdxShell"] --> B
H["ShellResolver"] --> C
...
- [RdxShellActionService.ts:154-270](file://src/main/tools/RdxShellActionService.ts#L154-L270)
...
- RdxCliInvokerService：将 RDX CLI 配置、参数拼装、可用性检查、编解码与追踪整合为可复用的 CLI 调用入口。
- RdxShellActionService：基于配置的 RDX Action 运行器，变量替换、JSON 信封解析、诊断格式化与日志记录。
- executeRdxShell：受严格上下文与租约约束的原生 RDX 执行入口，包含输入校验、所有权校验、协议一致性校验与回执生成。
...
### executeRdxShell
职责
- 严格上下文校验：仅 General agent 且具备有效租约时可执行原生 RDX 操作。
- 输入校验：operation 白名单、args 白名单字段保护、实验 ID 准备回执。
- 执行与一致性：调用 RdxCliInvokerService，校验 result_kind 与 replaySessionId 一致。
...
- RdxCliInvokerService 依赖 ShellInvocationService 执行具体命令，并依赖 SettingsService 获取配置。
- RdxShellActionService 依赖 ShellInvocationService 与 SettingsService/AppPathService。
- executeRdxShell 依赖 RdxCliInvokerService、RdxNativeProtocol、RdxExecutionReceipts 以及会话租约服务。
...
SI --> RCI["RdxCliInvokerService"]
RCI --> RSA["RdxShellActionService"]
RCI --> ERX["executeRdxShell"]
...
## 故障排查指南
常见问题与定位
- 命令未配置或不可用：检查 ShellInvocationService 的空命令分支与 RdxCliInvokerService 的可用性检查。
...
- RDX 执行被拒绝：检查 executeRdxShell 的上下文与租约校验，确保 General agent、拥有会话/项目租约、非 default context。
...
- RdxCliInvokerService 与 RdxShellActionService 将配置、参数与结果标准化，便于追踪与复用。
- executeRdxShell 在严格上下文与租约约束下执行原生 RDX 操作，并提供可验证的回执。
...
- RDX 上下文：仅 per-session lease，禁止全局镜像；工具执行前必须校验所有权。
- 秘密隔离：safeStorage 不可用则 fail-closed，对外仅返回掩码预览。

最佳实践
- 始终设置合理的 timeoutMs 与 AbortSignal，避免长时间阻塞。
- 使用 RdxShellActionService 的变量替换，避免硬编码敏感信息。
- 对大输出使用 RingBuffer 限制，避免内存压力。
- 对 RDX 原生操作，确保具备租约与权限，并使用 executeRdxShell 以获得回执。
