...
- 外部能力桥接：src/main/tools/*（RDX CLI、Shell Action、执行回执等）
...
#### 外部进程与 RDX 能力
- RdxNativeProtocol/RdxExecutionReceipts：定义与外部 RDX 工具的协议与执行回执结构。
- RdxShellActionService/executeRdxShell：封装 Shell Action 与 RDX CLI 调用，统一错误与日志。

章节来源
- [RdxNativeProtocol.ts:1-200](file://src/main/tools/RdxNativeProtocol.ts#L1-L200)
- [RdxExecutionReceipts.ts:1-200](file://src/main/tools/RdxExecutionReceipts.ts#L1-L200)
- [RdxShellActionService.ts:1-200](file://src/main/tools/RdxShellActionService.ts#L1-L200)
- [executeRdxShell.ts:1-200](file://src/main/tools/executeRdxShell.ts#L1-L200)
...
- 端到端：结合 RDX CLI 与 Shell Action 的集成测试，验证外部进程调用链路。
...
- 外部进程：RDX CLI/Shell Action 调用失败时，查看回执结构与错误码，定位权限或环境问题。
...
- 集成验证：结合 RDX CLI/Shell Action 进行端到端测试。
...
- 模拟外部依赖：对 Web/Shell/RDX 调用使用桩或沙箱环境，隔离不稳定因素。
