...
| --- | --- | --- | --- | --- | --- | --- |
| UI-composer-model-pill-hug | Composer 模型胶囊随生效模型名 hug，不预留 12rem 槽；超长 ID 用 max-width 封顶并仅实际溢出渐隐；窄容器只收紧 max-width（8rem / 6rem） | `check:design-tokens`；`check:appearance`；disposable `start:agent-browser` | disposable `qa-1789473202505-45ca43348c554df2`（已清理）；project `proj_829103f4d381`；session `sess_4d6a0866d72a`；同源 `/app`；Dark；1440/1920：`选择模型` menuW=89.3、`grok-4.6`=87、长名=192 且 `is-overflowing`，flex `0 1 auto` max=192；390×844 单行无水平溢出 max=96；500px 容器 max=128。证据 `working-44df112a/composer-model-pill-hug.json`。无 token | planned | — | 2026-09-15 |
