...
| Task | 状态 | 范围与验证 |
| --- | --- | --- |
| RC5-1 审查修复 | 通过 | 类型阻断、共享品牌、Composer 几何与生命周期、图标加载与生成；身份模型不变 |
| RC5-2 验证 | 待验证 | 聚焦测试、完整 coverage/type/lint/contracts/gates/build，Browser QA 与原生图标 |
| RC5-3 发布 | 执行中 | rc.5 包校验、main/tag/资产 digest、未签名预发布 |
| RC5-4 清理 | 待执行 | 自有进程/临时产物清理、证据账本与桌面启动权 |
