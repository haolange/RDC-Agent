...
审批卡、Work Process tool 卡、Asked 卡、计划卡、计划阅读面板、handoff 建议行、子代理委派卡、Tasks 快照卡、图像缩略图与 Compact 摘要共用 `--transcript-card-*` 变量。普通工具和子代理共用 transcript 卡头组件：图标、标题、状态、耗时和箭头同一行，细线下方为正文。子代理的任务预览另占一行，与卡头共用点击和键盘入口；预览常规字重、次级文字色，溢出才淡出。卡片展开后直接排列冻结 capsule 的原话字段，不保留「委派任务」一级折叠或重复正文底壳；事实与引用、执行约束仅在有内容时出现。工作过程、最终回复与调用详情为独立透明披露区，固定右端箭头和清晰 focus-visible。主子过程共用 Working Process 内容与工具行组件；子过程使用紧凑密度，普通主工具保留卡片密度，长代码、JSON、表格才使用内层载荷表面。最终回复直接复用主回复 Markdown。运行点阵只表达非确定性活动，不把时间映射成完成比例；终态静止并固定耗时。缩略图经 `conversation:getToolImagePreview` 取 `data:` URL。用量 Tokens / Cache / Reasoning 在宽面保持三栏，标题、主数值和次级信息同刻度对齐，无固定空高；窄面才单列，未知与真实零值保持区别。

Tasks 历史卡在主回合每次真实创建或更新后各留一张，卡头以「添加待办／已更新待办」区别事件，右端显示当刻 `已完成 / 总数` 和固定箭头，不显示伪造的工具耗时。批量创建只生成一张卡；卡内逐项沿用 Task 状态映射，以空圈、进行中标记、勾选、受阻警示及取消标记代替序号，完成项降低对比、进行中项优先可读，受阻原因紧贴所属任务。逐项无独立底壳，长标题换行且窄屏不溢出。最新历史卡默认展开、较早卡默认收起，手动开合跨窗口化重挂载保留；旧记录缺少事件种类时显示中性「任务列表」。右栏 Progress 保持序号外观并展示最新任务状态，定位时可展开相应的历史卡。

任务预览只在外卡折叠时存在；外卡展开后从 DOM 卸载预览，正文位置只出现一份冻结 capsule 内容。点阵仅在折叠态位于卡头分隔线下、渐隐任务文字后方，与状态、耗时、箭头所在列左右对齐；展开后卸载点阵，卡头保持普通工具的单行高度，宽卡为 12 列 × 2 行；卡片容器不宽于 34rem 时为 6 列 × 2 行，并扩大行距，使窄卡内点距接近均匀，不暗示进度比例。事实与引用、执行约束在任务内容内部各占一张二级披露卡；调用详情内的参数、父回执、实际发送内容、执行标识也各占一张。二级卡复用 transcript 边框、圆角与表面 token，以较轻标题区别于主区；收起只显示卡头，展开后由细线连接所属正文，卡与卡之间留白，不在整组外再套卡。不可用记录显示说明而非空展开入口，hover 保持低对比且 focus-visible 保留圆角焦点轮廓。任务原话字段在同一内容流按语义章节排版；任务与最终回复不另套正文气泡。最终回复沿用 `MessageMarkdown` 的首尾及段落语义。原始内容在所属二级卡内复用工具回执排版并按页面延伸。工作过程不加正文壳，嵌入时仅由披露正文提供起始留白；主子工具行共用一棵语义 DOM 和工具卡骨架，紧凑样式只收紧间距与内边距，不去除普通工具的边框、卡头分隔线或正文归属。`background_wait` 仍是轻量事件。共享工具列表仅约束直属步骤，不隐藏子执行时间线。无叙述的 turn 不预留叙述间距；具备真实 toolCallId 的 Hook 诊断归入对应工具卡：正常完成仅在展开详情中显示，警告/错误在收起卡中提示并在详情保留；不改变工具自身执行状态。无有效工具归属的生命周期诊断仍按原事件位置显示，旧记录不猜测归属。子代理工作过程正文顶部为 --space-3、底部为 --space-4，不给末事件增加间距；不把诊断当作 commentary 或补造不可见 thinking。默认工具摘要与输出完全相同时只展示一份，原始回执保持完整。

旧工作记录中缺少 `toolCallId` 的 Hook 完成诊断仍位于真实事件位置，以紧凑的共享诊断卡呈现：收起时只显示「Hook 诊断」，展开后显示原始文字。不能根据相邻位置推断它属于哪个工具。新记录中具有真实关联 ID 的 Hook 仍在所属工具详情内显示。

独立 Hook 诊断卡与前一工具的可见边框保持 `--space-3`（12px）间距，包含前一工具恰好是嵌套步骤末项的情况；下一 turn 继续使用共享 turn 间距。收起和展开均保持这一边界，窄屏不得因卡片间距产生横向溢出。
