...
1. **Spaces / 控制列**：Brand、Cards / Candidates / Conflicts 三段切换、User + 已注册 Project 多选（默认全选）、type / lifecycle 过滤、七 lane 开关、结构索引状态与 `Rebuild Index`、`Import ColdData…`。
...
- Semantic lane 未 ready 时不得声称语义检索，不得静默降级后仍点亮 Semantic。
- Center 的 Rebuild Index 只重建 `KnowledgeIndexService` 结构索引。语义索引重建属于 Settings > Models > Embedding。
