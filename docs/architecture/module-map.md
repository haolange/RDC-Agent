...
| Agent workbench | `src/main/workflow/debugger/AgentOrchestrator.ts`, `src/main/settings/AgentManifestService.ts`, `src/main/agent-runtime/*` | `.agent.md`, `AgentId`, `AgentManifestDefinition`, `WorkflowState`, `BUILTIN_AGENT_TOOL_IDS` / tool tokens | `conversation:sendMessage`, `agent:*`, `workflow:stop`, `workflow:getState`, `workflow:listRuns` | Composer, AgentChat, Settings > Agents, control panel | typecheck, settings-agents, tool-system, browser smoke |
