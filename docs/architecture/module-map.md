...
| Agent workbench | `src/main/workflow/debugger/AgentOrchestrator.ts`, `src/main/settings/AgentManifestService.ts`, `src/main/agent-runtime/*` | `.agent.md`, `AgentId`, `AgentManifestDefinition`, `WorkflowState` | `conversation:sendMessage`, `agent:*`, `workflow:stop`, `workflow:getState`, `workflow:listRuns` | Composer, AgentChat, Settings > Agents, control panel | typecheck, settings-agents, browser smoke |
