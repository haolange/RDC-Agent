...
- `WorkTrace`: UI progress data in `ConversationWorkTrace`. Each `llm_turn` contains optional `ThinkingArtifact`, a `thinkingStatus` lifecycle flag, a canonical `result`, and nested tool calls. While running, the Work Process header uses dynamic Active Signal copy such as `正在思考`; after settle it is process-first (`思考过程 · 持续 ... · ... 个动作` / English `Thinking`) rather than a terminal status headline. The normal Work Process UI renders process-backed loops with a fixed hierarchy: provider-visible thinking disclosure first as the loop's top transcript node, loop narration/result text second, and flat folded tool / approval / `ask_user` evidence third (no `toolGroup` nesting shell). Loop-level thinking labels align with the header: active `正在思考`, settled `思考过程`. Desensitized Request Inspector lives in Control Panel, not inside the Work Process transcript. The canonical `result` remains runtime data for lifecycle/accounting, but answer-body-only result text must not duplicate into the Work Process. `assistant.thinking_delta` updates thinking only; `assistant.thinking_end` only marks thinking complete; `assistant.delta` streams assistant answer text outside the Work Process unless that loop also has tool evidence.
...
After at least one visible process section exists, a later answer-only `llm_turn` is rendered as a compact final-response boundary row instead of another process section. This keeps the transcript order stable during final-answer streaming: process evidence first, then a response boundary, then assistant body text outside Work Process. Late provider summary thinking from that answer-only turn may attach to the response boundary as a collapsed disclosure; repeated summary text that was already shown in the prior process section is dropped.
