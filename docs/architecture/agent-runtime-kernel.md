...
Fast, 1M context, reasoning effort, Agent, model, provider, and protocol changes apply to the next turn without proactively compacting or rewriting history. Every active turn owns the closed `RequestPlan` captured at creation, so an asynchronous model change can affect only the next send. Compaction is a derived request view used only when the target window requires it; the canonical journal is never replaced by a summary. Missing visible turn entries, invalid journal records, incompatible migration state, or persistence failure fail closed.

`previewNextRequestContext` is a read-only projection over that same production path. It materializes the active branch from `SessionContextJournal`, filters provider-native reasoning artifacts by exact `providerId + modelId + protocol`, resolves the effective tools/skills and pending attachments, builds the current `PromptPlan`, `RequestEnvelope`, and `RequestPlan`, then returns estimated input, prompt budget, complete window, breakdown, compaction intent, and a blocking reason. The renderer revisions requests and discards late results. Preview never persists branch repair, consumes pending handoff, executes tools, embeds attachment bytes, or mutates conversation history. Actual send rematerializes the journal and fails closed when vision media or tool schemas cannot fit the selected route/budget.

Settings and `/model` share the scoped agent-definition save service. A save atomically writes one agent manifest, derives its route, refreshes only that agent, and echoes the monotonic `clientRevision`; it does not rebuild the workspace, reinitialize IPC, reload the whole LLM settings object, or broadcast every provider catalog. Renderer selection is optimistic and only the latest failed revision may roll back.
