...
`RightRailProjectionService` collects only explicit, user-recognizable output files through `SessionArtifactSource`. `output_register` is the only agent path into this lane: it accepts a completed file inside the active project, rejects `.rdx/inputs` and escaping paths, copies it into the owning run, and registers the copy. `ArtifactStore` may remain the backend registration mechanism, but the renderer contract exposes only user-facing sources: `report`, `evidence`, `image`, `document`, `data`, or `other`. The projection does not surface live or frozen session plans, does not scan arbitrary action payload paths, and does not expose internal producer names such as artifact store, run report, or action output. Plan review lives on the transcript / Composer gate, not this lane. Current-run records are shown first, prior-run records are grouped as history, missing files remain visible as `failed`, and source inputs never enter this lane. The renderer names this lane Outputs. Investigation Artifacts use the separate Artifacts lane (`Progress / Artifacts / Outputs / Context / Capture`).
