...
| Knowledge 五服务（目标态 / 迁移中） | `src/main/knowledge/` | `Query/Index/Compile/Candidate/Write` 与七 lane、五个 deferred `knowledge_*` 工具、`$knowledge-scout` / `$knowledge-candidate`、Knowledge Center 三列 UI 已落地；持久写入仅 human review；ColdData 摄入为 staging/Draft，**不自动 Candidate**；`fixed ≠ verified`。durable canonical store 已落地（`<sessionPath>/knowledge-state.json`，跨进程锁 + revision + contentHash）；ColdData bounded read 归一化为 session Draft 并记录/复核源 hash/mtime/size；human review 后原子 + realpath 写入 space root。Semantic 真实向量索引已落地。`check:knowledge-system` 债务 allowlist 已空（hits=0）。**产品级真实 OpenAI 调用、Browser QA 与真实 user-space 持久化尚未跑（T18）** |
...
- **[AUTO]**（五服务 / 七 lane / 三列 UI 已落地；债务 allowlist 已空 hits=0）`pnpm run check:knowledge-system`：Knowledge 五服务单一事实源、七 lane 可用性、semantic `unavailable/stale` fail-closed、human-only 写入、ColdData 不自动 Candidate、脱敏 fixture、无自动 Promote。`knowledgeSystemContract.test.ts` 已由门禁硬执行，禁止 skip/todo/无断言空壳。**产品级 Browser QA 与本机 ColdData 真实验收尚未跑。**
