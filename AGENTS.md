...
| `ErrorRecovery` / `ProviderEmptyStreamError` / `ProviderWireFailureError` | `src/main/agent-runtime/agent/ErrorRecovery.ts` + `providers/internal/http.ts` | 空流 `PROVIDER_STREAM_EMPTY` 不得合成 HTTP 502；`empty_stream` 最多 1 次重试；真实 5xx→`server_error`（3 次）；503/529 与 `at capacity`/`high demand`/`overloaded`→`overloaded`；401 立即 abort；`AgentRecoveryAbortError` 必带 cause + category/attempts/maxAttempts/lastStatus?/bodySnippet；`createTurnFailedDiagnostic`→`CONVERSATION_LLM_REQUEST_FAILED` + 分类 userMessage + `technicalMessage`；`error_recovery_*` 不进 Work Process；`run.failed` 走内部 `runtime-run`，最终诊断块 `llm-request-failed` |
...
| 模块 | 路径 | 边界 |
| --- | --- | --- |
| `ProfileHandoffState` | `src/shared/types/profileHandoff.ts` + `src/main/sessions/handoffStateSchema.ts` + `src/main/sessions/HandoffStateStore.ts`（`<sessionPath>/handoff-state.json`） | 字段 `handoffId/lifecycle/sourceTurnId/sourceRequestId/sourceAgentId/toAgentId/chainRoot/depth/prompt/label/declaredModel/timestamps/cancelReason`；`declaredModel` 字段必存在、值可为 `null`；`prepared`=工具成功，`committed`=源 turn complete，`consumed`=目标消息 commit；事务顺序：内存草稿 `prepare` → 绑定 `turn.pendingHandoff` → `after-hook` → 持久化 `HandoffStateStore.prepare`；Hook / 持久化 / 绑定失败必须显式 cancel/rollback，不得遗留 active prepared；`send:true` 由源 turn complete / 会话 turn-idle 事件续跑，单次 generation token，有界 idle 观察，**禁止 microtask 自递归**；每用户 root 链最多 3；重启降级手动；Stop/Rewrite/branch/手动切换取消；审批不继承；同 session 一活跃；非法 model fail-closed；模型优先级始终 session `modelOverride` > 通过 `isAgentToolExecutableModel` 校验的 handoff `declaredModel` > target route。现有 `AgentHandoffDefinition` **不是**该状态机。**当前态**：事务顺序与事件驱动续跑已落地。产品级 Browser QA 全矩阵见 U05 / [`docs/product/acceptance-ledger.md`](docs/product/acceptance-ledger.md) |
| Knowledge 五服务 | `src/main/knowledge/` | 目标拓扑：`Query/Index/Compile/Candidate/Write` + **六 lane**（Identity/Path、Scope/Metadata、Lexical、Structural、Relation/Graph、Temporal/Version）markdown-first；五个 deferred `knowledge_*`、`$knowledge-scout` / `$knowledge-candidate`、Knowledge Center 三列 UI。canonical 读根 `realpath(~/.rdx/knowledge)` + `realpath(<projectRoot>/.rdx/knowledge)` 对 `read_file`/`read_image`/`glob`/`grep` 免审批（**U02 落地**）；写入仍经 `knowledge_*` + human review。单一事实源：`knowledge_*` 由 main-owned `KnowledgeIndexService` 提供；grep/glob/read 只是补充证据；禁止第二索引或 renderer 事实源。ColdData 摄入为 staging/Draft，**不自动 Candidate**；`fixed ≠ verified`。durable canonical store 已落地（`<sessionPath>/knowledge-state.json`）。**代码诚实**：源码仍是七 lane + Semantic / Embedding，由 **U02 删除** Embedding capability / Semantic lane / `llm.embedding`。**T18 ColdData 已证**（user-space 两张 case card + Center 可见 + 词法命中）。产品级 Browser QA 全矩阵见 U05 ledger。`check:knowledge-system` 债务 allowlist 已空（hits=0）；U02 将把门禁改为六 lane + forbidden embedding。 |
| Investigation 垂直 schema | Session Artifact only，namespace `rdc.investigation.v1` | schema + `InvestigationArtifactService` + `investigation_read` / `investigation_write` / `investigation_list` 已落地；`WorldState` / `EvidenceRecord` / `ClaimRecord`（含 `claimId`+`experimentId`） / `ExperimentRecord`（含 `experimentId`） / `ChallengeRecord`（含 `challengeId`） / `MissionCheckpoint` / `InvestigationArtifactManifest`；不侵入 `TaskRecord` / Profile / Message。15 个垂直方法 Skill（含 `debugger-causal-method` 与 `analyzer-architecture-method`）与 4 个 builtin Hook 模板已落地。Session rail 五卡已落地，Artifacts 只投影该 namespace。三条 Mission 方法面已接到 Skill/Hook/Capsule。Analyzer `claimKind` 不得越 Observed / Reconstructed / Authoring 层；Optimizer 无 rollback 的 mutate 不得关闭。事务已落地（journal / temp-set / commit marker / atomic replace）；完成合同已落地（turn 收口：checkpoint + ready report + `final_answer` 引用）；IPC `investigation:read({ sessionId, artifactId, expectedHash })` **已落地**；投影携带完整 `contentHash`（renderer 只缩显）。产品级 Browser QA 全矩阵见 U05；三条 Mission 正常 `completed` 见 U06 / ledger。`check:investigation-system` 债务 allowlist 已空（hits=0） |
| 并发工具组 | `src/main/agent-runtime/agent/ConcurrentToolScheduler.ts` + `toolConcurrency.ts` | 缺省不安全（仅 `isConcurrencySafe===true`）；只并发连续安全组，unsafe 独占；`shell`/write/task mutation/RDX（含 `rdx_probe`）/MCP/ask/handoff/output 串行；`requiresRdxLease=true` child 经显式 delegated lease 取得 parent 上下文并串行，完成/取消立即撤销；`requiresRdxLease=false` child **在 allowlist 层**拿不到 `rdx_context` / `rdx_probe` / `shell` 中的 RDX 路径；禁止并发 RDX 双 owner；`callIndex` 稳序；dispatch 前原子预算；abort `allSettled` join；部分失败不连坐 |
| Run schema v3 | `src/main/sessions/runV3/` | 唯一活跃 schema 为 `'3'`。无 `lastStage` / `WorkflowStage` / `recommendedSpecialists`。v0–v2 在 `.run-v3-migration.lock` 内 archive-and-rewrite 到 `migration-backups/run-v3/`；既有 `run-v2` archive 不迁移、不 GC。不得双读；不得从旧 stage 推断 Mission。见 `DESIGN.md` 裁决 I |
| Mission plan-only | `debugger` / `analyzer` / `optimizer` runtime allowlist | 仅允许只读信息、`ask_user`、`agent_handoff`、Tasks（剔除 `output_register`）、`plan_artifact`、investigation_*、knowledge_*（只读+显式 Candidate）、Memory 只读、Skill 发现、`subagent`（默认仅 `general`）、`rdx_context` + `rdx_probe`。禁止 `shell` / `code_interpreter` / write/edit/git / `output_register` / Memory 写删 / session 与 settings mutation / MCP / 未白名单 RDX action。四层 enforcement，Full access 不能绕过。见裁决 J |
| `rdx_probe` | T03/T06 实现；Settings `tooling.rdxCli` 只读 action | 新 tool id。输入 `{ action: 'enumerate' \| 'doctor' \| 'version' \| 'probe' \| 'lease_open' \| 'lease_close' \| 'preview_status'; capturePath?; contextId?; args? }`。只读 probe；lease open/close 仅当前 session；raw `.rdc` bytes 永不进模型。Mission 不得获得 generic shell。见裁决 J |
| Hook trust fingerprint | `HookEngine` + Settings Hooks | fingerprint = parsed definition + 所有 resolved 脚本/参数文件 bytes + canonical realpath + scope/provenance + PATH 解析后的 executable identity；任一变化 `needsRetrust`。builtin 默认信任且内容变化随仓库发布；user/project 必须显式 trust。旧仅-YAML-hash trust 首次加载失效并要求 retrust。12 canonical events 单一路径。见裁决 K。**当前态**：全指纹与 schemaVersion 2 已落地。产品级 Browser QA 全矩阵见 U05 / ledger |
...
- Session rail 只有五张不可折叠圆角卡 `Progress / Artifacts / Outputs / Context / Capture`。Artifacts 只投影 main-owned Investigation Artifacts；Outputs 仍只接受 `output_register`；Capture 保留。禁止再写三卡或四卡作为现行合同。
...
- Run `pnpm run check:right-rail` for UI/IPC changes. Browser QA must use the latest `start:agent-browser` bootstrap `/qa` surface, delete QA project sessions first, create isolated sessions, and cover empty state, real Tasks, Artifacts, Outputs, project import, RDX owner/diagnostic behavior, cross project/session gates, narrow drawer, Escape/focus return, Settings parity, unknown-channel bridge denial, and Artifacts 空态 / 有内容 / 与 Outputs 分离。
...
- **[MANUAL] [BROWSER-QA]** T00 之后宣称完成仍须门禁 + Browser QA，不得只靠文档或 commit message。`check:knowledge-system` / `check:investigation-system` 保持 zero-debt（hits=0）。
...
- **[AUTO] [BROWSER-QA]** Agent Loop / Tasks 改动必须覆盖相同指纹第二轮纠偏、第三轮终止、revision/result/args 变化复位、max-turn typed error、只读 route 仅 `task_list`/`task_get`、可写 route 仅在冻结工具集含 mutation 时宣称可写、text-only route 不宣称工具、`tool_search` authoritative no-match；Browser 真实会话至少命中一次三轮无进展终止并确认没有 `CONVERSATION_LLM_REQUEST_FAILED`。
...
- **[AUTO]** Builtin 工具目录、manifest token 展开与 `REJECTED_TOOL_TOKENS` 契约验证使用 `pnpm run check:tool-system`（当前 49 builtin ids（work-process coverage 另计 1 个 MCP fixture），含 `shell` / `read_image` / `code_interpreter` / `artifact_read` / `rdx_probe`、五个 deferred `knowledge_*` 与三个 deferred `investigation_*`；旧 token `bash` 硬拒）。
- **[MANUAL]** Verifier 结论落盘 [`docs/product/acceptance-ledger.md`](docs/product/acceptance-ledger.md)。`pnpm run check:acceptance-ledger` 由 **U04** 落地并接入 CI；落地前不得假装门禁已绿。
- **[AUTO]** `pnpm run check:legacy-residue` 零命中由 **U03** 落地（U04 接入 `check:gates`）；合法词（provider `deprecated`、Knowledge lifecycle `deprecated`、OpenAI-compatible、CSS Semantic Token、图形学 Semantics）用精确上下文白名单。
...
- **[AUTO]**（五服务 / 三列 UI / T10 一致性已落地；债务 allowlist 已空 hits=0）`pnpm run check:knowledge-system`：Knowledge 五服务单一事实源（Draft/Candidate/review → `KnowledgeDurableStore`；卡片正文/index → space markdown + `KnowledgeIndexService`）、无 Candidate/Draft 正式 Map、**六 lane**、human-only 写入、ColdData 不自动 Candidate、`fixed ≠ verified`、五工具 deferred、skill 交集、脱敏 fixture、无自动 Promote。U02 将重写门禁：`lanes.six` + `forbidden.embedding-runtime` 零命中；不得再把 Semantic hits / 七 lane / metadata-only ready 写成现行门禁。`knowledgeSystemContract.test.ts` 已由门禁硬执行，禁止 skip/todo/无断言空壳。**T18 ColdData 已证**，不得写成尚未跑。产品级 Browser QA 全矩阵见 U05 / ledger。
- **[AUTO]**（schema + service + 工具 + 15 Skill + 4 Hook + 五卡已落地；三条 Mission 方法面已接到 Skill/Hook/Capsule；债务 allowlist 已空 hits=0）`pnpm run check:investigation-system`：垂直 schema 字段完整性（含 `claimId`/`experimentId`/`challengeId`/`artifactId`）、`ready` 需要结构化 `sourceRefs>=1`（`artifactId`+`expectedHash`）+ `contentHash` 对 `contentRef` + kind Registry + 正文 schema 通过、认识论偏序不可升级、`S-CAUSAL-01` 必须引用可解引用 Experiment 且 `intervention.type!=none`、`status ∈ {recorded, rolled_back}`、`rollback.executed===true` 且 `rollback.baselineRestored===true` 并存在 verify evidence、`S-CLAIM-01` 投影须带 `compactProvenance`、`S-RDC-01` mutate 隔离、Analyzer `claimKind` 不得越层、Optimizer 无 rollback 的 mutate 不得关闭、不侵入 `TaskRecord`/`AgentProfile`/`ConversationMessage`。`investigationSystemContract.test.ts` 已由门禁硬执行，禁止 skip/todo/无断言空壳。`rail.five-cards` 断言五卡且存在 `id="artifacts"`。Mission 正常 `completed` 须 turn 收口门禁（checkpoint + ready report + `final_answer` 引用）；Partial / Inconclusive / Blocked 不得伪装 completed。Renderer 唯一读取通道 `investigation:read` 已落地。产品级 Browser QA 全矩阵见 U05；三条 Mission 正常 `completed` 见 U06 / ledger。
