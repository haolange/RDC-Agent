...
- **[AUTO]** Builtin 工具目录、manifest token 展开与 `REJECTED_TOOL_TOKENS` 契约验证使用 `pnpm run check:tool-system`（当前 40 ids，含 `shell` / `read_image` / `code_interpreter` / `artifact_read`；旧 token `bash` 硬拒）。
