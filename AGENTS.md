...
| `EmbeddingCatalog` / `EmbeddingExecutionService` | `src/shared/provider-catalog/embedding*.ts` + `src/main/settings/EmbeddingExecutionService.ts` | 独立 embeddings protocol/adapter；manifest `embeddings` → catalog → execution；opaque credential `operation=embed`；consent 默认关；semantic `unavailable/stale` fail-closed；identity/dimension/chunker/corpus hash；显式 rebuild。Semantic lane 只有完整、原子、hash-bound 的**真实向量索引**才能 `ready`；**禁止**只写 metadata snapshot 就标 `ready`。**当前态**：rebuild 只写 snapshot、hits 恒为空，属假 ready。**不得**进入 Agent/Composer/subagent picker；Discovery agent modality 边界不放松。**产品级 Browser QA 与本机 ColdData 真实验收尚未跑** |
| Knowledge 五服务（目标态 / 迁移中） | `src/main/knowledge/` | `Query/Index/Compile/Candidate/Write` 与七 lane、五个 deferred `knowledge_*` 工具、`$knowledge-scout` / `$knowledge-candidate`、Knowledge Center 三列 UI 已落地；持久写入仅 human review；ColdData 摄入为 staging/Draft，**不自动 Candidate**；`fixed ≠ verified`。durable canonical store 已落地（`<sessionPath>/knowledge-state.json`，跨进程锁 + revision + contentHash）；ColdData bounded read 归一化为 session Draft 并记录/复核源 hash/mtime/size；human review 后原子 + realpath 写入 space root。Semantic 假 ready 仍未做。`check:knowledge-system` 债务 allowlist 已空（hits=0）。**产品级 Browser QA 与真实 user-space 持久化尚未跑** |
