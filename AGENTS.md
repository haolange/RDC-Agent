...
| 模块 | 路径 | 边界 |
| --- | --- | --- |
| `Builtin Agent Profiles`（目标态 / 迁移中） | `resources/agent-runtime/agents/` | 目标四个 builtin：`general` / `debugger` / `analyzer` / `optimizer`；scope `builtin < user < project`；**不再写 user seed**。当前仍是 Ask/Plan/Edit + 三类 Mission 的 `~/.rdx/agents` seed，不得把目标写成已完成 |
| `AgentSeedMigration`（目标态 / 迁移中） | `~/.rdx/agents/.seed-migration.json` + `.migrated/<migration-id>/` | 历史 S0–S8 以 parse 后语义 hash 识别（model/icon/accent 变化也算用户修改，保守不迁）；匹配官方语义则原子备份不删除；`StorageIo` 幂等 / 可重入 / 有诊断。禁止交互导出选择面 |
| `ProfileHandoffState`（目标态 / 迁移中） | session-owned durable record（路径待实现） | 字段 `handoffId/lifecycle/sourceTurnId/sourceRequestId/sourceAgentId/toAgentId/chainRoot/depth/prompt/label/declaredModel/timestamps/cancelReason`；`declaredModel` 字段必存在、值可为 `null`；`prepared`=工具成功，`committed`=源 turn complete，`consumed`=目标消息 commit；`send:true` 自动续跑；每用户 root 链最多 3；重启降级手动；Stop/Rewrite/branch/手动切换取消；审批不继承；同 session 一活跃；非法 model fail-closed；模型优先级始终 session `modelOverride` > 通过 `isAgentToolExecutableModel` 校验的 handoff `declaredModel` > target route。现有 `AgentHandoffDefinition` **不是**该状态机 |
