...
| 模块 | 路径 | 边界 |
| --- | --- | --- |
| `ProfileHandoffState` | `src/shared/types/profileHandoff.ts` + `src/main/sessions/handoffStateSchema.ts` + `src/main/sessions/HandoffStateStore.ts`（`<sessionPath>/handoff-state.json`） | 字段 `handoffId/lifecycle/sourceTurnId/sourceRequestId/sourceAgentId/toAgentId/chainRoot/depth/prompt/label/declaredModel/timestamps/cancelReason`；`declaredModel` 字段必存在、值可为 `null`；`prepared`=工具成功，`committed`=源 turn complete，`consumed`=目标消息 commit；事务顺序：内存草稿 `prepare` → 绑定 `turn.pendingHandoff` → `after-hook` → 持久化 `HandoffStateStore.prepare`；Hook / 持久化 / 绑定失败必须显式 cancel/rollback，不得遗留 active prepared；`send:true` 由源 turn complete / 会话 turn-idle 事件续跑，单次 generation token，有界 idle 观察，**禁止 microtask 自递归**；每用户 root 链最多 3；重启降级手动；Stop/Rewrite/branch/手动切换取消；审批不继承；同 session 一活跃；非法 model fail-closed；模型优先级始终 session `modelOverride` > 通过 `isAgentToolExecutableModel` 校验的 handoff `declaredModel` > target route。现有 `AgentHandoffDefinition` **不是**该状态机。**当前态**：事务顺序与事件驱动续跑已落地。**产品级 Browser QA 与本机 ColdData 真实验收尚未跑** |
| `EmbeddingCatalog` / `EmbeddingExecutionService` | `src/shared/provider-catalog/embedding*.ts` + `src/main/settings/EmbeddingExecutionService.ts` | 独立 embeddings protocol/adapter；manifest `embeddings` → catalog → execution；opaque credential `operation=embed`；consent 默认关；semantic `unavailable/stale` fail-closed；identity/dimension/chunker/corpus hash/catalog hash；显式 rebuild。Semantic lane 只有完整、原子、hash-bound 的**真实向量索引**才能 `ready`；**禁止**只写 metadata snapshot 就标 `ready`。**当前态**：真实向量索引已落地（确定性 chunk → opaque embed → 严格校验 → atomic commit → 相似度检索）。**不得**进入 Agent/Composer/subagent picker；Discovery agent modality 边界不放松。**产品级真实 OpenAI 调用与 Browser QA 尚未跑（T18）** |
| Knowledge 五服务（目标态 / 迁移中） | `src/main/knowledge/` | `Query/Index/Compile/Candidate/Write` 与七 lane、五个 deferred `knowledge_*` 工具、`$knowledge-scout` / `$knowledge-candidate`、Knowledge Center 三列 UI 已落地；持久写入仅 human review；ColdData 摄入为 staging/Draft，**不自动 Candidate**；`fixed ≠ verified`。durable canonical store 已落地（`<sessionPath>/knowledge-state.json`，跨进程锁 + revision + contentHash）；ColdData bounded read 归一化为 session Draft 并记录/复核源 hash/mtime/size；human review 后原子 + realpath 写入 space root。Semantic 真实向量索引已落地。**T10 一致性已落地**：五服务 / 七 lane / 五工具 / 两 Skill / Center IPC 读同一 durable 事实；deferred 遵守真实 caller 与 skill 交集。`check:knowledge-system` 正向合同已强化，债务 allowlist 已空（hits=0）。**产品级真实 OpenAI 调用、Browser QA 与真实 user-space 持久化尚未跑（T18）** |
| Investigation 垂直 schema（目标态 / 迁移中） | Session Artifact only，namespace `rdc.investigation.v1` | schema + `InvestigationArtifactService` + `investigation_read` / `investigation_write` / `investigation_list` 已落地；`WorldState` / `EvidenceRecord` / `ClaimRecord`（含 `claimId`+`experimentId`） / `ExperimentRecord`（含 `experimentId`） / `ChallengeRecord`（含 `challengeId`） / `MissionCheckpoint` / `InvestigationArtifactManifest`；不侵入 `TaskRecord` / Profile / Message。15 个垂直方法 Skill（含 `debugger-causal-method` 与 `analyzer-architecture-method`）与 4 个 builtin Hook 模板已落地。Session rail 五卡已落地，Artifacts 只投影该 namespace。三条 Mission 方法面已接到 Skill/Hook/Capsule。Analyzer `claimKind` 不得越 Observed / Reconstructed / Authoring 层；Optimizer 无 rollback 的 mutate 不得关闭。目标态：record/manifest/index/supersede/stale-propagation **同一事务**；renderer 唯一读取通道 IPC `investigation:read({ sessionId, artifactId, expectedHash })`；Mission 正常 `completed` 须 checkpoint + ready report + `final_answer` 引用（见 `DESIGN.md` E / L）。**当前态**：事务已落地（journal / temp-set / commit marker / atomic replace）；完成合同已落地（turn 收口：checkpoint + ready report + `final_answer` 引用）；IPC `investigation:read({ sessionId, artifactId, expectedHash })` 已落地；投影携带完整 `contentHash`（renderer 只缩显）。**产品级 Browser QA 与本机 ColdData 真实验收尚未跑。** `check:investigation-system` 债务 allowlist 已空（hits=0） |
...
- **[AUTO]** Builtin 工具目录、manifest token 展开与 `REJECTED_TOOL_TOKENS` 契约验证使用 `pnpm run check:tool-system`（当前 49 ids，含 `shell` / `read_image` / `code_interpreter` / `artifact_read` / `rdx_probe`、五个 deferred `knowledge_*` 与三个 deferred `investigation_*`；旧 token `bash` 硬拒）。
