...
| Run schema v3 | `src/main/sessions/runV2/`（当前）→ 目标 `schemaVersion: '3'` | **当前仍为 v2**，含 `lastStage` / `WorkflowStage` / `recommendedSpecialists`。目标唯一活跃 schema 为 `'3'`；v0–v2 archive-and-rewrite；不得双读；不得从旧 stage 推断 Mission。见 `DESIGN.md` 裁决 I |
