...
| Knowledge 五服务（目标态 / 迁移中） | `src/main/knowledge/` | `Query/Index/Compile/Candidate/Write` 与七 lane、五个 deferred `knowledge_*` 工具、`$knowledge-scout` / `$knowledge-candidate`、Knowledge Center 三列 UI 已落地；持久写入仅 human review；ColdData 摄入为 staging/Draft，**不自动 Candidate**；`fixed ≠ verified`。目标态：durable canonical store（user/session ownership）；ColdData bounded read 归一化为 session Draft，记录并复核源 hash/mtime/size；human review 后原子 + realpath 写入 `~/.rdx/knowledge`。**当前态**：Candidate/Draft 为进程内 Map，写路径无 realpath、非原子；ColdData 不记录源 hash/mtime/size。`check:knowledge-system` 债务 allowlist 已空（hits=0）。**产品级 Browser QA 与本机 ColdData 真实验收尚未跑** |
