...
- **[AUTO]** `pnpm run check:legacy-residue` 零命中由 **U03** 落地并接入 `check:gates`；合法词（provider `deprecated`、Knowledge lifecycle `deprecated`、OpenAI-compatible、CSS Semantic Token、图形学 Semantics）用精确上下文白名单。
