...
写新版本 plan_artifact 和可解析 Checkpoint。用 execute 交接绑定 Plan URI/hash，并将 requiredSkillIds 明确设为 `renderdoc-execution`、`analyzer-architecture-method`、`rdx-cli-shell`、`analyzer-rdx-tools`，同时设置 returnTo=analyzer 和交付要求。后两项只供 General 学习 CLI 与本方向工具，不能给 Mission 或 General 增加权限。
