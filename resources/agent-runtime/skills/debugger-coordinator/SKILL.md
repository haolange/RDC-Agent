...
用 plan_artifact 提交当前计划等待用户审阅；拒绝则按意见修订同一份；批准后按返回的 handoff 目标调用 agent_handoff，绑定冻结 Plan URI/hash。写可解析 Checkpoint。execute 交接的 requiredSkillIds 明确设为 `renderdoc-execution`、`debugger-causal-method`、`rdx-cli-shell`、`debugger-rdx-tools`，同时设置 returnTo=debugger 和交付要求。后两项只供 General 学习 CLI 与本方向工具，不能给 Mission 或 General 增加权限。
