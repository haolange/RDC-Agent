...
- replacement 成功后，后续 `rd.export.screenshot target.semantic="event_output"`、`rd.texture.get_data`、`rd.texture.get_pixel_value` 都必须重新回到同一个 live replay event 读取；它们与 replacement 共享 `resolved_event_id` 与同一套 visual target 解析链。静态 `Capture Preview` 的默认截图语义是 `target.semantic="swapchain"`，用于观察最终 `Present` / swapchain backbuffer。
