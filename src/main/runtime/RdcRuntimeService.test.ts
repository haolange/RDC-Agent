...
let home = '';
let userData = '';
const service = new RdcRuntimeService();

beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-runtime-home-'));
  userData = fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-runtime-data-'));
...
describe('RdcRuntimeService', () => {
  ...
  it('writes user and project resources to canonical isolated paths with provenance status', () => {
    const project = fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-runtime-project-'));
    try {
      ...
      expect(agents.find((item) => item.scope === 'project')?.sourcePath).toContain(`${path.sep}.rdc-agent${path.sep}agents${path.sep}`);
    } finally { fs.rmSync(project, { recursive: true, force: true }); }
  });

  it('rejects project policy attempts to loosen user limits', () => {
    const project = fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-runtime-policy-'));
    try {
  ...
  it('aggregates resource and untrusted project-hook diagnostics into overview', () => {
    const project = fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-runtime-diagnostics-'));
    const trustPath = path.join(userData, 'hook-trust.json');
    const hooks = new HookEngine(trustPath);
    const hookService = new RdcRuntimeService(hooks);
    try {
  ...
  it('revokes project hook trust when the hook resource is deleted', () => {
    const project = fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-runtime-hook-'));
    const trustPath = path.join(userData, 'hook-trust.json');
    const hooks = new HookEngine(trustPath);
    const hookService = new RdcRuntimeService(hooks);
    try {
      hookService.upsert({ kind: 'hook', scope: 'project', projectRoot: project, id: 'verify', content: 'id: verify\nenabled: true\nevent: tool.before-call\ncommand: node\nargs: []\ntimeoutMs: 1000\nfailurePolicy: warn' });
      hooks.load(path.join(home, '.rdc-agent', 'hooks'), project);
