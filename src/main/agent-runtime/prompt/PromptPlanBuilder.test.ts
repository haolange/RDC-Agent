...
describe('PromptPlanBuilder', () => {
  it('builds a provenance-carrying plan without memory injection or hardcoded capabilities', async () => {
    ...
    const plan = new PromptPlanBuilder().build({
      ...
      skillCatalog: [],
      tools: ['read_file'], workDir: 'D:/Project', routeCapability: { providerId: 'deepseek', modelId: 'deepseek-v4', toolCallingMode: 'native-structured', reasoningVisibility: 'none', reasoningDelivery: 'stream-full', reasoningContract: { semantic: 'raw', source: 'deepseek-reasoning-content', displayLabel: 'Raw reasoning' }, supportsStreaming: true, supportsToolResults: true },
