...
export type RdxValidatedEvidence =
...
function invalid(): never { throw new Error('RDX_EVIDENCE_INVALID: native result does not provide supported execution evidence.'); }
export function verifyRollbackIdentity(definition: RdxOperationDefinition, args: Record<string, unknown>, contextId: string, experimentId?: string): void {
  if (definition.evidence_kind !== 'rollback' || !experimentId) return;
  const known = replacements.get(String(args.replacement_id));
  if (!known || known.contextId !== contextId || known.experimentId !== experimentId) throw new Error('RDX_EVIDENCE_INVALID: rollback must match a host-verified intervention for this experiment.');
}
export async function validateExecutionEvidence(definition: RdxOperationDefinition, args: Record<string, unknown>, result: Record<string, unknown>, context: ToolExecutionContext, contextId: string, experimentId: string): Promise<RdxValidatedEvidence> {
  ...
  if (result.frame_timing && typeof result.frame_timing === 'object') {
    ...
    const captureId = context.rdxBinding?.identity?.runtimeContext?.captureFileId;
