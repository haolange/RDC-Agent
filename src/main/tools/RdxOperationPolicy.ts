...
function denied(message: string): never { throw new Error('RDX_EXECUTION_DENIED: ' + message); }

export function validateRdxArguments(value: unknown, schema: Record<string, unknown>, location = 'args'): void {
  const satisfies = (candidate: unknown): boolean => {
    ...
    try { validateRdxArguments(value, candidate as Record<string, unknown>, location); return true; }
    catch { return false; }
  };
  ...
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    ...
    for (const [key, item] of Object.entries(record)) {
      ...
      if (properties[key]) validateRdxArguments(item, properties[key], `${location}.${key}`);
    }
  }
  if (Array.isArray(value)) for (const [index, item] of value.entries()) validateRdxArguments(item, (schema.items ?? {}) as Record<string, unknown>, `${location}[${index}]`);
}
...
export function authorizeRdxOperation(operation: string, input: Record<string, unknown>, context: ToolExecutionContext, replaySessionId: string, captureFileId?: string): { definition: RdxOperationDefinition; args: Record<string, unknown> } {
  ...
  const definition = binding.definitions.find(item => item.name === operation) as RdxOperationDefinition | undefined;
  ...
  validateRdxArguments(args, definition.input_schema);
