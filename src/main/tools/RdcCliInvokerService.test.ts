...
describe('RdcCliInvokerService.getRuntimeSummary', () => {
  it('returns diagnostic fields without recommendedSpecialists', async () => {
    const summary = await new RdcCliInvokerService().getRuntimeSummary();
    expect(summary).toMatchObject({
      runtime: expect.objectContaining({
        source: 'unconfigured',
        command: '',
        intermediateRoot: expect.stringMatching(/rdc-tool-intermediate$/u),
      }),
...
it('tags context-scoped action invocation ownership separately from native argv', async () => {
  const service = new RdcCliInvokerService();
...
import { operationFingerprint, RDC_APPLICATION_OPERATIONS } from './RdcOperationCatalog';
const roots = [0, 1].map(() => fs.mkdtempSync(path.join(os.tmpdir(), 'rdc-binding-test-')));
...
const versionReply = () => reply('rdc_tool.version', { tool_version: 'installed-build', schema_version: '3.0.0' });
const applicationOperations = Object.entries(RDC_APPLICATION_OPERATIONS).map(([name, contract]) => ({ ...operation, name, namespace: name.split('.')[1], scope: contract.scope, effects: contract.effects, input_schema: { type: 'object', additionalProperties: false, required: [], properties: Object.fromEntries(Object.entries(contract.parameters).map(([name, type]) => [name, { type }])) } }));
const catalogReply = () => reply('rdc_tool.tools.list', { schema_version: '1', tools: applicationOperations, tool_count: applicationOperations.length, fingerprint: operationFingerprint(applicationOperations) });
describe('configured CLI contract gate', () => {
  ...
  it.each([
    ['empty build identity', reply('rdc_tool.version', { tool_version: '', schema_version: '3.0.0' })],
    ['wrong kind', reply('other', { tool_version: 'installed-build', schema_version: '3.0.0' })],
    ['wrong schema', reply('rdc_tool.version', { tool_version: 'installed-build', schema_version: '2.0.0' })],
    ['CLI failure', { exitCode: 2, stdout: '', stderr: 'bad CLI', duration_ms: 1 }],
  ])('rejects %s before catalog use', async (_name, response) => {
    const service = new RdcCliInvokerService(); const execute = vi.spyOn(service, 'executeCLI').mockResolvedValue(response);
  ...
  it.each([
    ...
    const service = new RdcCliInvokerService();
    vi.spyOn(service, 'executeCLI').mockResolvedValueOnce(versionReply()).mockResolvedValueOnce(reply('rdc_tool.tools.list', { schema_version: '1', ...data }))
  ...
  it('keys catalog by executable, prefix, environment and working directory', async () => {
    const service = new RdcCliInvokerService(); const execute = vi.spyOn(service, 'executeCLI').mockImplementation(async command => command === 'version' ? versionReply() : catalogReply());
...
it.each(['schema', 'missing operation', 'missing argument'])('rejects incompatible application catalog: %s', async failure => {
  ...
  const service = new RdcCliInvokerService();
  vi.spyOn(service, 'executeCLI').mockResolvedValueOnce(versionReply()).mockResolvedValueOnce(reply('rdc_tool.tools.list', {
...
it('rechecks an installation updated at the same command path at the next boundary', async () => {
  ...
  execute.mockResolvedValueOnce(versionReply()).mockResolvedValueOnce(reply('rdc_tool.tools.list', { schema_version: '1', tools: [], tool_count: 0, fingerprint: operationFingerprint([]) }));
...
it.each(['1.0.0', '2.0.0', 'local-build'])('validates capabilities independently of package identity %s', async identity => {
  const service = new RdcCliInvokerService();
  vi.spyOn(service, 'executeCLI').mockResolvedValueOnce(reply('rdc_tool.version', { tool_version: identity, schema_version: '3.0.0' })).mockResolvedValueOnce(catalogReply());
  expect((await service.loadCatalog(cli)).tool_count).toBe(applicationOperations.length);
});
...
it('injects the host intermediate root into every CLI spawn env', async () => {
  ...
  const service = new RdcCliInvokerService({ invoke } as never);
  await service.executeCLI('version', ['--json'], { settings: cli });
  expect(invoke.mock.calls[0][0].env.RDC_TOOL_INTERMEDIATE_ROOT).toMatch(/rdc-tool-intermediate$/u);
});
...
it('fails closed before lifecycle or ordinary execution on an invalid installation', async () => {
  ...
  for (const command of ['call', 'context', 'daemon']) {
    const result = await service.executeCLI(command, ['open'], { settings: { ...cli, command: 'C:/Tools/rdc.bat' } });
    expect(result.exitCode).toBe(2);
    expect(result.stderr).toContain('RDC_BAT_REJECTED');
  }
...
it('invalidates cached success after a bound entry disappears', async () => {
  ...
  try { fs.unlinkSync(entry); await expect(service.loadCatalog(cli)).rejects.toThrow('RDC_BINDING_INVALID'); }
