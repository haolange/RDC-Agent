...
function signingKey(): string {
  ...
  if (!key) {
    if (secretStorageService.hasSecretRecord(KEY_REF)) throw new Error('RDX_RECEIPT_KEY_UNAVAILABLE: existing signing key cannot be decrypted.');
    key = randomBytes(32).toString('hex');
    secretStorageService.setSecret(KEY_REF, key);
  }
  return key;
}
export class RdxExecutionReceipts {
  constructor(
  ...
  write(receipt: RdxExecutionReceipt, signal?: AbortSignal): InvestigationContentRef {
    ...
    const written = this.resolver.write(receipt.sessionId,
      'session://tool-outputs/rdx-receipt-' + randomUUID() + '.json',
  ...
  read(sessionId: string, ref: InvestigationContentRef): RdxExecutionReceipt {
    ...
    if (read.category !== 'tool-outputs' || read.truncated || !read.text) throw new Error('RDX_RECEIPT_INVALID: incomplete receipt');
    ...
    if (typeof signed.body !== 'string' || typeof signed.signature !== 'string' || !/^[a-f0-9]{64}$/.test(signed.signature)) {
      throw new Error('RDX_RECEIPT_INVALID: unsigned content');
    }
...
export const rdxExecutionReceipts = new RdxExecutionReceipts();
