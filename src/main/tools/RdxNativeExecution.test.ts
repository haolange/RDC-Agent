...
const state = vi.hoisted(() => ({ receipts: null as RdxExecutionReceipts | null }));
vi.mock('./RdxExecutionReceipts', async (original) => ({
  ...await original<typeof import('./RdxExecutionReceipts')>(),
  rdxExecutionReceipts: {
    prepare: () => state.receipts!.prepare(),
    write: (...args: Parameters<RdxExecutionReceipts['write']>) => state.receipts!.write(...args),
  },
}));
...
describe.skipIf(!enabled)('native process execution and signed A-B-A (explicit disposable capture)', () => {
  it('changes actual pixels, rolls back, and verifies main-issued receipts', { timeout: 180_000 }, async () => {
    ...
    const native = async (command: string, args: string[] = []) => parseRdxNativeResult(
      await rdxCliInvokerService.executeCLI(command, [...args, '--daemon-context', contextId], { settings }));
    let replaySessionId = '';
    ...
    try {
      ...
      setRdxRuntimeContextForSession(sessionId, {
