...
export async function executeRdxShell(
  ...
  try {
    ...
    const after = assertRdxContextLeaseOwnership({ sessionId: context.sessionId, projectId: context.projectId });
    ...
    quarantineRdxContext(context.sessionId, lease.version, 'Native operation outcome is uncertain; inspect and recover through the capture lifecycle before further execution.');
    throw error;
  }
}
