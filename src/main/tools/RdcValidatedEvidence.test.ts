...
const definition = { evidence_kind: 'measurement' } as RdcOperationDefinition;
...
it('requires a verified applied replacement before signing rollback', async () => {
  const intervention = { evidence_kind: 'intervention' } as RdcOperationDefinition;
  const rollback = { evidence_kind: 'rollback' } as RdcOperationDefinition;
...
const frameContext = { rdcBinding: { identity: { runtimeContext: { captureFileId: 'capture-owned' } } } } as ToolExecutionContext;
const frameResult = {
