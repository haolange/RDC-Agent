...
import type { ConversationBranchState, ConversationFork } from '@shared/types/conversationBranch';
...
export function getForkNavigator(
  ...
  const activeIndex = fork.branches.findIndex((branch) => branch.branchId === fork.activeBranchId);
  const variantIndex = activeIndex >= 0 ? activeIndex : 0;
  return {
    variantIndex,
    variantCount: fork.branches.length,
    canPrev: variantIndex > 0,
    canNext: variantIndex < fork.branches.length - 1,
  };
}
