...
function isBuiltinProviderExplicitlyCustomized(provider: Partial<LlmProviderEntry>): boolean {
...
function sanitizeUserProvider(
  ...
  const builtinFallback = isBuiltinProviderId(rawId) ? createBuiltinProviderEntry(rawId) : undefined;
...
function hydrateProviderSecrets(
  ...
  return providers.map((provider) => {
    const resolvedSecret = provider.kind === 'ollama'
      ? ''
...
export class SettingsService {
  ...
  private rebuildPersistedSettings(
    ...
    const routeRefs = new Set(
      rawRoutes
        .map(sanitizeRoute)
        .filter((route): route is LlmAgentRoute => route !== null && Boolean(route.providerId))
