...
export async function withRdcOpenProgress<T>(contextId: string, cli: RdcCliInvokerSettings,
  ...
  const poll = async () => {
    while (!finished) {
      try {
        const result = parseRdcNativeResult(await rdcCliInvokerService.executeCLI('daemon', ['status', '--daemon-context', contextId], {
        ...
        if (!finished && result.result_kind === 'rdc_tool.daemon.status' && state?.context_id === contextId) {
