...
vi.mock('../tools/RdcCliInvokerService', () => ({ rdcCliInvokerService: { executeCLI: mocks.execute } }));
vi.mock('../settings/SettingsService', () => ({ settingsService: { getAll: () => ({ tooling: { rdcCli: { argsPrefix: [], enabled: true, command: 'rdc' } } }) } }));
...
it.each([
  ...
  await expect(observeReplay({ ...owner, backend: 'remote' }, {}, undefined, live)).rejects.toThrow('RDC_DISPLAY_RECEIPT');
});
