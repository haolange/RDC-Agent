...
`src/main/workflow/debugger/DebugWorkflowService.ts` 只作为 Runtime 内部执行服务保留，不能再作为 IPC、conversation、preload 或 renderer 的公开旁路入口。OpenAI / Claude Agent SDK 统一经 `AgentRunnerPort` 作为 stage 内 runner 接入，工具调用只能通过 `AgentToolPort -> ToolBridge` 回到 RDC 工具层。
