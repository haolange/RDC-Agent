...
`src/main/workflow/debugger/DebugWorkflowService.ts` 只作为 Runtime 内部执行服务保留，不能再作为 IPC、conversation、preload 或 renderer 的公开旁路入口。Agent turn 统一经 `AgentRuntime` 执行，工具调用只能通过 `ToolRegistry -> ToolBridge` 回到 RDC 工具层。
