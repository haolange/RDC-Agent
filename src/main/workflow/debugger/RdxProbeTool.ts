...
import { RDX_PROBE_ACTIONS, RDX_PROBE_READONLY_CLI_ACTIONS, RdxProbeInputSchema, compileRdxProbe, type RdxProbeInput } from '@shared/constants/rdxProbe';
...
export function createRdxProbeTool(
  sessionId?: string | null, projectId?: string | null, deps: RdxProbeToolDeps = {},
): AgentTool<RdxProbeInput, Record<string, unknown>> {
  const executeCli = deps.executeCli ?? ((command, args, options) => rdxCliInvokerService.executeCLI(command, args, options));
  const getLease = deps.getLease ?? getRdxContextLease;
  return {
    name: 'rdx_probe', label: 'RDX Probe',
    description: 'Query native rdx JSON through the frozen local CLI binding. Lease lifecycle is scoped to the current project capture. Never mutates capture bytes.',
    parameters: {
      type: 'object', required: ['action'], additionalProperties: false,
      properties: {
        action: { type: 'string', enum: [...RDX_PROBE_ACTIONS] },
        ...
        args: {
          ...
          properties: {
            action: { type: 'string', enum: [...RDX_PROBE_READONLY_CLI_ACTIONS] },
    ...
    async execute(_toolCallId, rawArgs, signal, _onUpdate, context) {
      try {
        ...
        if (!parsed.success) throw new Error('RDX_PROBE_SCHEMA: ' + parsed.error.issues.map((issue) => issue.message).join('; '));
        const input = parsed.data;
        signal?.throwIfAborted();
        if (context?.excludeRdxLeaseTools) throw new Error('RDX_PROBE_OWNER: offline child cannot access RDX.');
        const binding = context?.rdxBinding;
        const settings = deps.getRdxCliSettings?.() ?? binding?.cli;
        if (!settings?.enabled || !settings.command.trim()) throw new Error('RDX_PROBE_UNCONFIGURED: no frozen native CLI binding.');
        ...
        const compiled = compileRdxProbe(input);
        ...
        if (lease && (!current || current.version !== lease.version || current.contextId !== lease.contextId)) {
          throw new Error('RDX_PROBE_OWNER: lease changed during the call.');
        }
