...
export function WriteConfirmPanel({ write }: WriteConfirmPanelProps) {
  const { t } = useI18n();
  ...
  return (
    <div className="knowledge-center-sheet" data-confirmation-dialog data-testid="knowledge-center-confirm">
      ...
      {write.versionStale && <p className="knowledge-center-error">{t('knowledgeCenter.confirmVersionStale')}</p>}
      {write.error && <p className="knowledge-center-error">{write.error}</p>}
      ...
      <div className="knowledge-center-sheet-actions">
        ...
        <Button variant="secondary" onClick={write.close}>{t('knowledgeCenter.confirmCancel')}</Button>
      </div>
    </div>
  );
}
