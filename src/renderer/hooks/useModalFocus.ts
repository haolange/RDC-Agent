...
export function useModalFocus(options: {
  ...
  const { open, containerRef, onClose, busy = false, trap = true, trapRootRef } = options;
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      const container = containerRef.current;
      if (!container) return;
      const foreignConfirm = Array.from(document.querySelectorAll<HTMLElement>('[data-confirmation-dialog]'))
        .find((node) => !container.contains(node));
      if (foreignConfirm) return;
      if (event.key === 'Escape') {
        if (busy) return;
  ...
  }, [busy, containerRef, onClose, open, trap, trapRootRef]);
}
